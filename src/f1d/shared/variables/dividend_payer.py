"""Builder for DivDummy variable (1 if firm paid common dividends, else 0).
...
Returns one column: file_name, DivDummy.
"""
...
class DividendPayerBuilder(VariableBuilder):
    """Build DivDummy = (dvy > 0).astype(float) from raw Compustat quarterly data."""
    ...
    def build(self, years: range, root_path: Path) -> VariableResult:
        ...
        data = merged[["file_name", "DivDummy"]].copy()
        stats = self.get_stats(data["DivDummy"], "DivDummy")
        return VariableResult(
            data=data,
            stats=stats,
            metadata={"column": "DivDummy", "source": "Compustat/dvy>0"},
        )
