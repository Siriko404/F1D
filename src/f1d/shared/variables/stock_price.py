"""Builder for stock price at call date (H14 control variable).

Reads raw CRSP daily stock files via the shared CRSPEngine.get_raw_daily_data().
Returns columns: file_name, StockPrice.

StockPrice = PRC at call date (or nearest prior trading day).

VECTORIZED implementation using merge + groupby.
"""
...
class StockPriceBuilder(VariableBuilder):
    """Compute stock price at earnings call date (VECTORIZED).

    Control variable for H14: stock price level affects bid-ask spread.
    """
    ...
    def build(self, years: range, root_path: Path) -> VariableResult:
        ...
        # Load raw CRSP daily data (includes PRC)
        engine = get_engine()
        crsp_data = engine.get_raw_daily_data(root_path, years=list(years))

        if crsp_data.empty:
            logger.warning("StockPriceBuilder: No CRSP data loaded!")
            result_df = manifest[["file_name"]].copy()
            result_df["StockPrice"] = np.nan
            return VariableResult(
                data=result_df,
                stats=self.get_stats(result_df["StockPrice"], "StockPrice"),
                metadata={"column": "StockPrice", "source": "CRSP"},
            )

        # Compute stock price using vectorized operations
        results = self._compute_stock_price_vectorized(manifest, crsp_data)

        return VariableResult(
            data=results[["file_name", "StockPrice"]],
            stats=self.get_stats(results["StockPrice"], "StockPrice"),
            metadata={"column": "StockPrice", "source": "CRSP via get_raw_daily_data"},
        )
    ...
    def _compute_stock_price_vectorized(
        self, manifest: pd.DataFrame, crsp: pd.DataFrame
    ) -> pd.DataFrame:
        """Get stock price (PRC) at call date for each call (VECTORIZED + MEMORY EFFICIENT).

        Uses nearest prior trading day if call date is not a trading day.
        Processes in chunks by year to avoid memory explosion.
        """
        ...
        if len(valid) == 0:
            ...
            result["StockPrice"] = np.nan
            return result

        # Prepare CRSP data - add year column for filtering
        crsp = crsp.copy()
        ...
        for year in valid["year"].unique():
            year_calls = valid[valid["year"] == year].copy()
            ...
            # For each file_name, get the most recent price
            merged = merged.sort_values(["file_name", "date"], ascending=[True, False])
            latest = merged.groupby("file_name").first().reset_index()

            all_results.append(latest[["file_name", "PRC"]])

        if not all_results:
            logger.warning("StockPriceBuilder: No matching CRSP data!")
            result = manifest[["file_name"]].copy()
            result["StockPrice"] = np.nan
            return result
        ...
        result = manifest[["file_name"]].merge(
            combined.rename(columns={"PRC": "StockPrice"}),
            on="file_name",
            how="left",
        )

        logger.info(f"  StockPriceBuilder: {result['StockPrice'].notna().sum():,} valid observations")

        return result


__all__ = ["StockPriceBuilder"]
