"""Builder for 2-quarter lead Political Risk (PRiskQ_lead2) variable — H11-Lead2.

Loads Hassan et al. (2019) quarterly PRisk data and matches to earnings calls
by (gvkey, lead_calendar_quarter + 2).
...
Processing:
    ...
    2. Filter to requested years (and years +1,+2 for lead matches)
    3. Apply per-year 1%/99% winsorization
    4. For each call, determine calendar quarter from start_date
    5. Compute 2-quarter lead calendar quarter
    6. Merge on (gvkey, cal_q_lead2)

Output columns: file_name, PRiskQ_lead2

Temporal Structure:
    PRiskQ_lead2 is measured over calendar quarter Q+2
    Earnings call happens within calendar quarter Q
    2-quarter lead relationship
...
def _get_next2_quarter(cal_q: str) -> str:
    """Convert calendar quarter to 2 quarters ahead.

    Args:
        cal_q: Calendar quarter string in format "2010q2"

    Returns:
        Calendar quarter string 2 quarters ahead, e.g., "2010q2" → "2010q4", "2010q3" → "2011q1"
    """
...
def _load_prisk_quarterly_lead2(prisk_path: Path, years: range) -> pd.DataFrame:
    """Load and clean the Hassan PRisk quarterly CSV for lead-2 matching.

    Args:
        prisk_path: Path to the PRisk CSV file
        years: Range of years to include (also includes years +1,+2 for lead matches)
    ...
    # Filter to requested years PLUS next 2 years (for lead-2 matches)
    year_list = list(years)
    if year_list:
        year_list.extend([max(year_list) + 1, max(year_list) + 2])
...
class PRiskQLead2Builder(VariableBuilder):
    """Match 2-quarter lead PRisk onto each earnings call.

    For a call in calendar quarter Q, matches PRisk from quarter Q+2.
    This provides a longer lead independent variable for placebo testing.
    """

    def __init__(self, config: Dict[str, Any]):
        super().__init__(config)
        self.column = "PRiskQ_lead2"
    ...
    def build(self, years: range, root_path: Path) -> VariableResult:
        ...
        # 3. Compute 2-QUARTER LEAD calendar quarter
        manifest["cal_q_lead2"] = manifest["cal_q"].apply(_get_next2_quarter)

        # 4. Load quarterly PRisk data (includes 2 next years for lead-2 matches)
        prisk_path = root_path / PRISK_FILE
        print(f"    PRiskQLead2Builder: loading from {prisk_path.name} ...")
        prisk_df = _load_prisk_quarterly_lead2(prisk_path, years)
        print(f"    PRiskQLead2Builder: {len(prisk_df):,} firm-quarter PRisk rows loaded")

        # 5. Apply per-year winsorization (1%/99%)
        prisk_df = winsorize_by_year(prisk_df, ["PRisk"], year_col="year")
        print(f"    PRiskQLead2Builder: applied per-year 1%/99% winsorization")

        # 6. Merge on (gvkey, cal_q_lead2)
        merged = manifest.merge(
            prisk_df[["gvkey", "cal_q", "PRisk"]],
            left_on=["gvkey", "cal_q_lead2"],
            right_on=["gvkey", "cal_q"],
            how="left",
            suffixes=("", "_prisk"),
        )

        # Rename PRisk to PRiskQ_lead2
        merged = merged.rename(columns={"PRisk": "PRiskQ_lead2"})

        # 7. Align back to original manifest (preserve row order & count)
        data = manifest[["file_name"]].merge(
            merged[["file_name", "PRiskQ_lead2"]], on="file_name", how="left"
        )
        data = data.drop_duplicates(subset=["file_name"])

        # Compute match statistics
        n_matched = data["PRiskQ_lead2"].notna().sum()
        n_total = len(data)
        pct_matched = 100.0 * n_matched / n_total if n_total > 0 else 0.0
        print(
            f"    PRiskQLead2Builder: matched {n_matched:,} / {n_total:,} calls ({pct_matched:.1f}%)"
        )

        return VariableResult(
            data=data[["file_name", "PRiskQ_lead2"]].copy(),
            stats=self.get_stats(data["PRiskQ_lead2"], "PRiskQ_lead2"),
            metadata={
                "column": "PRiskQ_lead2",
                "source": "Hassan et al. (2019) quarterly PRisk",
                "description": (
                    "2-quarter lead political risk exposure matched to calls by "
                    "(gvkey, calendar_quarter+2). Per-year 1%/99% winsorized. "
                    "Lead by two quarters (t+2). Placebo test for reverse causality."
                ),
                "n_matched": n_matched,
                "n_total": n_total,
