...
def prepare_regression_data(
    ...
    if dw:
        ...
        dw.write(
            f"  Aggregated {before_agg:,} -> {after_agg:,} rows (removed duplicates)\n"
        )
    ...
    if dw:
        ...
        dw.write(
            f"  Final sample: {n_obs:,} obs, {n_firms} firms, {year_min}-{year_max}\n"
        )
...
def validate_data(df, dv_col, uncertainty_vars, control_vars, dw=None):
    ...
    if dw:
        ...
        # Check N >= 10,000
        if n_obs >= 10000:
            dw.write("    Sample size: PASS (>= 10,000)\n")
        else:
            dw.write("    Sample size: WARNING (< 10,000)\n")

        # Check N_firms >= 1,000
        if n_firms >= 1000:
            dw.write("    Firm count: PASS (>= 1,000)\n")
        else:
            dw.write("    Firm count: WARNING (< 1,000)\n")
...
def create_alternative_dvs(df, dw=None):
    ...
    if "roll_spread" in df_work.columns:
        ...
        if dw:
            dw.write("  WARNING: roll_spread not in data\n")

    # Bid-ask spread (if available)
    if "bidask_spread" in df_work.columns:
        df_work["bidask_spread_fwd"] = df_work.groupby("gvkey")["bidask_spread"].shift(
            -1
        )
        if dw:
        ...
        if dw:
            dw.write("  NOTE: bidask_spread not in data (optional DV)\n")

    return df_work


def create_timing_variants(df, dv_base="amihud_illiquidity", lags=None, dw=None):
    """
    ...
    for lag in lags:
        ...
        if dw:
            n_obs = len(df_lag)
            dw.write(
                f"  Timing {timing_names[lag]} ({dv_name}): {n_obs:,} observations\n"
            )

    return timing_variants


def run_h7_regression(
    df,
    uncertainty_var,
    dv_col="amihud_lag1",
    controls=None,
        ...
        for spec_name, spec_config in specs.items():
            ...
            if dw:
                if "error" not in result:
                    dw.write(
                        f"    N={result['n']}, R2w={result['r2_within']:.4f}, "
...
def run_robustness_suite(
    df, uncertainty_measures, control_vars, vif_threshold=5.0, dw=None
):
    """
    ...
    for dv_col, dv_label in alt_dvs:
        ...
        if len(df_alt) == 0:
            if dw:
                dw.write("    SKIPPED: No valid observations\n")
            continue
    ...
    if dw:
        ...
        dw.write(
            "  (H7 uses forward-looking DVs by design: amihud_lag1, roll_spread_lag1)\n"
        )
...
def apply_fdr_correction(results_list, alpha=0.05, dw=None):
    ...
    primary_results = [
        r for r in results_list if r["spec"] == "primary" and "error" not in r
    ]
    p_values = [
        r["p_one_sided"] for r in primary_results if not np.isnan(r["p_one_sided"])
    ]
...
def generate_results_markdown(
    results, output_dir, sample_stats, dv_col="amihud_lag1", dw=None
):
    """
    ...
    lines.append("")
    lines.append(
        f"**Sample:** {sample_stats['n_obs']:,} firm-years, {sample_stats['n_firms']:,} firms"
    )
    ...
    if len(fdr_signif) >= 4:  # Strong support
        ...
        lines.append(
            "PARTIALLY SUPPORTED - Some uncertainty measures predict illiquidity"
        )
    elif len(raw_signif) >= 2:
        lines.append("WEAK SUPPORT - Effects significant before FDR correction")
    else:
        lines.append(
            "NOT SUPPORTED - No significant effects of uncertainty on illiquidity"
        )

    lines.append("")
    lines.append(
        f"- Primary spec: {len(raw_signif)}/4 measures significant (p < 0.05, one-tailed)"
    )
    ...
    for r in primary_results:
        ...
        lines.append(
            f"| {var} | {beta:.6f} | {se:.6f} | {t:.2f} | {p:.4f} | {fdr:.4f} | {sig} |"
        )
    ...
    for spec_name in ["firm_only", "pooled", "double_cluster"]:
        spec_results = [
            r for r in results if r["spec"] == spec_name and "error" not in r
        ]
    ...
    lines.append("|---|---|---|---|---|---|---|")
    lines.append(
        "| primary | Yes | Yes | firm | "
        + f"{primary_results[0]['n']:,}"
        + f" | {len(raw_signif)}/4 | "
        + f"{np.mean([r['coefficient'] for r in primary_results if not np.isnan(r['coefficient'])]):.6f} |"
    )
    ...
    for spec_name, summ in spec_summary.items():
        ...
        lines.append(
            f"| {spec_name} | {fe} | {te} | {cl} | {summ['n']:,} | {summ['n_signif']}/4 | {summ['avg_beta']:.6f} |"
        )
    ...
    robustness_results = [
        ...
        if "error" not in r
        and (
            r.get("dv_type") != "Amihud (2002)"
            or r.get("iv_type") is not None
            or r.get("timing") is not None
        )
    ]
    ...
    if robustness_results:
        ...
        if alt_dv_results:
            ...
            for dv_type, group in dv_groups.items():
                ...
                avg_beta = np.mean(
                    [r["coefficient"] for r in group if not np.isnan(r["coefficient"])]
                )
                lines.append(
                    f"**{dv_type}:** {n_sig}/{len(group)} significant, avg beta = {avg_beta:.6f}"
                )
            lines.append("")
        else:
        ...
        if alt_iv_results:
            ...
            for iv_type, group in iv_groups.items():
                ...
                lines.append(
                    f"**{iv_type}:** {n_sig}/{len(group)} significant, avg beta = {avg_beta:.6f}"
                )
            lines.append("")
        else:
        ...
        if timing_results:
            ...
            for timing, group in timing_groups.items():
                ...
                lines.append(
                    f"**{timing}:** {n_sig}/{len(group)} significant, avg beta = {avg_beta:.6f}"
                )
            lines.append("")
        else:
        ...
        lines.append(
            f"**Robustness Summary:** {rob_sig}/{total_rob} ({rob_pct:.1f}%) robustness tests significant (p < 0.05)"
        )
        lines.append("")
    else:
        lines.append("## Robustness Tests")
    ...
    if len(fdr_signif) >= 4:
        lines.append(
            "**H7a SUPPORTED:** Managerial speech uncertainty predicts higher stock illiquidity."
        )
        lines.append("")
        lines.append(
            "The results provide strong support for the hypothesis that firms with more"
        )
        lines.append(
            "uncertain managerial speech face higher stock illiquidity, consistent with"
        )
        lines.append("increased information asymmetry and reduced informed trading.")
    elif len(fdr_signif) >= 2:
        lines.append(
            "**H7a PARTIALLY SUPPORTED:** Some uncertainty measures predict stock illiquidity."
        )
        lines.append("")
        lines.append(
            "The results provide partial support for the hypothesis. While some uncertainty"
        )
        lines.append(
            "measures show significant effects, the evidence is not consistent across all"
        )
        lines.append("measures.")
    elif len(raw_signif) >= 2:
        lines.append(
            "**H7a WEAK SUPPORT:** Effects present before multiple testing correction."
        )
        lines.append("")
        lines.append(
            "Some uncertainty measures show significant effects before FDR correction,"
        )
        lines.append(
            "but these do not survive multiple testing adjustment. The evidence is weak."
        )
    else:
        lines.append(
            "**H7a NOT SUPPORTED:** No significant effect of uncertainty on illiquidity."
        )
        lines.append("")
        lines.append(
            "The results do not support the hypothesis that managerial speech uncertainty"
        )
        lines.append(
            "predicts stock illiquidity. Any observed relationships are not statistically"
        )
...
def main():
    ...
    try:
        ...
        stats["processing"]["regression_prep"] = {
            ...
            "year_range": [
                stats["sample_stats"]["year_min"],
                stats["sample_stats"]["year_max"],
            ],
        }
        ...
        dw.write(
            f"  {len(UNCERTAINTY_MEASURES)} uncertainty measures x {len(SPECS)} specifications = "
        ...
        robustness_results = run_robustness_suite(
            reg_df, UNCERTAINTY_MEASURES, CONTROL_VARS, vif_threshold=5.0, dw=dw
        )
        ...
        # Count significant results
        primary_fdr_signif = sum(
            1
            for r in results
            if r["spec"] == "primary" and r.get("fdr_significant", False)
        )
        primary_raw_signif = sum(
            1
            for r in results
            if r["spec"] == "primary" and r.get("p_one_sided", 1) < 0.05
        )
