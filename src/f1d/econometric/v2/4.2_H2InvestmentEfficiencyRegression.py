...
def prepare_regression_data(
    ...
    reg_df = h2_df.merge(h1_leverage_df, on=merge_cols, how="left")

    if dw:
        missing_lev = reg_df["leverage"].isna().sum()
        dw.write(f"  After H1 leverage merge: {len(reg_df):,} obs ({missing_lev:,} missing leverage)\n")

    # Merge with speech data
    reg_df = reg_df.merge(speech_agg_df, on=merge_cols, how="left")

    if dw:
        missing_speech = reg_df[uncertainty_cols[0]].isna().sum() if uncertainty_cols else 0
        merge_rate = len(reg_df) / len(h2_df) * 100
        dw.write(
            f"  After speech merge: {len(reg_df):,} obs ({merge_rate:.1f}% of H2 data, {missing_speech:,} missing speech)\n"
        )
...
def generate_latex_table(
    ...
    for dv in ["efficiency_score", "roa_residual"]:
        ...
        dv_label = "Efficiency Score" if dv == "efficiency_score" else "ROA Residual"
        latex_path = output_dir / f"H2_Regression_Table_{dv}.tex"
        ...
        make_regression_table(
            ...
            caption=f"H2 Results: Speech Uncertainty and {dv_label}",
            label=f"tab:h2_{dv}",
