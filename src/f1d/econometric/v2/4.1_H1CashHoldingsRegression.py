...
def setup_paths(config: Dict[str, Any], timestamp: str) -> Dict[str, Path]:
    ...
    output_base = (
        root / "outputs" / "4_Econometric_V2" / "4.1_H1CashHoldingsRegression"
    )
    paths["output_dir"] = output_base / timestamp
    ensure_output_dir(paths["output_dir"])

    # Log directory - organize by script name
    log_base = root / "logs" / "4_Econometric_V2" / "4.1_H1CashHoldingsRegression"
...
def generate_results_markdown(
    ...
    lines.append("")
    lines.append(
        "| Uncertainty Measure | N | R2 | beta1 (SE) | p1 | beta3 (SE) | p3 | H1a | H1b |"
    )
    lines.append("|---|---|---|---|---|---|---|---|---|")
    ...
    lines.append("**Primary Specification:**")
    ...
    if primary_results:
        ...
        if top_h1a:
            lines.append("**Strongest support for H1a:**")
            for r in top_h1a[:3]:
                lines.append(
        ...
        if top_h1b:
            lines.append("**Strongest support for H1b:**")
            for r in top_h1b[:3]:
                lines.append(
...
def main() -> int:
    ...
    try:
        ...
        stats["input"]["h1_variables"] = {
            "rows": int(len(h1_df)),
            "unique_firm_years": int(
                h1_df[["gvkey", "fiscal_year"]].drop_duplicates().shape[0]
            ),
        ...
        stats["processing"]["regression_prep"] = {
            ...
            "year_range": [
                int(reg_df["fiscal_year"].min()),
                int(reg_df["fiscal_year"].max()),
            ],
        }
        ...
        dw.write(
            f"  Primary spec - H1b (beta3 < 0): {primary_signif_h1b}/6 significant\n"
        )
        dw.write("=" * 80 + "\n")
        dw.write("COMPLETE\n")
