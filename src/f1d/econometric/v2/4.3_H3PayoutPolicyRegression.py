...
"""
==============================================================================
STEP 4.3: H3 Payout Policy Regression
==============================================================================
ID: 4.3_H3PayoutPolicyRegression
Description: Panel OLS regressions for H3 (Speech Uncertainty & Payout Policy).
             Tests whether vague managers have less stable/more flexible dividend
             policies and whether leverage moderates this effect.

Model Specification:
    Payout_{t+1} = beta0 + beta1*Uncertainty_t + beta2*Leverage_t
                  + beta3*(Uncertainty_t * Leverage_t)
                  + gamma*Controls + Firm FE + Year FE + epsilon

Hypothesis Tests:

For div_stability (higher is more stable):
...
For payout_flexibility (higher is more flexible/volatile):
...
def load_h3_variables(h3_dir, dw=None):
    ...
    - Controls: earnings_volatility, fcf_growth, firm_maturity, firm_size, roa,
                tobins_q, cash_holdings

    Note: leverage is NOT in H3 data - must be merged from H1 data.
    """
...
def load_h1_leverage(h1_dir, dw=None):
    ...
    H1_CashHoldings.parquet has the leverage variable needed for H3 regressions.
    """
...
def prepare_regression_data(
    ...
    Steps:
    1. Merge H3 with H1 leverage on gvkey and fiscal_year (inner join)
    ...
    Args:
        h3_df: H3 variables with DVs and controls
...
def run_single_h3_regression(
    ...
    # Determine hypothesis test directions based on DV
    ...
    if dv_var == "div_stability":
        ...
        h3a_supported = (not np.isnan(p1_one)) and (p1_one < 0.05) and (beta1 < 0)
        h3b_supported = (not np.isnan(p3_one)) and (p3_one < 0.05) and (beta3 < 0)

    else:  # payout_flexibility
        # H3a_flexibility: beta1 > 0, H3b_flexibility: beta3 > 0
        if not np.isnan(p1_two) and not np.isnan(beta1):
            if beta1 > 0:
                p1_one = p1_two / 2
            else:
...
def save_regression_results(results, output_dir, dw=None):
    ...
    output_path = output_dir / "H3_Regression_Results.parquet"
...
def generate_results_markdown(results, output_dir, dw=None):
    """
    Generate human-readable markdown summary of H3 regression results.
    """
    lines = []
    lines.append("# H3 Payout Policy Regression Results")
    lines.append("")
...
def parse_args():
    ...
    parser = argparse.ArgumentParser(
        description="H3 Payout Policy Regression - Panel OLS with Uncertainty x Leverage interaction"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
...
def main():
    ...
    # Script header
    dw.write("=" * 80 + "\n")
    dw.write("STEP 4.3: H3 Payout Policy Regression\n")
    dw.write("=" * 80 + "\n")
    dw.write(f"Timestamp: {timestamp}\n")
    dw.write(f"Git SHA: {get_git_sha()}\n")
    dw.write(f"Config: {config.get('step_id', '4.3_H3PayoutPolicyRegression')}\n")
    dw.write("")
    ...
    try:
        ...
        for dv in DV_VARS:
            dw.write(f"\n  Primary spec ({dv}) - H3a: {h3a_counts[dv]}/6 significant\n")
            dw.write(f"  Primary spec ({dv}) - H3b: {h3b_counts[dv]}/6 significant\n")
        dw.write("=" * 80 + "\n")
        dw.write("COMPLETE\n")
