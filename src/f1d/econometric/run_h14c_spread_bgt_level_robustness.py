...
# ------------------------------------------------------------------
SUITE_ID = "H14c.r"
SUITE_DIR_NAME = "h14c_spread_bgt_level_robustness"
SUITE_TITLE = 'H14c: Speech Uncertainty and Bid-Ask Spread (Orthogonal Speaker Partition)'
SUITE_CAPTION = r'H14c Robustness: Orthogonal Speaker Partition'
SUITE_LABEL = "tab:h14c_robust"
...
def _save_latex_table(all_results, out_dir):
    ...
    lines = [
        ...
        r"\caption{H14c Robustness: Orthogonal Speaker Partition}",
        r"\label{tab:h14c_robust}", r"\scriptsize",
    ...
    with open(out_dir / "h14c_spread_bgt_level_robustness_table.tex", "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h14c_spread_bgt_level_robustness" / timestamp
    log_dir = setup_run_logging(log_base_dir=root / "logs", suite_name="H14c_BGTLevel_Spread_robust", timestamp=timestamp)
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics -- H14c BGTLevel_Spread Robustness (Main Sample)",
        label="tab:summary_stats_h14c_robust",
    )
    ...
    generate_manifest(
        ...
        output_files={"diagnostics": out_dir / "model_diagnostics.csv",
                      "table": out_dir / "h14c_spread_bgt_level_robustness_table.tex"},
