...
"""
================================================================================
STAGE 4: Test H20b Speech Uncertainty and Debt vs Equity Choice Hypothesis
================================================================================
ID: econometric/run_h20b_debt_choice
Description: Run H20b hypothesis test — does speech uncertainty predict the choice
             of debt vs equity financing? (Chang, Dasgupta & Hilary 2006, JF).
             Conditional on external financing.
             ChangDebtChoice = 1 if debt-only, 0 if equity-only.
             Dual issuers excluded per Chang et al. (2006).
...
Sample: Main only (FF12 != 8, 11). Restricted to external funders (ChangExternalFunding=1).
Leverage INCLUDED (not a bad control: Chang DV uses cash-flow items, not balance-sheet items).
SEs: Firm-clustered.
...
Outputs:
    - outputs/econometric/h20b_debt_choice/{timestamp}/...
...
# ------------------------------------------------------------------
SUITE_ID = "H20b.r"
SUITE_DIR_NAME = "h20b_debt_choice_robustness"
SUITE_TITLE = "H20b: Speech Uncertainty (Orthogonal Speaker Partition)"
SUITE_CAPTION = (
    r"H20b Robustness: Orthogonal Speaker Partition"
)
SUITE_LABEL = "tab:h20b_robust"
SAMPLE_LABEL = "Main sample (excludes financial and utility firms)."
HYP_DIR = "none"  # H20b: two-tailed (no signed prediction)
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Stage 4: H20b Debt Choice Chang (call-level)",
    )
...
def load_panel(root_path: Path, panel_path: Optional[str] = None) -> Tuple[pd.DataFrame, Path]:
    """Load call-level H19b/H20b financing panel."""
    print("\n" + "=" * 60)
    print("Loading H19b/H20b financing panel")
    print("=" * 60)
...
def restrict_to_external_funders(panel: pd.DataFrame) -> pd.DataFrame:
    """Restrict to external funders (ChangExternalFunding=1) only."""
    before = len(panel)
    panel = panel[panel["ChangExternalFunding"] == 1].copy()
    print(f"  External funders (ChangExternalFunding=1): {len(panel):,} / {before:,} "
          f"(dropped {before - len(panel):,} internal/NaN)")
    return panel
...
def prepare_regression_data(
    ...
    # Create Lagged_DV: ChangExternalFunding_lag (NOT ChangDebtChoice_lag — avoids 50% sample loss)
    panel = panel.copy()
...
def run_regression(
    ...
    for iv in KEY_IVS:
        ...
        # Two-tailed: H20b direction is theoretically ambiguous
...
def _save_latex_table(all_results: List[Dict[str, Any]], out_dir: Path) -> None:
    ...
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{H20b Robustness: Orthogonal Speaker Partition}",
        r"\label{tab:h20b_robust}",
        r"\scriptsize",
    ...
    lines += [
        ...
        r"ChangDebtChoice = 1 if debt-only financing, 0 if equity-only. ",
        r"Leverage included: ChangDebtChoice uses cash-flow items (dltisy$-$dltry$+$dlcchy), not balance-sheet items. ",
        r"Dual issuers excluded per Chang et al. (2006). ",
        r"Sample restricted to external funders (ChangExternalFunding=1). ",
    ...
    tex_path = out_dir / "h20b_debt_choice_robustness_table.tex"
...
def save_outputs(all_results: List[Dict[str, Any]], out_dir: Path) -> pd.DataFrame:
    ...
    for r in all_results:
        ...
        with open(out_dir / fname, "w", encoding="utf-8") as f:
            f.write(f"H20b Debt vs Equity Choice Regression (Chang et al. 2006)\n")
...
def _write_suite_spec_json(
    ...
    """Emit canonical suite_spec_H20b.json from runner state."""
    ...
    header_rows = [
        [
            {"label": "ChangDebtChoice", "span": 6},
            {"label": r"ChangDebtChoice\_lead", "span": 6},
        ]
    ]
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h20b_debt_choice_robustness" / timestamp

    log_dir = setup_run_logging(
        log_base_dir=root / "logs",
        suite_name="H20b_DebtChoice_robust",
        timestamp=timestamp,
    )

    print("=" * 80)
    print("STAGE 4: H20b Debt vs Equity Choice (Chang et al. 2006)")
    print("=" * 80)
    ...
    # Restrict to external funders BEFORE min-calls-per-firm
    panel = restrict_to_external_funders(panel)
    ext_n = len(panel)

    n_dv_valid = panel["ChangDebtChoice"].notna().sum()
    n_dv1 = (panel["ChangDebtChoice"] == 1).sum()
    n_dv0 = (panel["ChangDebtChoice"] == 0).sum()
    print(f"\n  After ChangExternalFunding=1 restriction: {ext_n:,} calls, {panel['gvkey'].nunique():,} firms")
    print(f"  ChangDebtChoice non-null: {n_dv_valid:,}")
    print(f"  ChangDebtChoice=1 (debt-only): {n_dv1:,} ({100*n_dv1/n_dv_valid:.2f}%)")
    print(f"  ChangDebtChoice=0 (equity-only): {n_dv0:,} ({100*n_dv0/n_dv_valid:.2f}%)")
    ...
    make_summary_stats_table(
        ...
        caption=r"Summary Statistics --- H20b Debt Choice Robustness (Chang et al. 2006) (External Funders, Main Sample)",
        label="tab:summary_stats_h20b_robust",
    )
    ...
    if all_results:
        ...
        attrition_stages = [
            ...
            ("External funders (ChangExternalFunding=1)", ext_n),
            ("ChangDebtChoice=1 (debt-only) in sample", n_dv1),
            ("After complete-case + min-calls (col 1)", first["n_obs"]),
        ]
        generate_attrition_table(attrition_stages, out_dir, "H20b Debt Choice (Chang) Robustness")
