...
"""
================================================================================
STAGE 4: Test H19 External vs Internal Financing Hypothesis
================================================================================
ID: econometric/run_h19_external_funding
Description: H19: Speech Uncertainty and External vs Internal Financing
             (Leary & Roberts 2010, JFE)

DV: ExternalFunding = 1 if firm used external financing in current fiscal year.
    ExternalFunding_lead = 1 if firm used external financing in next fiscal year.

12 Model Specifications:
    Cols 1-6:   DV = ExternalFunding (contemporaneous)
    Cols 7-12:  DV = ExternalFunding_lead (next fiscal year)
...
Inputs:
    - outputs/variables/h19_h20_financing/latest/h19_h20_financing_panel.parquet

Outputs:
    - outputs/econometric/h19_external_funding/{timestamp}/...
...
MODEL_SPECS = [
    # ExternalFunding (contemporaneous) — Calendar Year FE
    {"col": 1,  "dv": "ExternalFunding",      "fe": "industry",    "controls": "base",     "extra_controls": []},
    {"col": 2,  "dv": "ExternalFunding",      "fe": "firm",        "controls": "base",     "extra_controls": []},
    {"col": 3,  "dv": "ExternalFunding",      "fe": "industry",    "controls": "extended", "extra_controls": []},
    {"col": 4,  "dv": "ExternalFunding",      "fe": "firm",        "controls": "extended", "extra_controls": []},
    # ExternalFunding (contemporaneous) — Year-Quarter FE (Extended controls only)
    {"col": 5,  "dv": "ExternalFunding",      "fe": "industry_yq", "controls": "extended", "extra_controls": []},
    {"col": 6,  "dv": "ExternalFunding",      "fe": "firm_yq",     "controls": "extended", "extra_controls": []},
    # ExternalFunding_lead (next fiscal year) — Calendar Year FE
    {"col": 7,  "dv": "ExternalFunding_lead", "fe": "industry",    "controls": "base",     "extra_controls": []},
    {"col": 8,  "dv": "ExternalFunding_lead", "fe": "firm",        "controls": "base",     "extra_controls": []},
    {"col": 9,  "dv": "ExternalFunding_lead", "fe": "industry",    "controls": "extended", "extra_controls": []},
    {"col": 10, "dv": "ExternalFunding_lead", "fe": "firm",        "controls": "extended", "extra_controls": []},
    # ExternalFunding_lead (next fiscal year) — Year-Quarter FE (Extended controls only)
    {"col": 11, "dv": "ExternalFunding_lead", "fe": "industry_yq", "controls": "extended", "extra_controls": []},
    {"col": 12, "dv": "ExternalFunding_lead", "fe": "firm_yq",     "controls": "extended", "extra_controls": []},
]
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Stage 4: H19 External Funding (call-level)",
    )
...
def load_panel(root_path: Path, panel_path: Optional[str] = None) -> Tuple[pd.DataFrame, Path]:
    """Load call-level H19 panel."""
    print("\n" + "=" * 60)
    print("Loading H19 panel")
    print("=" * 60)
    ...
    if panel_path:
        ...
        panel_dir = get_latest_output_dir(
            root_path / "outputs" / "variables" / "h19_h20_financing",
            required_file="h19_h20_financing_panel.parquet",
        )
        panel_file = panel_dir / "h19_h20_financing_panel.parquet"
...
def prepare_regression_data(
    ...
    # Create Lagged_DV: previous fiscal year's ExternalFunding classification
    panel = panel.copy()
    panel["Lagged_DV"] = panel["ExternalFunding_lag"]
...
def _save_latex_table(all_results: List[Dict[str, Any]], out_dir: Path) -> None:
    ...
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{Speech Uncertainty and External vs Internal Financing}",
        r"\label{tab:h19_external_funding}",
        r"\scriptsize",
    ...
    lines.append(
        r" & \multicolumn{6}{c}{ExternalFunding}"
        r" & \multicolumn{6}{c}{ExternalFunding (next year)} \\"
    )
    ...
    lines += [
        ...
        r"ExternalFunding = 1 if firm used external financing; 0 = internal financing only. ",
    ...
    tex_path = out_dir / "h19_external_funding_table.tex"
...
def save_outputs(all_results: List[Dict[str, Any]], out_dir: Path) -> pd.DataFrame:
    ...
    for r in all_results:
        ...
        with open(out_dir / fname, "w", encoding="utf-8") as f:
            f.write(f"H19 External Funding Regression\n")
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h19_external_funding" / timestamp

    log_dir = setup_run_logging(
        log_base_dir=root / "logs",
        suite_name="H19_ExternalFunding",
        timestamp=timestamp,
    )

    print("=" * 80)
    print("STAGE 4: H19 External vs Internal Financing")
    print("=" * 80)
    ...
    n_external = (panel["ExternalFunding"] == 1).sum()
    n_internal = (panel["ExternalFunding"] == 0).sum()
    print(f"\n  Main sample: {main_n:,} calls, {panel['gvkey'].nunique():,} firms")
    print(f"  ExternalFunding non-null: {n_dv_valid:,}")
    print(f"  External=1: {n_external:,} ({100*n_external/n_dv_valid:.2f}%)")
    print(f"  Internal=0: {n_internal:,} ({100*n_internal/n_dv_valid:.2f}%)")
    print(f"  ExternalFunding_lead non-null: {panel['ExternalFunding_lead'].notna().sum():,}")
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics --- H19 External Funding (Main Sample)",
        label="tab:summary_stats_h19",
    )
    ...
    if all_results:
        ...
        attrition_stages = [
            ...
            ("ExternalFunding non-null", n_dv_valid),
            ("After complete-case + min-calls (col 1)", first["n_obs"]),
        ]
        generate_attrition_table(attrition_stages, out_dir, "H19 External Funding")
