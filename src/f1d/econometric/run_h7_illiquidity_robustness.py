...
# ------------------------------------------------------------------
SUITE_ID = "H7.r"
SUITE_DIR_NAME = "h7_illiquidity_robustness"
SUITE_TITLE = (
    "Speech Uncertainty and 3-Day Post-Call Illiquidity Change "
    "(Delta-Amihud, [+1,+3]-[-3,-1])"
)
SUITE_CAPTION = (
    r"H7: Speech Uncertainty and 3-Day Post-Call Illiquidity Change "
    r"($\Delta$Amihud, $[+1,+3]-[-3,-1]$)"
)
SUITE_LABEL = "tab:h7_robust"
SAMPLE_LABEL = "Main sample (excludes financial and utility firms)."
HYP_DIR = "positive"  # H7: beta(uncertainty) > 0 — more uncertainty -> higher illiquidity
...
def _save_latex_table(all_results, out_dir):
    ...
    lines = [
        ...
        r"\caption{H7 Robustness: Orthogonal Speaker Partition}",
        r"\label{tab:h7_robust}", r"\scriptsize",
    ...
    with open(out_dir / "h7_illiquidity_robustness_table.tex", "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    print(f"  Saved: h7_illiquidity_robustness_table.tex")
...
def _write_suite_spec_json(
    ...
    """Emit canonical suite_spec_H7.json from runner state."""
    ...
    header_rows = [
        [
            {"label": "DeltaILLIQ", "span": 6},
            {"label": r"DeltaILLIQ\_lead1", "span": 6},
        ]
    ]
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h7_illiquidity_robustness" / timestamp
    log_dir = setup_run_logging(log_base_dir=root / "logs", suite_name="H7_Illiquidity_robust", timestamp=timestamp)
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics -- H7 Illiquidity Robustness (Main Sample)",
        label="tab:summary_stats_h7_robust",
    )
