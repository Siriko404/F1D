...
"""
================================================================================
STAGE 4: Test H7c BGTLevel_Amihud (12-col 2-DV)
================================================================================
ID: econometric/run_h7c_amihud_bgt_level
...
Model Specifications (12 columns in one table):
    Cols 1-6:  DV = BGTLevel_Amihud (contemporaneous)
    Cols 7-12: DV = BGTLevel_Amihud_lead1 (next-quarter lead)
...
DV: BGTLevel_Amihud -- change in Amihud illiquidity around call ([+1,+3] - [-3,-1] days).
DV (lead): BGTLevel_Amihud_lead1 -- next-quarter call's BGTLevel_Amihud.
...
# ----------------------------------------------------------------------
# H7c window construction:
# BGT (2018) 25-day post-call Amihud illiquidity LEVEL. Window = [0, +25] trading days, day 0 INCLUDED per BGT verbatim. Day-0 inclusion is mechanically biased relative to a strictly post-call window because day 0 is the highest-volume day of the firm-quarter (earnings call); BGT-faithful, not a bug.
# ----------------------------------------------------------------------
...
# ------------------------------------------------------------------
SUITE_ID = "H7c.r"
SUITE_DIR_NAME = "h7c_amihud_bgt_level_robustness"
SUITE_TITLE = 'H7c: Speech Uncertainty and Illiquidity (Orthogonal Speaker Partition)'
SUITE_CAPTION = r'H7c Robustness: Orthogonal Speaker Partition'
SUITE_LABEL = "tab:h7c_robust"
...
MODEL_SPECS = [
    ...
    {"col": 1,  "dv": "BGTLevel_Amihud",       "fe": "industry",    "controls": "base"},
    {"col": 2,  "dv": "BGTLevel_Amihud",       "fe": "firm",        "controls": "base"},
    {"col": 3,  "dv": "BGTLevel_Amihud",       "fe": "industry",    "controls": "extended"},
    {"col": 4,  "dv": "BGTLevel_Amihud",       "fe": "firm",        "controls": "extended"},
    {"col": 5,  "dv": "BGTLevel_Amihud",       "fe": "industry_yq", "controls": "extended"},
    {"col": 6,  "dv": "BGTLevel_Amihud",       "fe": "firm_yq",     "controls": "extended"},
    # Next-quarter lead DV (cols 7-12)
    {"col": 7,  "dv": "BGTLevel_Amihud_lead1", "fe": "industry",    "controls": "base"},
    {"col": 8,  "dv": "BGTLevel_Amihud_lead1", "fe": "firm",        "controls": "base"},
    {"col": 9,  "dv": "BGTLevel_Amihud_lead1", "fe": "industry",    "controls": "extended"},
    {"col": 10, "dv": "BGTLevel_Amihud_lead1", "fe": "firm",        "controls": "extended"},
    {"col": 11, "dv": "BGTLevel_Amihud_lead1", "fe": "industry_yq", "controls": "extended"},
    {"col": 12, "dv": "BGTLevel_Amihud_lead1", "fe": "firm_yq",     "controls": "extended"},
]
...
SUMMARY_STATS_VARS = [
    {"col": "BGTLevel_Amihud", "label": "BGTLevel_Amihud$_t$"},
    {"col": "BGTLevel_Amihud_lead1", "label": "BGTLevel_Amihud$_{t+1}$"},
...
def _lag_column_for_dv(dv: str) -> str:
    ...
    For lead DV (e.g., BGTLevel_Amihud_lead1): returns the base DV name (the
...
def load_panel(root_path: Path, panel_path: Optional[str] = None) -> pd.DataFrame:
    ...
    columns = [
        ...
        # DV (contemp + lag + lead)
        "BGTLevel_Amihud", "BGTLevel_Amihud_lag", "BGTLevel_Amihud_lead1",
...
def _save_latex_table(all_results, out_dir):
    ...
    lines = [
        ...
        r"\caption{H7c Robustness: Orthogonal Speaker Partition}",
        r"\label{tab:h7c_robust}", r"\scriptsize",
    ...
    lines += [
        ...
        r"BGTLevel_Amihud$_t$ from BGT (2018, JAR) 25-day post-call window. ",
        r"BGTLevel_Amihud$_{t+1}$ = next-quarter call's BGTLevel_Amihud (calendar quarter, strict consecutive). ",
    ...
    with open(out_dir / "h7c_amihud_bgt_level_robustness_table.tex", "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h7c_amihud_bgt_level_robustness" / timestamp
    log_dir = setup_run_logging(log_base_dir=root / "logs", suite_name="H7c_BGTLevel_Amihud_robust", timestamp=timestamp)

    print("=" * 80)
    print("STAGE 4: Test H7c BGTLevel_Amihud (12-col 2-DV)")
    print("=" * 80)
    ...
    print(f"  BGTLevel_Amihud non-null: {panel['BGTLevel_Amihud'].notna().sum():,}")
    print(f"  BGTLevel_Amihud_lead1 non-null: {panel['BGTLevel_Amihud_lead1'].notna().sum():,}")
    for iv in KEY_IVS:
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics -- H7c BGTLevel_Amihud Robustness (Main Sample)",
        label="tab:summary_stats_h7c_robust",
    )
    ...
    if all_results:
        ...
        generate_attrition_table([
            ...
            ("BGTLevel_Amihud non-null", panel["BGTLevel_Amihud"].notna().sum()),
            ("BGTLevel_Amihud_lead1 non-null", panel["BGTLevel_Amihud_lead1"].notna().sum()),
            ("Complete-case + min-calls (col 1)", first_meta.get("n_obs", 0)),
        ], out_dir, "H7c BGTLevel_Amihud")

    generate_manifest(
        output_dir=out_dir, stage="stage4", timestamp=timestamp,
        input_paths={"panel": panel_file},
        output_files={"diagnostics": out_dir / "model_diagnostics.csv",
                      "table": out_dir / "h7c_amihud_bgt_level_robustness_table.tex"},
        panel_path=panel_file,
    )

    duration = (datetime.now() - start_time).total_seconds()
    with open(out_dir / "report_step4_H7c.md", "w", encoding="utf-8") as f:
        f.write(f"# H7c Report (12-col 2-DV)\n\n**Duration:** {duration:.1f}s\n**Sample:** Main only\n")
    print(f"  Saved: report_step4_H7c.md")
