...
"""
================================================================================
STAGE 4: Test H7e BGTAvg_Amihud (12-col 2-DV)
================================================================================
ID: econometric/run_h7e_amihud_bgt_avg
...
Model Specifications (12 columns in one table):
    Cols 1-6:  DV = BGTAvg_Amihud (contemporaneous)
    Cols 7-12: DV = BGTAvg_Amihud_lead1 (next-quarter lead)
...
DV: BGTAvg_Amihud -- change in Amihud illiquidity around call ([+1,+3] - [-3,-1] days).
DV (lead): BGTAvg_Amihud_lead1 -- next-quarter call's BGTAvg_Amihud.
...
# ----------------------------------------------------------------------
# H7e window construction:
# F1D extension of BGT (2018): mean Amihud over symmetric [-25, +25] trading days, day 0 INCLUDED (51-day window). Window length is BGT-verbatim, symmetric shape is F1D-pipeline convention.
# ----------------------------------------------------------------------
...
# ------------------------------------------------------------------
SUITE_ID = "H7e"
SUITE_DIR_NAME = "h7e_amihud_bgt_avg"
SUITE_TITLE = 'H7e: Speech Uncertainty and BGT-Window 25-Day Amihud Average (, 51-day symmetric)'
SUITE_CAPTION = r'H7e: Speech Uncertainty and BGT-Window 25-Day Amihud Average ($[-25,+25]$, 51-day symmetric)'
SUITE_LABEL = "tab:h7e"
...
MODEL_SPECS = [
    ...
    {"col": 1,  "dv": "BGTAvg_Amihud",       "fe": "industry",    "controls": "base"},
    {"col": 2,  "dv": "BGTAvg_Amihud",       "fe": "firm",        "controls": "base"},
    {"col": 3,  "dv": "BGTAvg_Amihud",       "fe": "industry",    "controls": "extended"},
    {"col": 4,  "dv": "BGTAvg_Amihud",       "fe": "firm",        "controls": "extended"},
    {"col": 5,  "dv": "BGTAvg_Amihud",       "fe": "industry_yq", "controls": "extended"},
    {"col": 6,  "dv": "BGTAvg_Amihud",       "fe": "firm_yq",     "controls": "extended"},
    # Next-quarter lead DV (cols 7-12)
    {"col": 7,  "dv": "BGTAvg_Amihud_lead1", "fe": "industry",    "controls": "base"},
    {"col": 8,  "dv": "BGTAvg_Amihud_lead1", "fe": "firm",        "controls": "base"},
    {"col": 9,  "dv": "BGTAvg_Amihud_lead1", "fe": "industry",    "controls": "extended"},
    {"col": 10, "dv": "BGTAvg_Amihud_lead1", "fe": "firm",        "controls": "extended"},
    {"col": 11, "dv": "BGTAvg_Amihud_lead1", "fe": "industry_yq", "controls": "extended"},
    {"col": 12, "dv": "BGTAvg_Amihud_lead1", "fe": "firm_yq",     "controls": "extended"},
]
...
SUMMARY_STATS_VARS = [
    {"col": "BGTAvg_Amihud", "label": "BGTAvg_Amihud$_t$"},
    {"col": "BGTAvg_Amihud_lead1", "label": "BGTAvg_Amihud$_{t+1}$"},
...
def _lag_column_for_dv(dv: str) -> str:
    ...
    For lead DV (e.g., BGTAvg_Amihud_lead1): returns the base DV name (the
...
def load_panel(root_path: Path, panel_path: Optional[str] = None) -> pd.DataFrame:
    ...
    columns = [
        ...
        # DV (contemp + lag + lead)
        "BGTAvg_Amihud", "BGTAvg_Amihud_lag", "BGTAvg_Amihud_lead1",
...
def _save_latex_table(all_results, out_dir):
    ...
    lines = [
        ...
        r"\caption{H7e: Speech Uncertainty and BGT-Window 25-Day Amihud AVERAGE (contemp + $t+1$ lead) [F1D extension]}",
        r"\label{tab:h7e_bgt_avg_amihud}", r"\scriptsize",
    ...
    lines += [
        ...
        r"BGTAvg_Amihud$_t$ from BGT (2018, JAR) 25-day post-call window. ",
        r"BGTAvg_Amihud$_{t+1}$ = next-quarter call's BGTAvg_Amihud (calendar quarter, strict consecutive). ",
    ...
    with open(out_dir / "h7e_amihud_bgt_avg_table.tex", "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
...
def _write_suite_spec_json(
    ...
    """Emit canonical suite_spec_H7e.json from runner state."""
    ...
    header_rows = [
        [
            {"label": r"BGTAvg\_Amihud", "span": 6},
            {"label": r"BGTAvg\_Amihud\_lead1", "span": 6},
        ]
    ]
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h7e_amihud_bgt_avg" / timestamp
    log_dir = setup_run_logging(log_base_dir=root / "logs", suite_name="H7e_BGTAvg_Amihud", timestamp=timestamp)

    print("=" * 80)
    print("STAGE 4: Test H7e BGTAvg_Amihud (12-col 2-DV)")
    print("=" * 80)
    ...
    print(f"  BGTAvg_Amihud non-null: {panel['BGTAvg_Amihud'].notna().sum():,}")
    print(f"  BGTAvg_Amihud_lead1 non-null: {panel['BGTAvg_Amihud_lead1'].notna().sum():,}")
    for iv in KEY_IVS:
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics -- H7e BGTAvg_Amihud (Main Sample)",
        label="tab:summary_stats_h7e",
    )
    ...
    if all_results:
        ...
        generate_attrition_table([
            ...
            ("BGTAvg_Amihud non-null", panel["BGTAvg_Amihud"].notna().sum()),
            ("BGTAvg_Amihud_lead1 non-null", panel["BGTAvg_Amihud_lead1"].notna().sum()),
            ("Complete-case + min-calls (col 1)", first_meta.get("n_obs", 0)),
        ], out_dir, "H7e BGTAvg_Amihud")

    generate_manifest(
        output_dir=out_dir, stage="stage4", timestamp=timestamp,
        input_paths={"panel": panel_file},
        output_files={"diagnostics": out_dir / "model_diagnostics.csv",
                      "table": out_dir / "h7e_amihud_bgt_avg_table.tex"},
        panel_path=panel_file,
    )

    duration = (datetime.now() - start_time).total_seconds()
    with open(out_dir / "report_step4_H7e.md", "w", encoding="utf-8") as f:
        f.write(f"# H7e Report (12-col 2-DV)\n\n**Duration:** {duration:.1f}s\n**Sample:** Main only\n")
    print(f"  Saved: report_step4_H7e.md")
