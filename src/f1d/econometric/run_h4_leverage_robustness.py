...
FE Time Index: cal_yr (calendar year) or cal_yr_qtr (calendar year-quarter for _yq specs).
...
Outputs:
    ...
    - outputs/econometric/h4_leverage/{timestamp}/h4_leverage_robustness_table.tex
...
# ------------------------------------------------------------------
SUITE_DIR_NAME = "h4_leverage_robustness"
...
SUB_TABLE_SPECS = [
    {
        "suite_id": "H4a.r",
        "title": "Speech Uncertainty and Book Leverage (Orthogonal Speaker Partition)",
    ...
    {
        "suite_id": "H4b.r",
        "title": "Speech Uncertainty and Debt-to-Capital (Orthogonal Speaker Partition)",
...
def run_regression(
    ...
    Time index: cal_yr (calendar year) or cal_yr_qtr (calendar year-quarter).
    """
    col_num = spec["col"]
    dv = spec["dv"]
    fe_type = spec["fe"]
    ...
    # MultiIndex: gvkey (entity) × time (calendar year or calendar year-quarter)
    ...
    # Build metadata with per-IV one-tailed p-values (H4: β<0, uncertainty → less leverage)
    meta: Dict[str, Any] = {
        "col": col_num,
        "dv": dv,
        "fe": fe_type,
    ...
    # Per-IV coefficients with one-tailed p-values (H4: β<0, uncertainty → less leverage)
    for iv in KEY_IVS:
        ...
        # One-tailed β<0: halve p if β<0 (supports H4), else 1 - p/2
...
def _save_latex_table(all_results: List[Dict[str, Any]], out_dir: Path) -> None:
    ...
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{Speech Uncertainty and Book Leverage (Orthogonal Speaker Partition)}",
        r"\label{tab:h4a_robust}",
        r"\scriptsize",
    ...
    lines += [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{Speech Uncertainty and Debt-to-Capital (Orthogonal Speaker Partition)}",
        r"\label{tab:h4b_robust}",
        r"\scriptsize",
        r"\begin{tabular}{l" + "c" * n_cols + "}",
        r"\toprule",
    ]
    ...
    lines += [
        ...
        r"Calendar Year FE uses \texttt{cal\_yr}; Cal Year-Qtr FE uses \texttt{cal\_yr\_qtr}. ",
    ...
    tex_path = out_dir / "h4_leverage_robustness_table.tex"
    with open(tex_path, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    print(f"  Saved: h4_leverage_robustness_table.tex")
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h4_leverage_robustness" / timestamp

    # Setup logging
    log_dir = setup_run_logging(
        log_base_dir=root / "logs",
        suite_name="H4_Leverage_robust",
        timestamp=timestamp,
    )
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics — H4 Leverage Robustness (Main Sample)",
        label="tab:summary_stats_h4_robust",
    )
    ...
    if all_results:
        ...
        generate_attrition_table(attrition_stages, out_dir, "H4 Leverage Robustness")
    ...
    generate_manifest(
        ...
        output_files={
            ...
            "table": out_dir / "h4_leverage_robustness_table.tex",
        },
    ...
    # H4 significance summary (one-tailed)
    for iv in KEY_IVS:
        sig_count = sum(
