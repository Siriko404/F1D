...
"""
================================================================================
STAGE 4: Test H1.2 Financing-Constraint-Moderated Cash Holdings — CEO 2-IV Variant
================================================================================
ID: econometric/run_h1_2_cash_constraint_ceo2iv
Description: CEO-only 2-IV variant of H1.2. The main speech-uncertainty IV set
             is restricted to UncAnsCEO_c (CEO Q&A) + UncPreCEO_c (CEO Presentation),
             both mean-centered on the Main sample. Drops UncAnsMgr/UncPreMgr
             and UncAnsNoCEO/UncPreNoCEO entirely. Two interaction terms estimated:
             UncAnsCEO_c × Unrated and UncPreCEO_c × Unrated. BelowIG level dummy
             is retained; BelowIG interactions are SUPPRESSED per project_step8_
             infrastructure_progress H1.2 BelowIG suppression decision
             (IG vs Unrated only).
...
Model Specification (Unrated moderator retained; BelowIG level only):
    CashRatio = b1*UncAnsCEO_c + b2*UncPreCEO_c
              + b3*BelowIG + b4*Unrated
              + b5*(UncAnsCEO_c x Unrated) + b6*(UncPreCEO_c x Unrated)
              + controls + IndustryFE + CalendarYearFE + e

    Reference group: IG + BelowIG firms (Unrated=0) — level-only BelowIG shift.
    b5 = differential sensitivity of CEO Q&A uncertainty → cash for unrated firms
    b6 = differential sensitivity of CEO Pres uncertainty → cash for unrated firms
    b1,b2 = main CEO-speech-uncertainty slopes for rated (IG∪BelowIG) firms
    b3, b4 = level shifts at mean uncertainty
...
Hypothesis: All CEO main IVs (UncAnsCEO_c, UncPreCEO_c) and Unrated interactions
    (UncAnsCEO_c × Unrated, UncPreCEO_c × Unrated) one-tailed positive per HFC
    framework. BelowIG/Unrated level shifts: two-tailed (no directional prior).
...
Outputs:
    - outputs/econometric/h1_2_cash_constraint_ceo2iv/{timestamp}/...
...
# CEO-only 2-IV stack (both mean-centered on Main sample)
IVS_RAW = ["UncAnsCEO", "UncPreCEO"]
IVS_CENTERED = ["UncAnsCEO_c", "UncPreCEO_c"]
...
# Three-category moderator. BelowIG LEVEL is kept, but BelowIG interactions are
# suppressed (per H1.2 BelowIG suppression decision — IG vs Unrated only).
MOD_BELOW_IG = "BelowIG"
MOD_UNRATED = "Unrated"
# Per-IV interaction term names (Unrated only)
INT_UNRATED_UNCANSCEO = "UncAnsCEO_c_x_Unrated"
INT_UNRATED_UNCPRECEO = "UncPreCEO_c_x_Unrated"
INT_UNRATED_TERMS = [INT_UNRATED_UNCANSCEO, INT_UNRATED_UNCPRECEO]
...
# H1.2.ceo2 displays interaction cols (5-8 + 13-16 → renumbered 1-8).
# The top main-IV rows (UncAnsCEO_c, UncPreCEO_c) pull from the matching
# unconditional spec (interaction_col - 4); moderator levels and Unrated
# interactions come from the interaction spec.
# ------------------------------------------------------------------
SUITE_ID = "H1.2.ceo2"
SUITE_DIR_NAME = "h1_2_cash_constraint_ceo2iv"
SUITE_TITLE = (
    "Financial Constraint-Moderated CEO Speech Uncertainty and Cash Holdings "
    "(CEO 2-IV: Q&A + Presentation)"
)
SUITE_CAPTION = (
    "H1.2 CEO 2-IV: Financial Constraint--Moderated CEO Speech Uncertainty and "
    "Cash Holdings"
)
SUITE_LABEL = "tab:h1_2_ceo2"
...
SUMMARY_STATS_VARS = [
    ...
    {"col": "UncAnsCEO", "label": "CEO QA Uncertainty (raw)"},
    {"col": "UncAnsCEO_c", "label": "CEO QA Uncertainty (centered)"},
...
def prepare_regression_data(
    ...
    required = ([dv] + IVS_RAW + IVS_CENTERED + [MOD_BELOW_IG, MOD_UNRATED]
    ...
    # (BelowIG interactions SUPPRESSED — IG vs Unrated only)
    if use_interactions:
        df[INT_UNRATED_UNCANSCEO] = df["UncAnsCEO_c"] * df[MOD_UNRATED]
        df[INT_UNRATED_UNCPRECEO] = df["UncPreCEO_c"] * df[MOD_UNRATED]
...
def run_regression(
    ...
    use_interactions = spec.get("interactions", True)
    # Always include both centered CEO IVs + both moderator level dummies.
    # Interaction specs add Unrated interactions (BelowIG interactions suppressed).
    base_exog = list(IVS_CENTERED) + [MOD_BELOW_IG, MOD_UNRATED]
    if use_interactions:
    ...
    # Extract main CEO IV coefficients (always one-tailed positive per HFC)
    beta_qa, se_qa, p_two_qa = _extract_coef(model, "UncAnsCEO_c")
    beta_pre, se_pre, p_two_pre = _extract_coef(model, "UncPreCEO_c")
    # Moderator levels (two-tailed)
    beta_big, se_big, p_two_big = _extract_coef(model, MOD_BELOW_IG)
    beta_unr, se_unr, p_two_unr = _extract_coef(model, MOD_UNRATED)

    if use_interactions:
        beta_int_qa, se_int_qa, p_two_int_qa = _extract_coef(model, INT_UNRATED_UNCANSCEO)
        beta_int_pre, se_int_pre, p_two_int_pre = _extract_coef(model, INT_UNRATED_UNCPRECEO)
    else:
        beta_int_qa, se_int_qa, p_two_int_qa = np.nan, np.nan, np.nan
        beta_int_pre, se_int_pre, p_two_int_pre = np.nan, np.nan, np.nan

    # One-tailed p for both main IVs (expected positive per HFC)
    def _one_tailed(b: float, p2: float) -> float:
        if np.isnan(p2) or np.isnan(b):
            return float("nan")
        return p2 / 2 if b > 0 else 1 - p2 / 2

    p_one_qa = _one_tailed(beta_qa, p_two_qa)
    p_one_pre = _one_tailed(beta_pre, p_two_pre)
    # Per HFC framework, interactions are also one-tailed positive
    p_one_int_qa = _one_tailed(beta_int_qa, p_two_int_qa)
    p_one_int_pre = _one_tailed(beta_int_pre, p_two_int_pre)

    print(f"  [OK] {elapsed:.1f}s | R2={model.rsquared:.4f}  Adj R2={1 - (1 - model.rsquared) * (model.nobs - 1) / model.df_resid:.4f}")
    print(f"  UncAnsCEO_c: b={beta_qa:.4f} p1={p_one_qa:.4f} {_sig_stars_one(p_one_qa)}")
    print(f"  UncPreCEO_c: b={beta_pre:.4f} p1={p_one_pre:.4f} {_sig_stars_one(p_one_pre)}")
    print(f"  {MOD_BELOW_IG}: b={beta_big:.4f} p2={p_two_big:.4f}")
    print(f"  {MOD_UNRATED}: b={beta_unr:.4f} p2={p_two_unr:.4f}")
    if use_interactions:
        print(f"  {INT_UNRATED_UNCANSCEO}: b={beta_int_qa:.4f} p1={p_one_int_qa:.4f} "
              f"{_sig_stars_one(p_one_int_qa)}")
        print(f"  {INT_UNRATED_UNCPRECEO}: b={beta_int_pre:.4f} p1={p_one_int_pre:.4f} "
              f"{_sig_stars_one(p_one_int_pre)}")
    ...
    meta = {
        ...
        # CEO Q&A main IV
        "beta_iv_qa": beta_qa, "se_iv_qa": se_qa,
        "p_one_iv_qa": p_one_qa, "p_two_iv_qa": p_two_qa,
        # CEO Pres main IV
        "beta_iv_pre": beta_pre, "se_iv_pre": se_pre,
        "p_one_iv_pre": p_one_pre, "p_two_iv_pre": p_two_pre,
        # Below-IG level
        ...
        # Unrated interactions (per-IV)
        "beta_int_unrated_qa": beta_int_qa, "se_int_unrated_qa": se_int_qa,
        "p_one_int_unrated_qa": p_one_int_qa, "p_two_int_unrated_qa": p_two_int_qa,
        "beta_int_unrated_pre": beta_int_pre, "se_int_unrated_pre": se_int_pre,
        "p_one_int_unrated_pre": p_one_int_pre, "p_two_int_unrated_pre": p_two_int_pre,
        "extra_controls": ",".join(extra_controls) if extra_controls else "",
        # VIF
        "vif_int_unrated_qa": vif.get(INT_UNRATED_UNCANSCEO, np.nan) if (vif and use_interactions) else np.nan,
        "vif_int_unrated_pre": vif.get(INT_UNRATED_UNCPRECEO, np.nan) if (vif and use_interactions) else np.nan,
        # Counts
...
def _save_latex_table(all_results: List[Dict[str, Any]], out_dir: Path) -> None:
    ...
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{H1.2 CEO 2-IV: Financial Constraint--Moderated CEO Speech Uncertainty and Cash Holdings}",
        r"\label{tab:h1_2_ceo2}",
        r"\small",
    ...
    # Main CEO Q&A IV (one-tailed)
    _row(r"UncAnsCEO\_c", "beta_iv_qa", "se_iv_qa", "p_one_iv_qa", _sig_stars_one)
    # Main CEO Pres IV (one-tailed)
    _row(r"UncPreCEO\_c", "beta_iv_pre", "se_iv_pre", "p_one_iv_pre", _sig_stars_one)
    # Below-IG level (two-tailed)
    _row("BelowIG", "beta_below_ig", "se_below_ig", "p_two_below_ig", _sig_stars_two)
    # Unrated level (two-tailed)
    _row("Unrated", "beta_unrated", "se_unrated", "p_two_unrated", _sig_stars_two)
    # Interaction: UncAnsCEO_c x Unrated (one-tailed)
    _row(r"UncAnsCEO\_c\_x\_Unrated", "beta_int_unrated_qa",
         "se_int_unrated_qa", "p_one_int_unrated_qa", _sig_stars_one)
    # Interaction: UncPreCEO_c x Unrated (one-tailed)
    _row(r"UncPreCEO\_c\_x\_Unrated", "beta_int_unrated_pre",
         "se_int_unrated_pre", "p_one_int_unrated_pre", _sig_stars_one)
    ...
    lines += [
        ...
        r"CEO 2-IV variant of H1.2: restricts the speech-uncertainty IV stack to ",
        r"two CEO-only measures (UncAnsCEO Q\&A, UncPreCEO Presentation), both mean-centered ",
        r"on the Main sample. ",
        r"Main IVs (UncAnsCEO\_c, UncPreCEO\_c) and Unrated interactions: one-tailed ($\beta > 0$). ",
        r"BelowIG and Unrated level dummies: two-tailed. ",
        r"BelowIG interactions SUPPRESSED (IG vs Unrated only). ",
        r"Reference group for level dummies: investment-grade firms (S\&P long-term issuer rating BBB$-$ or above). ",
    ...
    tex_path = out_dir / "h1_2_cash_constraint_ceo2iv_table.tex"
...
def save_outputs(all_results: List[Dict[str, Any]], out_dir: Path) -> pd.DataFrame:
    ...
    for r in all_results:
        ...
        with open(out_dir / fname, "w", encoding="utf-8") as f:
            ...
            f.write(f"H1.2 CEO 2-IV Financing-Constraint-Moderated Cash Holdings [{spec_type}]\n")
            f.write(f"Col: ({col_num})\n")
            f.write(f"DV: {meta['dv']}\n")
            f.write(f"IVs: {', '.join(IVS_RAW)} (mean-centered)\n")
            f.write(f"Moderators: BelowIG (level only), Unrated (level + interactions)\n")
            if has_int:
                f.write(f"Interactions: {', '.join(INT_UNRATED_TERMS)} (BelowIG interactions SUPPRESSED)\n")
            else:
            ...
            if has_int:
                f.write(f"VIF(int_unrated_qa): {meta.get('vif_int_unrated_qa', 'N/A')}\n")
                f.write(f"VIF(int_unrated_pre): {meta.get('vif_int_unrated_pre', 'N/A')}\n")
...
def generate_report(
    ...
    """Generate markdown report for CEO 2-IV variant."""
    iv_means_str = ", ".join(f"{k}={v:.4f}" for k, v in iv_means.items())
    lines = [
        "# H1.2 CEO 2-IV Financing-Constraint-Moderated Cash Holdings Report",
        "",
        f"**Generated:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        f"**Duration:** {duration:.1f} seconds",
        f"**Design:** UncAnsCEO_c, UncPreCEO_c (CEO 2-IV) × Unrated interactions",
        f"          (BelowIG interactions SUPPRESSED — IG vs Unrated only)",
        f"**Channel:** CH1 — Precautionary liquidity under external-finance frictions",
        f"**Moderator:** Three-category: IG/BelowIG (joint ref slope) / Unrated (interaction)",
        f"**IV centering means:** {iv_means_str}",
        f"**Sample years:** {YEAR_MIN}-{YEAR_MAX}",
        f"**Parent suite:** H1.2 (Cash × Constraint)",
        "",
        "## Results",
        "",
        "| Col | DV | Spec | b_qa (p1) | b_pre (p1) | b_int_qa (p1) | b_int_pre (p1) | N | R2 |",
        "|-----|----|------|-----------|------------|---------------|----------------|---|-----|",
    ]
    ...
    for r in all_results:
        ...
        if not m:
            continue
        s_qa = _sig_stars_one(m["p_one_iv_qa"])
        s_pre = _sig_stars_one(m["p_one_iv_pre"])
        if m.get("interactions"):
            s_int_qa = _sig_stars_one(m["p_one_int_unrated_qa"])
            s_int_pre = _sig_stars_one(m["p_one_int_unrated_pre"])
            int_qa_str = f"{m['beta_int_unrated_qa']:.4f}{s_int_qa} ({m['p_one_int_unrated_qa']:.3f})"
            int_pre_str = f"{m['beta_int_unrated_pre']:.4f}{s_int_pre} ({m['p_one_int_unrated_pre']:.3f})"
        else:
            int_qa_str = "—"
            int_pre_str = "—"
        ...
        lines.append(
            ...
            f"{m['beta_iv_qa']:.4f}{s_qa} ({m['p_one_iv_qa']:.3f}) | "
            f"{m['beta_iv_pre']:.4f}{s_pre} ({m['p_one_iv_pre']:.3f}) | "
            f"{int_qa_str} | {int_pre_str} | "
            f"{m['n_obs']:,} | {m['r2']:.4f} |"
        )
    ...
    lines += [
        ...
        "## Interpretation",
        "",
        "- b_qa, b_pre: CEO-speech-uncertainty slopes for rated firms (IG ∪ BelowIG joint slope)",
        "- b_int_qa, b_int_pre: differential sensitivity for unrated firms",
        "- If b_int_qa > 0 and significant: HFC channel supported for unrated (constrained-by-no-rating)",
        "- BelowIG interactions suppressed (IG vs Unrated only); BelowIG dummy retained as level shift",
    ]

    with open(out_dir / "report_step4_H1_2_ceo2iv.md", "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    print("  Saved: report_step4_H1_2_ceo2iv.md")
...
def _write_suite_spec_json(
    ...
    """Emit canonical suite_spec_H1.2.ceo2.json from CEO 2-IV moderation runner state.

    16 underlying regressions:
      - Cols 1-4: unconditional specs, DV=CashRatio_t
      - Cols 5-8: interaction specs, DV=CashRatio_t (BelowIG interactions SUPPRESSED)
      - Cols 9-12: unconditional specs, DV=CashRatio_lead
      - Cols 13-16: interaction specs, DV=CashRatio_lead

    Display table = 8 columns: interaction cols 5-8 + 13-16. Per parent
    convention, main CEO IV slopes (`UncAnsCEO_c`, `UncPreCEO_c`) come from
    the matched UNCONDITIONAL spec (interaction_col - 4); moderator levels
    and Unrated interactions come from the interaction spec.
    """
    results_by_col = {
    ...
    for interaction_col in display_cols:
        ...
        # Interaction-model coefs: moderator levels + Unrated interactions
        # (BelowIG interactions intentionally absent — suppressed).
        interaction_vars = [
            MOD_BELOW_IG,
            MOD_UNRATED,
            INT_UNRATED_UNCANSCEO,
            INT_UNRATED_UNCPRECEO,
        ]
        ...
        # Main CEO IV slopes from the unconditional spec (one-tailed positive).
        uncond_coefs = extract_coefs_panelols(
            model=uncond_model,
            key_ivs=list(IVS_CENTERED),
            all_vars=list(IVS_CENTERED),
            hyp_dir="positive",
        )

        merged = {}
        for ivc in IVS_CENTERED:
            if ivc in uncond_coefs:
                merged[ivc] = uncond_coefs[ivc]
        merged.update(interaction_coefs)
        coefs_per_col.append(merged)

    # Display IVs: 2 main CEO IVs + Unrated level + 2 Unrated interactions.
    # BelowIG level estimated but not displayed (matches parent T53 convention).
    ivs = [
        {"name": "UncAnsCEO_c", "label": r"UncAnsCEO\_c", "tail": "one_pos"},
        {"name": "UncPreCEO_c", "label": r"UncPreCEO\_c", "tail": "one_pos"},
        {"name": MOD_UNRATED, "label": "Unrated", "tail": "one_pos"},
        {"name": INT_UNRATED_UNCANSCEO,
         "label": r"UncAnsCEO\_c\_x\_Unrated", "tail": "one_pos"},
        {"name": INT_UNRATED_UNCPRECEO,
         "label": r"UncPreCEO\_c\_x\_Unrated", "tail": "one_pos"},
    ]
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h1_2_cash_constraint_ceo2iv" / timestamp

    log_dir = setup_run_logging(
        log_base_dir=root / "logs",
        suite_name="H1_2_CashConstraint_ceo2iv",
        timestamp=timestamp,
    )

    print("=" * 80)
    print("STAGE 4: H1.2 CEO 2-IV Financing-Constraint-Moderated Cash Holdings")
    print("=" * 80)
    print(f"Timestamp:  {timestamp}")
    print(f"Output:     {out_dir}")
    print(f"Design:     2 CEO IVs × 2 DVs × (base + interaction) × 4 FE types = 16 models (8 displayed)")
    print(f"Channel:    CH1 — Precautionary liquidity under external-finance frictions")
    print(f"Moderator:  IG/BelowIG joint slope (ref) vs Unrated (interaction); BelowIG interactions SUPPRESSED")
    ...
    # Center both CEO IVs on Main sample
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics --- H1.2 CEO 2-IV Cash Holdings (Main Sample, 2002--2016)",
        label="tab:summary_stats_h1_2_ceo2",
    )
    ...
    if all_results:
        ...
        generate_attrition_table(
            ...
            "H1.2 CEO 2-IV Financing-Constraint-Moderated Cash Holdings",
        )
    ...
    for r in all_results:
        ...
        if m.get("interactions"):
            ...
            print(f"  Col ({m['col']}) {m['dv']} [int]: "
                  f"QA b={m['beta_iv_qa']:.4f}{s_qa} | "
                  f"Pre b={m['beta_iv_pre']:.4f}{s_pre} | "
                  f"Int(QA×Unr) b={m['beta_int_unrated_qa']:.4f}{s_int_qa} | "
                  f"Int(Pre×Unr) b={m['beta_int_unrated_pre']:.4f}{s_int_pre}")
        else:
            print(f"  Col ({m['col']}) {m['dv']} [base]: "
                  f"QA b={m['beta_iv_qa']:.4f}{s_qa} | Pre b={m['beta_iv_pre']:.4f}{s_pre}")
...
if __name__ == "__main__":
    ...
    if args.dry_run:
        ...
        print(f"  Interactions: {', '.join(INT_UNRATED_TERMS)} (BelowIG SUPPRESSED)")
