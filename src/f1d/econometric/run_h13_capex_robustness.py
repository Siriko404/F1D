...
# ------------------------------------------------------------------
SUITE_ID = "H13.r"
SUITE_DIR_NAME = "h13_capex_robustness"
SUITE_TITLE = "Speech Uncertainty and Capital Expenditure (Orthogonal Speaker Partition)"
SUITE_CAPTION = "H13 Robustness: Orthogonal Speaker Partition"
SUITE_LABEL = "tab:h13_robust"
SAMPLE_LABEL = "Main sample (excludes financial and utility firms)."
HYP_DIR = "none"  # H13: two-tailed, no signed prediction
...
def run_regression(
    ...
    # MultiIndex: gvkey (entity) x time_col (calendar year or cal yr-qtr)
...
def _save_latex_table(all_results: List[Dict[str, Any]], out_dir: Path) -> None:
    ...
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{Speech Uncertainty and Capital Expenditure (Orthogonal Speaker Partition)}",
        r"\label{tab:h13_robust}",
        r"\scriptsize",
    ...
    tex_path = out_dir / "h13_capex_robustness_table.tex"
    with open(tex_path, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    print(f"  Saved: h13_capex_robustness_table.tex")
...
def _write_suite_spec_json(
    ...
    """Emit canonical suite_spec_H13.json from runner state."""
    ...
    iv_tail = "two"  # two-tailed — no signed hypothesis
    header_rows = [
        [
            {"label": "Capex", "span": 6},
            {"label": r"Capex\_lead", "span": 6},
        ]
    ]
    ...
    paths = write_suite_spec(
        ...
        tail=TAIL,
        ivs=[{"name": iv, "label": iv, "tail": iv_tail} for iv in KEY_IVS],
        controls={
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h13_capex_robustness" / timestamp

    # Setup logging
    log_dir = setup_run_logging(
        log_base_dir=root / "logs",
        suite_name="H13_Capex_robust",
        timestamp=timestamp,
    )
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics -- H13 Capital Expenditure Robustness (Main Sample)",
        label="tab:summary_stats_h13_robust",
    )
    ...
    if all_results:
        ...
        generate_attrition_table(attrition_stages, out_dir, "H13 Capital Expenditure Robustness")
