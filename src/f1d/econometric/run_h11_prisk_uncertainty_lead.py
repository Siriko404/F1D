...
Description: Run H11-Lead Political Risk hypothesis tests by loading the call-level
             ...
             Tests BOTH 1-quarter lead (PRisk_lead) AND 2-quarter lead (PRisk_lead2).

Model Specification:
    Uncertainty_t ~ PRisk_lead_t + Controls_it + EntityEffects + TimeEffects
    Uncertainty_t ~ PRisk_lead2_t + Controls_it + EntityEffects + TimeEffects
...
Independent Variables:
    - PRisk_lead: Political risk from quarter t+1 (1-quarter lead)
    - PRisk_lead2: Political risk from quarter t+2 (2-quarter lead)
...
Hypothesis Tests (two-tailed, placebo/falsification test):
    H11-Lead:  beta(PRisk_lead) = 0  -- future political risk should NOT predict current speech uncertainty
    H11-Lead2: beta(PRisk_lead2) = 0 -- future political risk should NOT predict current speech uncertainty
...
CONFIG = {
    ...
    "iv_vars": ["PRisk_lead", "PRisk_lead2"],  # Test both lead-1 and lead-2
}
...
SUMMARY_STATS_VARS = [
    ...
    {"col": "PRisk_lead", "label": "Political Risk$_{t+1}$"},
    {"col": "PRisk_lead2", "label": "Political Risk$_{t+2}$"},
    # Controls
...
def run_regression(
    ...
    hyp_name = "h11_lead_sig" if iv_var == "PRisk_lead" else "h11_lead2_sig"
...
def _save_latex_table(all_results: List[Dict[str, Any]], out_dir: Path) -> None:
    ...
    def get_res_lead1(dv):
        for r in all_results:
            if r.get("sample") == "Main" and r.get("dv") == dv and r.get("iv") == "PRisk_lead":
                return r
        return None
    ...
    def get_res_lead2(dv):
        for r in all_results:
            if r.get("sample") == "Main" and r.get("dv") == dv and r.get("iv") == "PRisk_lead2":
                return r
        return None

    r_mq_1 = get_res_lead1("UncAnsMgr")
    r_cq_1 = get_res_lead1("UncAnsCEO")
    r_mp_1 = get_res_lead1("UncPreMgr")
    r_cp_1 = get_res_lead1("UncPreCEO")

    r_mq_2 = get_res_lead2("UncAnsMgr")
    r_cq_2 = get_res_lead2("UncAnsCEO")
    r_mp_2 = get_res_lead2("UncPreMgr")
    r_cp_2 = get_res_lead2("UncPreCEO")
...
def main(panel_path: str | None = None) -> int:
        ...
        for dv in CONFIG["dependent_variables"]:
            for sample in CONFIG["samples"]:
                ...
                if model is not None:
                    ...
                    lead_suffix = "lead1" if iv_var == "PRisk_lead" else "lead2"
    ...
    if all_results:
        main_result = next(
            (r for r in all_results if r.get("sample") == "Main" and r.get("iv") == "PRisk_lead"),
            all_results[0]
        )
        attrition_stages = [
