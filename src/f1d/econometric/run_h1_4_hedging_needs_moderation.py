...
"""
================================================================================
STAGE 4: H1.4 HedgingNeeds Channel — CEO 3-IV Decomp × HighHN (ACW 2007)
================================================================================
ID: econometric/run_h1_4_hedging_needs_moderation
Description: Cash-flow-volatility-moderated CEO speech uncertainty and cash
             ...
             with HedgingNeeds moderator from ACW (2007) JFI Section 4.2.3 p.538-539
             verbatim: corr(firm CF, ind 3yr-ahead median sales growth) of quarterly OCF
             (StdDev/|Mean|), lagged 1 quarter.

             Single regression per cell on Main sample. HedgingNeeds is built via
             HedgingNeedsBuilder (returns one column on file_name).
             HighHN_dummy = 1[HedgingNeeds >= ff12-year median]. Reference group =
             Low HedgingNeeds (below-median) firms.

             2 HighHN interactions estimated: UncResCEO × HighHN (state
             amplification) + UncPreCEO × HighHN (presentation amplification).
             ClarityCEO × HighHN DROPPED — trait × moderator has ambiguous
             direction (mirrors H1.2 decomp design).

             Sample window 2004-2018: pre-2004 calls have NaN HedgingNeeds because
             Compustat datadate min = 2000-01-31 and ACW requires 16 trailing
             quarters. Plan default: drop pre-2004Q1 (preserves ACW verbatim).
...
Tail directions:
    ...
    Interaction UncResCEO × HighHN: one-tail POS
        (HedgingNeeds-conditional ACTIVATION at state level — speech uncertainty
         engages the cash response only among high-HedgingNeeds firms; ACW
         constrained-vs-unconstrained asymmetry extended to speech channel)
    Interaction UncPreCEO × HighHN: one-tail POS
        (HedgingNeeds-conditional ACTIVATION at presentation level)
    HighHN level dummy: two-tailed (no directional prior on level shift).

Channel: CH-HedgingNeeds — Precautionary liquidity amplification by realized cash-flow
volatility (ACW 2007 JCF asymmetry: cash holdings of constrained firms
increase in HedgingNeeds; insignificant for unconstrained firms — extended to speech
uncertainty interaction).

Parent suite: H1.2.ceo2.decomp (HFC moderator equivalent design pattern).
Anchor: ACW (2007) JCF Section 3 p.51-52 verbatim CV(CF) formula.

Model Specification:
    CashRatio = b1*ClarityCEO_c + b2*UncResCEO_c + b3*UncPreCEO_c
              + b4*HighHN
              + b5*(UncResCEO_c x HighHN) + b6*(UncPreCEO_c x HighHN)
...
Moderator: BINARY HighHN dummy from HedgingNeedsBuilder.
    HighHN = 1 if HedgingNeeds >= ff12-year median; else 0.
    Built from Compustat oancfy via ACW (2007) 16q CV.
    Merge: VariableResult on file_name (no look-ahead — HedgingNeeds lagged 1 quarter).
...
Sample: Main only (FF12 not in {8, 11}). Fiscal years 2002-2018. Full panel
    ...
    1990-2002 pulled separately to avoid the post-2004 ACW 16q window cliff).
    Empirical HedgingNeeds coverage: 2002=82%, 2018=94%; total 90.8% non-null.
...
Inputs:
    ...
    - inputs/comp_na_daily_all/comp_na_daily_all.parquet (via HedgingNeedsBuilder)

Outputs:
    - outputs/econometric/h1_4_hedging_needs_moderation/{timestamp}/...
...
from f1d.shared.variables import HedgingNeedsBuilder
...
# Binary moderator (HighHN vs LowHN reference) per ACW 2007 ff12-year median split.
MOD_HIGH_HN = "HighHedgingNeeds"

# 2 HedgingNeeds interaction terms (Clarity × HighHN DROPPED — trait × moderator mixes
# competing forces; no clean monotone prediction; mirrors H1.2 decomp design).
INT_UNCRES_HIGHHN = "UncResCEO_c_x_HighHN"
INT_UNCPRE_HIGHHN = "UncPreCEO_c_x_HighHN"
ALL_INTERACTIONS = [INT_UNCRES_HIGHHN, INT_UNCPRE_HIGHHN]
...
DISPLAY_IVS = [
    ...
    MOD_HIGH_HN,          # Row 4 (int, two-tail)
    INT_UNCRES_HIGHHN,    # Row 5 (int, POS)
    INT_UNCPRE_HIGHHN,    # Row 6 (int, POS)
]
...
IV_TAIL_DIRECTION: Dict[str, str] = {
    ...
    MOD_HIGH_HN:         "none",  # two-tailed level dummy
    INT_UNCRES_HIGHHN:   "positive",
    INT_UNCPRE_HIGHHN:   "positive",
}
...
VARIABLE_LABELS = {
    ...
    MOD_HIGH_HN:         "HighHedgingNeeds",
    INT_UNCRES_HIGHHN:   r"UncRes $\times$ HighHN",
    INT_UNCPRE_HIGHHN:   r"UncPre $\times$ HighHN",
}

MIN_CALLS_PER_FIRM = 5
YEAR_MIN = 2002  # full panel; HedgingNeeds pre-2004 backfilled via Compustat_Quarterly_OCF_Extended (1990-2002 gap fill)
YEAR_MAX = 2018
...
# ------------------------------------------------------------------
SUITE_ID = "H1.4.hedging_needs"
SUITE_DIR_NAME = "h1_4_hedging_needs_moderation"
SUITE_TITLE = (
    "Cash-Flow-Volatility-Moderated CEO Speech Uncertainty and Cash Holdings "
    "(CEO 3-IV Decomp x HighHN; ACW 2007)"
)
SUITE_CAPTION = (
    r"H1.4 CEO 3-IV Decomp HedgingNeeds: ClarityCEO + UncResCEO (DWZ Full) + "
    r"UncPreCEO $\times$ HighHN (ACW 16q CV)"
)
SUITE_LABEL = "tab:h1_4_hedging_needs"
SAMPLE_LABEL = (
    ...
    "ACW 16-trailing-quarter HedgingNeeds available from 1994-Q1 onwards in source data."
)
...
SUMMARY_STATS_VARS = [
    ...
    {"col": MOD_HIGH_HN, "label": "HighHN (dummy, ff12-year median)"},
    {"col": "HedgingNeeds", "label": "HedgingNeeds (ACW 16q CV)"},
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Stage 4: H1.4 HedgingNeeds-Moderated Cash Holdings (ACW 2007)",
    )
...
def load_and_merge_hedging_needs(panel: pd.DataFrame, root_path: Path, years: range) -> pd.DataFrame:
    """Load ACW HedgingNeeds via HedgingNeedsBuilder + binarize on ff12-year median.

    HighHN = 1 if call's lagged HedgingNeeds >= median across (ff12_code, year) cell; else 0.
    Reference group: LowHN (below industry-year median).
    Calls with NaN HedgingNeeds get NaN HighHN — dropped at complete-case filter.

    ACW (2007) JFI Section 4.2.3 p.538-539 verbatim: 5-yr rolling correlation between
    firm CashFlow and FF12-industry 3-yr-ahead median sales growth.
    """
    print("\n" + "=" * 60)
    print("Merging HedgingNeeds (ACW 2007 corr formula) + binarizing on ff12-year median")
    print("=" * 60)

    builder = HedgingNeedsBuilder({})
    hedging_needs_result = builder.build(years, root_path)
    hedging_needs_df = hedging_needs_result.data  # columns: file_name, HedgingNeeds

    print(f"  HedgingNeeds builder: {len(hedging_needs_df):,} rows; non-null = {hedging_needs_df['HedgingNeeds'].notna().sum():,}")

    before = len(panel)
    panel = panel.merge(hedging_needs_df, on="file_name", how="left", validate="one_to_one")
    assert len(panel) == before, f"HedgingNeeds merge changed row count: {before} -> {len(panel)}"
    ...
    median_by_iy = panel.groupby(["ff12_code", "year"])["HedgingNeeds"].transform("median")
    panel[MOD_HIGH_HN] = np.where(
        panel["HedgingNeeds"].notna() & median_by_iy.notna(),
        (panel["HedgingNeeds"] >= median_by_iy).astype(float),
        np.nan,
    )

    n_high = int((panel[MOD_HIGH_HN] == 1).sum())
    n_low = int((panel[MOD_HIGH_HN] == 0).sum())
    n_na = int(panel[MOD_HIGH_HN].isna().sum())
    n_total = len(panel)

    print(f"  HedgingNeeds coverage: {n_high + n_low:,}/{n_total:,} ({100*(n_high+n_low)/n_total:.1f}%)")
    print(f"    HighHN (>= ff12-year median):  {n_high:,} ({100*n_high/n_total:.1f}%)")
    print(f"    LowHN  (<  ff12-year median):  {n_low:,} ({100*n_low/n_total:.1f}%)")
    print(f"    NaN (no HedgingNeeds — pre-existence/gap): {n_na:,} ({100*n_na/n_total:.1f}%)")
...
def prepare_regression_data(
    ...
    required = ([dv] + IVS_REG_A_RAW + IVS_REG_A_CENTERED + [MOD_HIGH_HN]
    ...
    if use_interactions:
        df[INT_UNCRES_HIGHHN] = df["UncResCEO_c"] * df[MOD_HIGH_HN]
        df[INT_UNCPRE_HIGHHN]   = df["UncPreCEO_c"] * df[MOD_HIGH_HN]
    ...
    n_high = int(df[MOD_HIGH_HN].sum())
    n_low = int((df[MOD_HIGH_HN] == 0).sum())
    print(f"  After >={MIN_CALLS_PER_FIRM} calls/firm: "
          f"{len(df):,} calls, {n_firms:,} firms, {n_time_periods:,} firm-time-periods")
    print(f"  Binary split: {n_low:,} LowHN ({100*n_low/len(df):.1f}%) / "
          f"{n_high:,} HighHN ({100*n_high/len(df):.1f}%)")
...
def run_regression(
    ...
    if use_interactions:
        exog = KEY_IVS + [MOD_HIGH_HN] + ALL_INTERACTIONS + all_controls
    else:
        exog = KEY_IVS + [MOD_HIGH_HN] + all_controls
    ...
    n_high = int(df_prepared[MOD_HIGH_HN].sum())
    n_low = int((df_prepared[MOD_HIGH_HN] == 0).sum())
    ...
    if vif and use_interactions:
        ...
        meta[f"vif_{MOD_HIGH_HN}"] = vif.get(MOD_HIGH_HN, np.nan)
...
def _save_latex_table(all_results: List[Dict[str, Any]], out_dir: Path) -> None:
    ...
    lines += [
        ...
        r"\textit{ClarityCEO} $\times$ HighHN NOT estimated: trait $\times$ moderator mixes competing forces ",
        r"(volatility pushes cash UP via precautionary motive; high clarity pushes cash DOWN via reduced perception) --- ",
        r"no clean monotone HedgingNeeds prediction. ",
        r"$^{*}p<0.10$, $^{**}p<0.05$, $^{***}p<0.01$ (one-tailed for directional IVs; two-tailed for HighHN level). ",
        r"Per-IV directions: \textit{ClarityCEO} $\beta < 0$; \textit{UncResCEO}, \textit{UncPreCEO}, ",
        r"and HedgingNeeds interactions $\beta > 0$; \textit{HighHN} level two-tailed. ",
        r"Moderator is BINARY: HighHN vs LowHN reference group (ACW 2007; HighHN $=$ above-median ",
        r"HedgingNeeds within (FF12-industry, calendar-year) cell). ",
        r"HedgingNeeds = corr(firm CF, ind 3yr-ahead median sales growth) (std/$|$mean$|$) of quarterly OCF, lagged one quarter (ACW 2007 Section 3 p.52 verbatim). ",
    ...
    tex_path = out_dir / "h1_4_hedging_needs_moderation_table.tex"
...
def save_outputs(all_results: List[Dict[str, Any]], out_dir: Path) -> pd.DataFrame:
    ...
    for r in all_results:
        ...
        with open(out_dir / fname, "w", encoding="utf-8") as f:
            ...
            f.write(f"H1.4 HedgingNeeds-Moderated Cash Holdings (CEO 3-IV Decomp x HighHN; ACW 2007) [{spec_type}]\n")
            f.write(f"Col: ({col_num})\n")
            f.write(f"DV: {meta['dv']}\n")
            f.write(f"IVs: {', '.join(IVS_REG_A_RAW)} (DWZ Eq.5 Full; mean-centered)\n")
            f.write(f"Moderator: HighHN only (binary; ff12-year median split; ACW 2007)\n")
            if has_int:
                f.write(f"Interactions: {', '.join(ALL_INTERACTIONS)}\n")
                f.write(f"Tail: UncResxHighHN + UncPrexHighHN one-tail POS (ClarityxHighHN DROPPED)\n")
            else:
            ...
            f.write(f"N: LowHN={meta.get('n_low', 'N/A')}, HighHN={meta.get('n_high', 'N/A')}\n")
...
def generate_report(
    ...
    """Generate markdown report for H1.4 HedgingNeeds moderation."""
    iv_means_str = ", ".join(f"{k}={v:+.4f}" for k, v in iv_means.items())
    lines = [
        "# H1.4 HedgingNeeds-Moderated CEO Speech Uncertainty and Cash Holdings (ACW 2007)",
        "",
        f"**Generated:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        f"**Duration:** {duration:.1f} seconds",
        f"**Design:** Cash ~ Clarity_c + UncRes_c + UncPre_c + HighHN [+ 2 ints] + ctrls + FE",
        f"**Display rows (6 IVs):**",
        f"  1. ClarityCEO_c (NEG)             2. UncResCEO_c (POS)              3. UncPreCEO_c (POS)",
        f"  4. HighHN (two-tail)           5. UncResCEO_c x HighHN (POS)  6. UncPreCEO_c x HighHN (POS)",
        f"**Per-IV tails:** ClarityCEO NEG; UncResCEO + UncPreCEO + HedgingNeeds interactions POS; HighHN two-tailed.",
        f"**ClarityCEO x HighHN DROPPED**: trait x moderator mixes competing forces; no clean monotone HedgingNeeds prediction.",
        f"**Channel:** CH-HedgingNeeds - ACW (2007) precautionary HedgingNeeds amplification extended to speech uncertainty",
        f"**IV centering means:** {iv_means_str}",
        f"**Sample years:** {YEAR_MIN}-{YEAR_MAX}",
        "",
        "## Results (interaction-spec coefs; conditional slopes at HighHN=0 = LowHN baseline)",
        "",
    ...
    for r in all_results:
        ...
        if m.get("interactions"):
            b_int_a = m.get(f"{INT_UNCRES_HIGHHN}_beta", np.nan)
            p_int_a = m.get(f"{INT_UNCRES_HIGHHN}_p_one", np.nan)
            b_int_p = m.get(f"{INT_UNCPRE_HIGHHN}_beta", np.nan)
            p_int_p = m.get(f"{INT_UNCPRE_HIGHHN}_p_one", np.nan)
    ...
    lines += [
        ...
        "- UncResCEO main slope = conditional slope at HighHN=0 (LowHN baseline) — null in interaction specs",
        "- UncPreCEO main slope = conditional slope at HighHN=0 (LowHN baseline) — null in interaction specs",
        "- INT_UNCRES_HIGHHN (POS): HedgingNeeds-CONDITIONAL ACTIVATION at state level (engages only in high-HedgingNeeds firms)",
        "- INT_UNCPRE_HIGHHN (POS): HedgingNeeds-CONDITIONAL ACTIVATION at presentation level",
        "- Trait x moderator interaction NOT estimated (theoretically ambiguous direction)",
        "- Reference group: LowHN (below ff12-year median) firms; binary moderator per ACW 2007 design",
        "",
        "## Framing note (corrected post-run)",
        "",
        "ACTIVATION not AMPLIFICATION: in interaction specs, UncResCEO and UncPreCEO main effects are null at the LowHN baseline. The interaction terms therefore measure the EMERGENCE of the speech-uncertainty cash response in high-HedgingNeeds firms, not the strengthening of an existing baseline effect. This mirrors ACW (2007)'s constrained-vs-unconstrained asymmetry: the cash response is absent in low-HedgingNeeds firms and activates in high-HedgingNeeds firms.",
    ]

    with open(out_dir / "report_step4_H1_4_hedging_needs_moderation.md", "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    print("  Saved: report_step4_H1_4_hedging_needs_moderation.md")
...
def _write_suite_spec_json(
    ...
    for interaction_col in display_cols:
        ...
        # Int model: Unrated (none), INT_UNCRES_HIGHHN (pos), INT_UNCPRE_HIGHHN (pos)
        int_vars = [MOD_HIGH_HN] + ALL_INTERACTIONS
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    log_dir = setup_run_logging(
        ...
        suite_name="H1_4_HedgingNeeds_Moderation",
        timestamp=timestamp,
    )

    print("=" * 80)
    print("STAGE 4: H1.4 HedgingNeeds-Moderated CEO Speech Uncertainty and Cash Holdings (ACW 2007)")
    print("=" * 80)
    print(f"Timestamp:  {timestamp}")
    print(f"Output:     {out_dir}")
    print(f"Design:     3 decomp IVs x 2 DVs x (base + interaction) x 4 FE types = 16 models (8 displayed)")
    print(f"Channel:    CH-HedgingNeeds - ACW (2007) precautionary HedgingNeeds amplification")
    print(f"Moderator:  Binary HighHN vs LowHN (ff12-year median split)")
    print(f"IVs:        {', '.join(IVS_REG_A_RAW)}  (DWZ Eq.5 Full method)")
    print(f"Tails:      Clarity NEG / UncRes POS / UncRes x HighHN POS  (Clarity x HighHN DROPPED as ambiguous)")
    print(f"Sample:     {YEAR_MIN}-{YEAR_MAX}")

    # Load panel + merge decomp parquet
    panel, panel_file = load_panel(root, panel_path)

    # Merge HedgingNeeds (ACW 2007 16q CV) + binarize HighHN on ff12-year median
    panel = load_and_merge_hedging_needs(panel, root, years=range(YEAR_MIN, YEAR_MAX + 1))
    ...
    print(f"  Binary (Main): {n_low:,} LowHN ({100*n_low/main_n:.1f}%) / "
          f"{n_high:,} HighHN ({100*n_high/main_n:.1f}%)")
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics --- H1.4 HedgingNeeds-Moderated Cash Holdings (Main Sample, 2002--2018)",
        label="tab:summary_stats_h1_4_hedging_needs",
    )
...
if __name__ == "__main__":
    ...
    if args.dry_run:
        ...
        print(f"  Moderator: {MOD_HIGH_HN} (binary)")
