...
def run_regression(
    ...
    print(f"  After >={min_calls} calls filter: {len(df_reg):,} calls, {df_reg['ceo_id'].nunique():,} managers")
...
def extract_clarity_scores(
    ...
    print(f"  ClarityManager: mean={manager_fe['ClarityManager'].mean():.4f}, std={manager_fe['ClarityManager'].std():.4f}")
...
def generate_report(
    ...
    for sample_name, result in results.items():
        ...
        report_lines.append(
            f"| {sample_name} | {n_obs:,} | {n_mgr:,} | {r2:.4f} |"
            if isinstance(r2, float) else
            f"| {sample_name} | {n_obs} | {n_mgr} | {r2} |"
        )
    ...
    if all_clarity_scores:
        ...
        for sample in ["Main", "Finance", "Utility"]:
            ...
            for i, row in sample_df.nlargest(5, "ClarityManager").iterrows():
                report_lines.append(
                    f"| {sample_df.index.get_loc(i) + 1} | {row['ceo_id']} | {row['ClarityManager']:.3f} |"
                )
            ...
            for i, row in sample_df.nsmallest(5, "ClarityManager").iterrows():
                report_lines.append(
                    f"| {sample_df.index.get_loc(i) + 1} | {row['ceo_id']} | {row['ClarityManager']:.3f} |"
                )
