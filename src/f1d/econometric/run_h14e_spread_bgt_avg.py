...
"""
================================================================================
STAGE 4: Test H14e BGTAvg_Spread (12-col 2-DV)
================================================================================
ID: econometric/run_h14e_spread_bgt_avg
...
Model Specifications (12 columns):
    Cols 1-6:  DV = BGTAvg_Spread (contemporaneous)
    Cols 7-12: DV = BGTAvg_Spread_lead1 (next-quarter lead)

DV: BGTAvg_Spread = mean(RelSpread[+1,+3]) - mean(RelSpread[-3,-1])
...
# F1D extension: mean closing-quote spread over symmetric [-25, +25] trading days (51-day window), day 0 INCLUDED. Window length is BGT-verbatim, symmetric shape is F1D-pipeline convention.
# ----------------------------------------------------------------------
...
# ------------------------------------------------------------------
SUITE_ID = "H14e"
SUITE_DIR_NAME = "h14e_spread_bgt_avg"
SUITE_TITLE = 'H14e: Speech Uncertainty and BGT-Window 25-Day Bid-Ask Spread Average (, 51-day symmetric)'
SUITE_CAPTION = r'H14e: Speech Uncertainty and BGT-Window 25-Day Bid-Ask Spread Average ($[-25,+25]$, 51-day symmetric)'
SUITE_LABEL = "tab:h14e"
...
SUMMARY_STATS_VARS = [
    {"col": "BGTAvg_Spread", "label": r"$\Delta$Spread$_t$"},
    {"col": "BGTAvg_Spread_lead1", "label": r"$\Delta$Spread$_{t+1}$"},
...
def load_panel(root_path: Path, panel_path: Optional[str] = None) -> pd.DataFrame:
    ...
    # Bug 4 fix (Phase 7): avg spread ~1e-3 with coefs ~1e-5. Rescale by 10^4.
...
def _save_latex_table(all_results, out_dir):
    ...
    lines = [
        ...
        r"\caption{H14e: Speech Uncertainty and BGT-Window 25-Day Closing-Quote Bid-Ask Spread AVERAGE (contemp + $t+1$ lead) [F1D extension]}",
        r"\label{tab:h14e_bgt_avg_spread}", r"\scriptsize",
    ...
    lines += [
        ...
        r"BGTAvg_Spread$_t$ = BGT (2018) 25-day window applied to Lee (2016) closing-quote spread formula. ",
        r"BGTAvg_Spread$_{t+1}$ = next-quarter call's BGTAvg_Spread (calendar quarter, strict consecutive). ",
        r"Lagged\_DV control = true t-1 prior-quarter lag. ",
        r"Unit of observation: individual earnings call.",
        r"\end{minipage}", r"\end{table}",
    ]
    with open(out_dir / "h14e_spread_bgt_avg_table.tex", "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
...
def _write_suite_spec_json(
    ...
    """Emit canonical suite_spec_H14e.json from runner state."""
    ...
    header_rows = [
        [
            {"label": r"BGTAvg\_Spread", "span": 6},
            {"label": r"BGTAvg\_Spread\_lead1", "span": 6},
        ]
    ]
    paths = write_suite_spec(
        ...
        render_hints={
            "scaling_note": (
                r"BGTAvg\_Spread values rescaled by $10^4$ for readability; "
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h14e_spread_bgt_avg" / timestamp
    log_dir = setup_run_logging(log_base_dir=root / "logs", suite_name="H14e_BGTAvg_Spread", timestamp=timestamp)

    print("=" * 80)
    print("STAGE 4: Test H14e BGTAvg_Spread (12-col 2-DV)")
    print("=" * 80)
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics -- H14e BGTAvg_Spread (Main Sample)",
        label="tab:summary_stats_h14e",
    )
    ...
    if all_results:
        ...
        ], out_dir, "H14e BGTAvg_Spread")

    generate_manifest(
        output_dir=out_dir, stage="stage4", timestamp=timestamp,
        input_paths={"panel": panel_file},
        output_files={"diagnostics": out_dir / "model_diagnostics.csv",
                      "table": out_dir / "h14e_spread_bgt_avg_table.tex"},
        panel_path=panel_file,
    )

    duration = (datetime.now() - start_time).total_seconds()
    with open(out_dir / "report_step4_H14e.md", "w", encoding="utf-8") as f:
        f.write(f"# H14e Report (12-col 2-DV)\n\n**Duration:** {duration:.1f}s\n")
