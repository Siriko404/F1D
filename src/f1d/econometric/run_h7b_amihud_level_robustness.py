...
# ------------------------------------------------------------------
SUITE_ID = "H7b.r"
SUITE_DIR_NAME = "h7b_amihud_level_robustness"
SUITE_TITLE = "Speech Uncertainty and 3-Day Post-Call Amihud Illiquidity Level ([+1,+3])"
SUITE_CAPTION = (
    r"H7b: Speech Uncertainty and 3-Day Post-Call Amihud Illiquidity Level ($[+1,+3]$)"
)
SUITE_LABEL = "tab:h7b_robust"
...
def _save_latex_table(all_results, out_dir):
    ...
    lines = [
        ...
        r"\label{tab:h7b_robust}", r"\scriptsize",
    ...
    with open(out_dir / "h7b_amihud_level_robustness_table.tex", "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    print(f"  Saved: h7b_amihud_level_robustness_table.tex")
...
def _write_suite_spec_json(
    ...
    """Emit canonical suite_spec_H7b.json from runner state."""
    ...
    header_rows = [
        [
            {"label": "PostCallAmihud", "span": 6},
            {"label": r"PostCallAmihud\_lead1", "span": 6},
        ]
    ]
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h7b_amihud_level_robustness" / timestamp
    log_dir = setup_run_logging(log_base_dir=root / "logs", suite_name="H7b_Amihud_Level_robust", timestamp=timestamp)
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics -- H7b Amihud Level Robustness (Main Sample)",
        label="tab:summary_stats_h7b_robust",
    )
