...
"""
================================================================================
STAGE 4: Test H24b Global Economic Policy Uncertainty -> Call Language Uncertainty
================================================================================
ID: econometric/test_h24b_global_epu
Description: Reverse-direction suite - aggregate Global Economic Policy Uncertainty
             (Davis 2016, NBER WP 22740; extends Baker, Bloom & Davis 2016 to a
             GDP-weighted average of 16 country EPU indices) predicts call-level
             language uncertainty. Analogous structure to H11 (PRisk -> Uncertainty).

Model Specification:
    Uncertainty_it ~ log(GEPU)_mt + PresControl_it + Lagged_DV_i,t-1 +
...
Hypothesis (one-tailed, positive):
    H24b: beta(log(GEPU)) > 0
          Higher aggregate global economic policy uncertainty -> higher call-level
...
Outputs:
    - outputs/econometric/h24b_global_epu/{timestamp}/regression_results_{sample}_{dv}.txt
    - outputs/econometric/h24b_global_epu/{timestamp}/h24b_global_epu_table.tex
    - outputs/econometric/h24b_global_epu/{timestamp}/model_diagnostics.csv
    - outputs/econometric/h24b_global_epu/{timestamp}/summary_stats.csv / .tex
    - outputs/econometric/h24b_global_epu/{timestamp}/sample_attrition.csv / .tex
    - outputs/econometric/h24b_global_epu/{timestamp}/run_manifest.json
...
Reference: Davis, Steven J. (2016) "An Index of Global Economic Policy Uncertainty,"
           NBER Working Paper No. 22740. Builds on Baker, Bloom & Davis (2016, QJE).
...
MACRO_IV = "GEPU_log"
MACRO_IV_RAW = "GEPU_current"
MACRO_IV_LABEL = r"$\log(\text{GEPU})_{t}$"

SUITE_ID = "H24b"
SUITE_NAME = "H24b_Global_EPU"
SUITE_DIR = "h24b_global_epu"
SUITE_CAPTION = (
    "H24b: Global Economic Policy Uncertainty and Call Language Uncertainty"
)
SUITE_LABEL = "tab:h24b_global_epu"
...
def _save_latex_table(all_results: List[Dict[str, Any]], out_dir: Path) -> None:
    ...
    lines.extend(
        [
            ...
            r"columns 5--8). ",
            r"Key regressor is $\log(\text{GEPU})$, the log of the "
            r"Davis (2016, NBER WP 22740) GDP-weighted Global Economic Policy "
            r"Uncertainty index (extends the BBD 2016 framework to 16 countries), "
            r"matched to each call by its calendar month. ",
...
def main(panel_path: str | None = None) -> int:
    ...
    make_summary_stats_table(
        ...
        caption=f"Summary Statistics - {SUITE_ID} Global EPU",
    ...
    if all_results:
        ...
        generate_attrition_table(
            attrition_stages, out_dir, f"{SUITE_ID} Global EPU"
        )
