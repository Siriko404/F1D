...
Model Specifications (12 columns):
    Cols 1-6:  DV = DSPREAD (contemporaneous)
    Cols 7-12: DV = DSPREAD_lead1 (next-quarter lead)
...
# ------------------------------------------------------------------
SUITE_ID = "H14.r"
SUITE_DIR_NAME = "h14_bidask_spread_robustness"
SUITE_TITLE = "Speech Uncertainty and Lee (2016) 3-Day Bid-Ask Spread Change (Orthogonal Speaker Partition)"
SUITE_CAPTION = (
    r"H14 Robustness: Orthogonal Speaker Partition"
)
SUITE_LABEL = "tab:h14_robust"
...
MODEL_SPECS = [
    ...
    {"col": 7,  "dv": "DSPREAD_lead1", "fe": "industry",    "controls": "base"},
    {"col": 8,  "dv": "DSPREAD_lead1", "fe": "firm",        "controls": "base"},
    {"col": 9,  "dv": "DSPREAD_lead1", "fe": "industry",    "controls": "extended"},
    {"col": 10, "dv": "DSPREAD_lead1", "fe": "firm",        "controls": "extended"},
    {"col": 11, "dv": "DSPREAD_lead1", "fe": "industry_yq", "controls": "extended"},
    {"col": 12, "dv": "DSPREAD_lead1", "fe": "firm_yq",     "controls": "extended"},
]
...
SUMMARY_STATS_VARS = [
    {"col": "DSPREAD", "label": r"$\Delta$Spread$_t$"},
    {"col": "DSPREAD_lead1", "label": r"$\Delta$Spread$_{t+1}$"},
...
def load_panel(root_path: Path, panel_path: Optional[str] = None) -> pd.DataFrame:
    ...
    columns = [
        ...
        "DSPREAD", "DSPREAD_lag", "DSPREAD_lead1",
        "PreCallSpread",
        "UncAnsCEO", "UncPreCEO",
...
def _save_latex_table(all_results, out_dir):
    ...
    lines = [
        ...
        r"\caption{Speech Uncertainty and Bid-Ask Spread Changes (Orthogonal Speaker Partition)}",
        r"\label{tab:h14_robust}", r"\scriptsize",
    ...
    with open(out_dir / "h14_bidask_spread_robustness_table.tex", "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    print(f"  Saved: h14_bidask_spread_robustness_table.tex")
...
def _write_suite_spec_json(
    ...
    """Emit canonical suite_spec_H14.json from runner state."""
    ...
    header_rows = [
        [
            {"label": "DSPREAD", "span": 6},
            {"label": r"DSPREAD\_lead1", "span": 6},
        ]
    ]
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h14_bidask_spread_robustness" / timestamp
    log_dir = setup_run_logging(log_base_dir=root / "logs", suite_name="H14_BidAskSpread_robust", timestamp=timestamp)

    print("=" * 80)
    print("STAGE 4: Test H14 Bid-Ask Spread Hypothesis (12-col 2-DV)")
    print("=" * 80)
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics -- H14 Bid-Ask Spread Robustness (Main Sample)",
        label="tab:summary_stats_h14_robust",
    )
    ...
    if all_results:
        ...
        generate_attrition_table([
            ...
            ("DSPREAD_lead1 non-null", panel["DSPREAD_lead1"].notna().sum()),
            ("Complete-case + min-calls (col 1)", first_meta.get("n_obs", 0)),
        ], out_dir, "H14 Bid-Ask Spread Robustness")

    generate_manifest(
        output_dir=out_dir, stage="stage4", timestamp=timestamp,
        input_paths={"panel": panel_file},
        output_files={"diagnostics": out_dir / "model_diagnostics.csv",
                      "table": out_dir / "h14_bidask_spread_robustness_table.tex"},
        panel_path=panel_file,
    )

    duration = (datetime.now() - start_time).total_seconds()
    with open(out_dir / "report_step4_H14.md", "w", encoding="utf-8") as f:
        f.write(f"# H14 Bid-Ask Spread Report (12-col 2-DV)\n\n**Duration:** {duration:.1f}s\n")
