...
"""
================================================================================
STAGE 4: Test H20 Speech Uncertainty and Debt vs Equity Choice Hypothesis
================================================================================
ID: econometric/run_h20_debt_choice
Description: Run H20 hypothesis test — does speech uncertainty predict the choice
             of debt vs equity financing? (Leary & Roberts 2010, JFE).
             Conditional on external financing.
             DebtChoice = 1 if debt-only, 0 if equity or dual.

6 Model Specifications:
    Cols 1-2:   DV = DebtChoice, Calendar Year FE (Base / Extended)
    Cols 3-4:   DV = DebtChoice, Calendar Year FE (Extended)
    Cols 5-6:   DV = DebtChoice, Year-Quarter FE (Extended)
...
Outputs:
    - outputs/econometric/h20_debt_choice/{timestamp}/...
...
MODEL_SPECS = [
    # DebtChoice — Calendar Year FE
    {"col": 1, "dv": "DebtChoice", "fe": "industry",    "controls": "base",     "extra_controls": []},
    {"col": 2, "dv": "DebtChoice", "fe": "firm",        "controls": "base",     "extra_controls": []},
    {"col": 3, "dv": "DebtChoice", "fe": "industry",    "controls": "extended", "extra_controls": []},
    {"col": 4, "dv": "DebtChoice", "fe": "firm",        "controls": "extended", "extra_controls": []},
    # DebtChoice — Year-Quarter FE (Extended controls only)
    {"col": 5, "dv": "DebtChoice", "fe": "industry_yq", "controls": "extended", "extra_controls": []},
    {"col": 6, "dv": "DebtChoice", "fe": "firm_yq",     "controls": "extended", "extra_controls": []},
]
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Stage 4: H20 Debt Choice (call-level)",
    )
...
def load_panel(root_path: Path, panel_path: Optional[str] = None) -> Tuple[pd.DataFrame, Path]:
    """Load call-level H19/H20 financing panel."""
    print("\n" + "=" * 60)
    print("Loading H19/H20 financing panel")
    print("=" * 60)
...
def restrict_to_external_funders(panel: pd.DataFrame) -> pd.DataFrame:
    """Restrict to external funders (ExternalFunding=1) only."""
    before = len(panel)
    panel = panel[panel["ExternalFunding"] == 1].copy()
    print(f"  External funders (ExternalFunding=1): {len(panel):,} / {before:,} "
...
def prepare_regression_data(
    ...
    # Create Lagged_DV: ExternalFunding_lag (NOT DebtChoice_lag — avoids 50% sample loss)
    panel = panel.copy()
...
def run_regression(
    ...
    for iv in KEY_IVS:
        ...
        # Two-tailed: H20 direction is theoretically ambiguous
...
def _save_latex_table(all_results: List[Dict[str, Any]], out_dir: Path) -> None:
    ...
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{Speech Uncertainty and Debt vs Equity Choice}",
        r"\label{tab:h20_debt_choice}",
        r"\scriptsize",
    ...
    tex_path = out_dir / "h20_debt_choice_table.tex"
...
def save_outputs(all_results: List[Dict[str, Any]], out_dir: Path) -> pd.DataFrame:
    ...
    for r in all_results:
        ...
        with open(out_dir / fname, "w", encoding="utf-8") as f:
            f.write(f"H20 Debt vs Equity Choice Regression\n")
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h20_debt_choice" / timestamp

    log_dir = setup_run_logging(
        log_base_dir=root / "logs",
        suite_name="H20_DebtChoice",
        timestamp=timestamp,
    )

    print("=" * 80)
    print("STAGE 4: H20 Debt vs Equity Choice")
    print("=" * 80)
    ...
    n_dv_valid = panel["DebtChoice"].notna().sum()
    n_dv1 = (panel["DebtChoice"] == 1).sum()
    n_dv0 = (panel["DebtChoice"] == 0).sum()
    print(f"\n  After ExternalFunding=1 restriction: {ext_n:,} calls, {panel['gvkey'].nunique():,} firms")
    print(f"  DebtChoice non-null: {n_dv_valid:,}")
    print(f"  DebtChoice=1 (debt-only): {n_dv1:,} ({100*n_dv1/n_dv_valid:.2f}%)")
    print(f"  DebtChoice=0 (equity/dual): {n_dv0:,} ({100*n_dv0/n_dv_valid:.2f}%)")
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics --- H20 Debt Choice (External Funders, Main Sample)",
        label="tab:summary_stats_h20",
    )
    ...
    if all_results:
        ...
        attrition_stages = [
            ...
            ("External funders (ExternalFunding=1)", ext_n),
            ("DebtChoice=1 (debt-only) in sample", n_dv1),
            ("After complete-case + min-calls (col 1)", first["n_obs"]),
        ]
        generate_attrition_table(attrition_stages, out_dir, "H20 Debt Choice")
