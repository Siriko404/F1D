...
# ------------------------------------------------------------------
SUITE_ID = "H7d.r"
SUITE_DIR_NAME = "h7d_amihud_bgt_delta_robustness"
SUITE_TITLE = 'H7d: Speech Uncertainty and Illiquidity (Orthogonal Speaker Partition)'
SUITE_CAPTION = r'H7d Robustness: Orthogonal Speaker Partition'
SUITE_LABEL = "tab:h7d_robust"
...
def _save_latex_table(all_results, out_dir):
    ...
    lines = [
        ...
        r"\caption{H7d Robustness: Orthogonal Speaker Partition}",
        r"\label{tab:h7d_robust}", r"\scriptsize",
    ...
    with open(out_dir / "h7d_amihud_bgt_delta_robustness_table.tex", "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h7d_amihud_bgt_delta_robustness" / timestamp
    log_dir = setup_run_logging(log_base_dir=root / "logs", suite_name="H7d_BGTDelta_Amihud_robust", timestamp=timestamp)
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics -- H7d BGTDelta_Amihud Robustness (Main Sample)",
        label="tab:summary_stats_h7d_robust",
    )
    ...
    generate_manifest(
        ...
        output_files={"diagnostics": out_dir / "model_diagnostics.csv",
                      "table": out_dir / "h7d_amihud_bgt_delta_robustness_table.tex"},
