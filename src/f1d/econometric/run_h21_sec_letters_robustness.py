...
# ------------------------------------------------------------------
SUITE_ID = "H21.r"
SUITE_DIR_NAME = "h21_sec_letters_robustness"
SUITE_TITLE = "Speech Uncertainty and SEC Comment Letter Count (Orthogonal Speaker Partition)"
SUITE_CAPTION = "H21 Robustness: Orthogonal Speaker Partition"
SUITE_LABEL = "tab:h21_robust"
SAMPLE_LABEL = "Main sample (excludes financial and utility firms)."
HYP_DIR = "positive"  # H21: beta(uncertainty) > 0 — more uncertainty -> more letters
...
def _save_latex_table(all_results: List[Dict[str, Any]], out_dir: Path) -> None:
    ...
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{Speech Uncertainty and SEC Comment Letter Count (Orthogonal Speaker Partition)}",
        r"\label{tab:h21_robust}",
        r"\scriptsize",
    ...
    tex_path = out_dir / "h21_sec_letters_robustness_table.tex"
...
def _write_suite_spec_json(
    ...
    """Emit canonical suite_spec_H21.json from runner state."""
    ...
    header_rows = [[{"label": r"SEC\_Letters\_fwd", "span": 6}]]
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h21_sec_letters_robustness" / timestamp

    log_dir = setup_run_logging(
        log_base_dir=root / "logs",
        suite_name="H21_SEC_Letters_robust",
        timestamp=timestamp,
    )
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics --- H21 SEC Letters Count Robustness (Main Sample)",
        label="tab:summary_stats_h21_robust",
    )
    ...
    if all_results:
        ...
        generate_attrition_table(attrition_stages, out_dir, "H21 SEC Letters Count Robustness")
