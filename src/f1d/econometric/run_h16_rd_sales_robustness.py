...
# ------------------------------------------------------------------
SUITE_ID = "H16.r"
SUITE_DIR_NAME = "h16_rd_sales_robustness"
SUITE_TITLE = "Speech Uncertainty and R&D Investment Intensity (Orthogonal Speaker Partition)"
SUITE_CAPTION = r"H16 Robustness: Orthogonal Speaker Partition"
SUITE_LABEL = "tab:h16_robust"
SAMPLE_LABEL = "Main sample (excludes financial and utility firms)."
HYP_DIR = "none"  # H16: two-tailed (no signed prediction)
...
def _save_latex_table(all_results: List[Dict[str, Any]], out_dir: Path) -> None:
    ...
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{Speech Uncertainty and R\&D Investment Intensity (Orthogonal Speaker Partition)}",
        r"\label{tab:h16_robust}",
        r"\scriptsize",
    ...
    tex_path = out_dir / "h16_rd_sales_robustness_table.tex"
    with open(tex_path, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    print(f"  Saved: h16_rd_sales_robustness_table.tex")
...
def _write_suite_spec_json(
    ...
    """Emit canonical suite_spec_H16.json from runner state."""
    ...
    header_rows = [
        [
            {"label": "RDSales", "span": 6},
            {"label": r"RDSales\_lead", "span": 6},
        ]
    ]
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h16_rd_sales_robustness" / timestamp

    # Setup logging
    log_dir = setup_run_logging(
        log_base_dir=root / "logs",
        suite_name="H16_RDSales_robust",
        timestamp=timestamp,
    )
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics -- H16 R\\&D Investment Intensity Robustness (Main Sample)",
        label="tab:summary_stats_h16_robust",
    )
    ...
    if all_results:
        ...
        generate_attrition_table(attrition_stages, out_dir, "H16 R&D Investment Intensity Robustness")
