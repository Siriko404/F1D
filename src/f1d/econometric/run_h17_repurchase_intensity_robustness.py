...
# ------------------------------------------------------------------
SUITE_ID = "H17.r"
SUITE_DIR_NAME = "h17_repurchase_intensity_robustness"
SUITE_TITLE = "Speech Uncertainty and Repurchase Intensity (Orthogonal Speaker Partition)"
SUITE_CAPTION = "H17 Robustness: Orthogonal Speaker Partition"
SUITE_LABEL = "tab:h17_robust"
SAMPLE_LABEL = "Main sample (excludes financial and utility firms)."
HYP_DIR = "none"  # H17: two-tailed (no signed prediction)
...
def _save_latex_table(all_results: List[Dict[str, Any]], out_dir: Path) -> None:
    ...
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{Speech Uncertainty and Repurchase Intensity (Orthogonal Speaker Partition)}",
        r"\label{tab:h17_robust}",
        r"\scriptsize",
    ...
    tex_path = out_dir / "h17_repurchase_intensity_robustness_table.tex"
...
def _write_suite_spec_json(
    ...
    """Emit canonical suite_spec_H17.json from runner state."""
    ...
    header_rows = [
        [
            {"label": "RepurchaseIntensity", "span": 6},
            {"label": r"RepurchaseIntensity\_lead\_qtr", "span": 6},
        ]
    ]
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h17_repurchase_intensity_robustness" / timestamp

    log_dir = setup_run_logging(
        log_base_dir=root / "logs",
        suite_name="H17_RepurchaseIntensity_robust",
        timestamp=timestamp,
    )
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics --- H17 Repurchase Intensity Robustness (Main Sample)",
        label="tab:summary_stats_h17_robust",
    )
    ...
    if all_results:
        ...
        generate_attrition_table(attrition_stages, out_dir, "H17 Repurchase Intensity Robustness")
