...
# ------------------------------------------------------------------
SUITE_ID = "H22.r"
SUITE_DIR_NAME = "h22_equity_constraints_robustness"
SUITE_TITLE = "Speech Uncertainty and Equity Financing Constraints (Orthogonal Speaker Partition)"
SUITE_CAPTION = "H22 Robustness: Orthogonal Speaker Partition"
SUITE_LABEL = "tab:h22_robust"
SAMPLE_LABEL = "Main sample (excludes financial and utility firms)."
HYP_DIR = "positive"  # H22: beta(uncertainty) > 0 — more uncertainty -> tighter equity constraints
...
def _save_latex_table(all_results: List[Dict[str, Any]], out_dir: Path) -> None:
    ...
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{Speech Uncertainty and Equity Financing Constraints (Orthogonal Speaker Partition)}",
        r"\label{tab:h22_robust}",
        r"\scriptsize",
    ...
    tex_path = out_dir / "h22_equity_constraints_robustness_table.tex"
...
def _write_suite_spec_json(
    ...
    """Emit canonical suite_spec_H22.json from runner state."""
    ...
    header_rows = [[{"label": r"EquityDelayCon\_lead", "span": 4}]]
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h22_equity_constraints_robustness" / timestamp

    log_dir = setup_run_logging(
        log_base_dir=root / "logs",
        suite_name="H22_EquityConstraints_robust",
        timestamp=timestamp,
    )
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics --- H22 Equity Constraints Robustness (Main Sample)",
        label="tab:summary_stats_h22_robust",
    )
    ...
    if all_results:
        ...
        generate_attrition_table(attrition_stages, out_dir, "H22 Equity Constraints Robustness")
