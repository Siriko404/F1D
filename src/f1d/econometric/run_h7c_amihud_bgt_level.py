...
def _save_latex_table(all_results, out_dir):
    ...
    lines = [
        ...
        r"\caption{H7c: Speech Uncertainty and BGT (2018) 25-Day Post-Call Amihud Illiquidity LEVEL (contemp + $t+1$ lead)}",
        r"\label{tab:h7c_bgt_level_amihud}", r"\scriptsize",
    ...
    with open(out_dir / "h7c_amihud_bgt_level_table.tex", "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h7c_amihud_bgt_level" / timestamp
    log_dir = setup_run_logging(log_base_dir=root / "logs", suite_name="H7c_BGTLevel_Amihud", timestamp=timestamp)
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics -- H7c BGTLevel_Amihud (Main Sample)",
        label="tab:summary_stats_h7c",
    )
    ...
    generate_manifest(
        ...
        output_files={"diagnostics": out_dir / "model_diagnostics.csv",
                      "table": out_dir / "h7c_amihud_bgt_level_table.tex"},
