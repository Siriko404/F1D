...
"""
================================================================================
STAGE 4: Test H14e — CEO 3-IV DWZ DECOMP (12-col 2-DV)
================================================================================
ID: econometric/run_h14e_spread_bgt_avg_ceo2iv_decomp
Description: DWZ-decomposition variant of H14e reaction suite. Replaces the
             4-IV stack (UncAnsCEO + UncPreCEO + UncAnsMgr + UncPreMgr) with
             the DWZ Eq.5 3-IV CEO-only decomposition: ClarityCEO (persistent
             CEO trait, = -CEO FE) + UncResCEO (call-level state residual)
             + UncPreCEO (raw, presentation-segment uncertainty). Full method:
             single full-panel DWZ Eq.4 fit.

             Per locked design (drivers + reaction = Full method only),
             companion QtrExp variant NOT built for §4.2 reaction suite.

             H14e DV is the symmetric 51-day average spread around the call
             event (mean(RelSpread[-25,+25])), complementing H14c's
             post-call-only level (mean(RelSpread[0,+25])). Together they form
             a level/window-shape robustness pair on the same Pres-channel
             reaction signal.
...
Tail directions (asymmetric per BGT 2018 + DWZ decomposition logic):
    ClarityCEO: one-tail NEG
        (high persistent CEO clarity → less market-perceived information
         asymmetry → narrower spreads around the calls around the call)
    UncResCEO: one-tail POS
    UncPreCEO: one-tail POS
        (positive uncertainty surprise → more information asymmetry → wider
         spreads around the call; BGT 2018 mechanism)

Model Specifications (12 columns):
    Cols 1-6:  DV = BGTAvg_Spread          (contemporaneous; symmetric 51-day avg)
    Cols 7-12: DV = BGTAvg_Spread_lead1    (next-quarter lead)

DV: BGTAvg_Spread = mean(RelSpread[-25,+25]) over the 51-day symmetric window
    centered on the call day (call day INCLUDED). Daily relative spread:
    RelSpread_d = 2*(ASK_d - BID_d)/(ASK_d + BID_d) from CRSP closing quotes;
    pooled-winsorized at 1%/99%; rescaled by 1e4 (bps).
...
# ----------------------------------------------------------------------
# H14e window construction:
# F1D extension: mean closing-quote spread over symmetric [-25, +25] trading
# days (51-day window), day 0 INCLUDED. Window length is BGT-verbatim;
# symmetric shape is F1D-pipeline convention. Variable named BGTAvg_Spread
# in the panel; user-facing strings refer to Spread_{Avg} or BGTAvg_Spread.
# ----------------------------------------------------------------------
...
# ------------------------------------------------------------------
SUITE_ID = "H14e.ceo2.decomp"
SUITE_DIR_NAME = "h14e_spread_bgt_avg_ceo2iv_decomp"
SUITE_TITLE = (
    'H14e CEO 3-IV Decomp: DWZ Full QA Decomposition + Raw UncPreCEO and '
    'Symmetric 51-Day Bid-Ask Spread Average (25 days pre-call to 25 days post-call)'
)
SUITE_CAPTION = (
    r'H14e CEO 3-IV Decomp: ClarityCEO + UncResCEO (DWZ Eq.5 Full) + UncPreCEO (raw) and '
    r'Symmetric 51-Day Bid-Ask Spread Average ($[-25,+25]$, 25 days pre-call to 25 days post-call)'
)
SUITE_LABEL = "tab:h14e_ceo2_decomp"
SAMPLE_LABEL = (
...
MODEL_SPECS = [
    {"col": 1,  "dv": "BGTAvg_Spread",       "fe": "industry",    "controls": "base"},
    {"col": 2,  "dv": "BGTAvg_Spread",       "fe": "firm",        "controls": "base"},
    {"col": 3,  "dv": "BGTAvg_Spread",       "fe": "industry",    "controls": "extended"},
    {"col": 4,  "dv": "BGTAvg_Spread",       "fe": "firm",        "controls": "extended"},
    {"col": 5,  "dv": "BGTAvg_Spread",       "fe": "industry_yq", "controls": "extended"},
    {"col": 6,  "dv": "BGTAvg_Spread",       "fe": "firm_yq",     "controls": "extended"},
    {"col": 7,  "dv": "BGTAvg_Spread_lead1", "fe": "industry",    "controls": "base"},
    {"col": 8,  "dv": "BGTAvg_Spread_lead1", "fe": "firm",        "controls": "base"},
    {"col": 9,  "dv": "BGTAvg_Spread_lead1", "fe": "industry",    "controls": "extended"},
    {"col": 10, "dv": "BGTAvg_Spread_lead1", "fe": "firm",        "controls": "extended"},
    {"col": 11, "dv": "BGTAvg_Spread_lead1", "fe": "industry_yq", "controls": "extended"},
    {"col": 12, "dv": "BGTAvg_Spread_lead1", "fe": "firm_yq",     "controls": "extended"},
]
...
SUMMARY_STATS_VARS = [
    {"col": "BGTAvg_Spread", "label": r"Spread$_{Avg,t}$"},
    {"col": "BGTAvg_Spread_lead1", "label": r"Spread$_{Avg,t+1}$"},
...
def load_panel(root_path: Path, panel_path: Optional[str] = None) -> pd.DataFrame:
    ...
    columns = [
        ...
        "BGTAvg_Spread", "BGTAvg_Spread_lag", "BGTAvg_Spread_lead1",
        "PreCallSpread",
        "UncPreCEO",
    ...
    print(f"  BGTAvg_Spread non-null: {panel['BGTAvg_Spread'].notna().sum():,}")
    print(f"  BGTAvg_Spread_lead1 non-null: {panel['BGTAvg_Spread_lead1'].notna().sum():,}")

    # Bug 4 fix (Phase 7): spread ~1e-3 with coefs ~1e-5. Rescale by 10^4.
    panel["BGTAvg_Spread"] = panel["BGTAvg_Spread"] * 1e4
    panel["BGTAvg_Spread_lag"] = panel["BGTAvg_Spread_lag"] * 1e4
    panel["BGTAvg_Spread_lead1"] = panel["BGTAvg_Spread_lead1"] * 1e4
...
def _save_latex_table(all_results, out_dir):
    ...
    lines.append(
        r" & \multicolumn{6}{c}{Spread$_{Avg,t}$}"
        r" & \multicolumn{6}{c}{Spread$_{Avg,t+1}$} \\"
    )
    ...
    lines += [
        ...
        r"\textit{ClarityCEO}: one-tailed NEG ($\beta < 0$; clear CEO $\Rightarrow$ narrower spreads around the call). ",
        r"\textit{UncResCEO}: one-tailed POS ($\beta > 0$; uncertainty surprise $\Rightarrow$ wider spreads; BGT 2018). ",
        r"\textit{UncPreCEO}: one-tailed POS ($\beta > 0$; pres uncertainty $\Rightarrow$ wider spreads). ",
        r"Standard errors (in parentheses) firm-level clustered. ",
        r"Main sample (excludes financial and utility firms). ",
        r"Spread$_{Avg,t}$ = mean daily closing-quote relative bid-ask spread over the symmetric 51-day window centered on the call day, $[-25,+25]$ trading days. Daily formula: $2(\text{ASK}-\text{BID})/(\text{ASK}+\text{BID})$ from CRSP closing quotes. ",
        r"Spread$_{Avg,t+1}$ = next-quarter call's Spread$_{Avg}$ (calendar quarter, strict consecutive). ",
        r"Lagged\_DV control = true t-1 prior-quarter lag. ",
        r"Unit of observation: individual earnings call.",
        r"\end{minipage}", r"\end{table}",
    ]
    with open(out_dir / "h14e_spread_bgt_avg_ceo2iv_decomp_table.tex", "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    print(f"  Saved: h14e_spread_bgt_avg_ceo2iv_decomp_table.tex")
...
def _write_suite_spec_json(
    ...
    """Emit canonical suite_spec_H14e.ceo2.decomp.json with per-IV directional p_one.
    ...
    for spec in MODEL_SPECS:
        ...
        if col_num not in results_by_col:
            raise RuntimeError(
                f"H14e.ceo2.decomp spec build: missing result for col {col_num}"
            )
    ...
    header_rows = [
        [
            {"label": r"Spread$_{Avg,t}$", "span": 6},
            {"label": r"Spread$_{Avg,t+1}$", "span": 6},
        ]
    ]
    ivs_payload = [
        {
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    log_dir = setup_run_logging(log_base_dir=root / "logs", suite_name="H14e_BGTAvg_Spread_ceo2iv_decomp", timestamp=timestamp)

    print("=" * 80)
    print("STAGE 4: Test H14e CEO 3-IV DECOMP BGTAvg_Spread (12-col 2-DV)")
    print("=" * 80)
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics -- H14e CEO 3-IV Decomp BGTAvg_Spread (Main Sample)",
        label="tab:summary_stats_h14e_ceo2_decomp",
    )
    ...
    if all_results:
        ...
        generate_attrition_table([
            ...
            ("BGTAvg_Spread non-null", panel["BGTAvg_Spread"].notna().sum()),
            ("BGTAvg_Spread_lead1 non-null", panel["BGTAvg_Spread_lead1"].notna().sum()),
            ("Complete-case + min-calls (col 1)", first_meta.get("n_obs", 0)),
        ], out_dir, "H14e CEO 3-IV Decomp BGTAvg_Spread")

    generate_manifest(
        output_dir=out_dir, stage="stage4", timestamp=timestamp,
        input_paths={"panel": panel_file},
        output_files={"diagnostics": out_dir / "model_diagnostics.csv",
                      "table": out_dir / "h14e_spread_bgt_avg_ceo2iv_decomp_table.tex"},
        panel_path=panel_file,
    )

    duration = (datetime.now() - start_time).total_seconds()
    with open(out_dir / "report_step4_H14e_ceo2iv_decomp.md", "w", encoding="utf-8") as f:
        f.write(f"# H14e CEO 3-IV Decomp Report (12-col 2-DV)\n\n**Duration:** {duration:.1f}s\n")
