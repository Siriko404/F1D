...
# ------------------------------------------------------------------
SUITE_ID = "H12.r"
SUITE_DIR_NAME = "h12_payout_robustness"
SUITE_TITLE = "Speech Uncertainty and Quarterly Payout Ratio (Orthogonal Speaker Partition)"
SUITE_CAPTION = "H12 Robustness: Orthogonal Speaker Partition"
SUITE_LABEL = "tab:h12_robust"
SAMPLE_LABEL = "Main sample (excludes financial and utility firms)."
HYP_DIR = "negative"  # H12: beta(uncertainty) < 0 — more uncertainty -> lower payout
...
def _save_latex_table(all_results: List[Dict[str, Any]], out_dir: Path) -> None:
    ...
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{Speech Uncertainty and Quarterly Payout Ratio (Orthogonal Speaker Partition)}",
        r"\label{tab:h12_robust}",
        r"\scriptsize",
    ...
    tex_path = out_dir / "h12_payout_robustness_table.tex"
...
def _write_suite_spec_json(
    ...
    """Emit canonical suite_spec_H12.json from runner state."""
    ...
    header_rows = [
        [
            {"label": r"PayoutRatio\_q", "span": 6},
            {"label": r"PayoutRatio\_q\_lead\_qtr", "span": 6},
        ]
    ]
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h12_payout_robustness" / timestamp

    log_dir = setup_run_logging(
        log_base_dir=root / "logs",
        suite_name="H12_Payout_robust",
        timestamp=timestamp,
    )
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics --- H12 Quarterly Payout Ratio Robustness (Main Sample)",
        label="tab:summary_stats_h12_robust",
    )
    ...
    if all_results:
        ...
        generate_attrition_table(attrition_stages, out_dir, "H12 Quarterly Payout Ratio Robustness")
