...
"""
================================================================================
STAGE 4: Test H14d BGTDelta_Spread (12-col 2-DV)
================================================================================
ID: econometric/run_h14d_spread_bgt_delta
...
Model Specifications (12 columns):
    Cols 1-6:  DV = BGTDelta_Spread (contemporaneous)
    Cols 7-12: DV = BGTDelta_Spread_lead1 (next-quarter lead)

DV: BGTDelta_Spread = mean(RelSpread[+1,+3]) - mean(RelSpread[-3,-1])
...
# ----------------------------------------------------------------------
# H14d window construction:
# F1D extension: mean closing-quote spread over [+1, +25] minus mean over [-25, -1] trading days, day 0 excluded. Window length is BGT-verbatim, shape is F1D-pipeline convention.
# ----------------------------------------------------------------------
...
# ------------------------------------------------------------------
SUITE_ID = "H14d"
SUITE_DIR_NAME = "h14d_spread_bgt_delta"
SUITE_TITLE = 'H14d: Speech Uncertainty and BGT-Window 25-Day Bid-Ask Spread Delta ()'
SUITE_CAPTION = r'H14d: Speech Uncertainty and BGT-Window 25-Day Bid-Ask Spread Delta ($[+1,+25]-[-25,-1]$)'
SUITE_LABEL = "tab:h14d"
...
MODEL_SPECS = [
    {"col": 1,  "dv": "BGTDelta_Spread",       "fe": "industry",    "controls": "base"},
    {"col": 2,  "dv": "BGTDelta_Spread",       "fe": "firm",        "controls": "base"},
    {"col": 3,  "dv": "BGTDelta_Spread",       "fe": "industry",    "controls": "extended"},
    {"col": 4,  "dv": "BGTDelta_Spread",       "fe": "firm",        "controls": "extended"},
    {"col": 5,  "dv": "BGTDelta_Spread",       "fe": "industry_yq", "controls": "extended"},
    {"col": 6,  "dv": "BGTDelta_Spread",       "fe": "firm_yq",     "controls": "extended"},
    {"col": 7,  "dv": "BGTDelta_Spread_lead1", "fe": "industry",    "controls": "base"},
    {"col": 8,  "dv": "BGTDelta_Spread_lead1", "fe": "firm",        "controls": "base"},
    {"col": 9,  "dv": "BGTDelta_Spread_lead1", "fe": "industry",    "controls": "extended"},
    {"col": 10, "dv": "BGTDelta_Spread_lead1", "fe": "firm",        "controls": "extended"},
    {"col": 11, "dv": "BGTDelta_Spread_lead1", "fe": "industry_yq", "controls": "extended"},
    {"col": 12, "dv": "BGTDelta_Spread_lead1", "fe": "firm_yq",     "controls": "extended"},
]
...
SUMMARY_STATS_VARS = [
    {"col": "BGTDelta_Spread", "label": r"$\Delta$Spread$_t$"},
    {"col": "BGTDelta_Spread_lead1", "label": r"$\Delta$Spread$_{t+1}$"},
...
def load_panel(root_path: Path, panel_path: Optional[str] = None) -> pd.DataFrame:
    ...
    columns = [
        ...
        "BGTDelta_Spread", "BGTDelta_Spread_lag", "BGTDelta_Spread_lead1",
        "PreCallSpread",
        "UncAnsCEO", "UncPreCEO",
        "UncAnsMgr", "UncPreMgr",
...
def run_regression(df_prepared: pd.DataFrame, spec: Dict[str, Any]) -> Tuple[Any, Dict[str, Any]]:
    ...
    # Rank-deficient two-way clustered VCV fallback: if any SE is NaN, re-fit
    # with firm-only clustering (preserves coefficients, swaps cluster structure).
    cluster_fallback = False
    if model.std_errors.isna().any():
        nan_vars = model.std_errors[model.std_errors.isna()].index.tolist()
        print(f"  WARNING: two-way clustered VCV rank-deficient; NaN SE for: {nan_vars}")
        print(f"  Falling back to firm-only clustering for col ({col_num}).")
        model = model_obj.fit(cov_type="clustered", cluster_entity=True, cluster_time=False)
...
def _save_latex_table(all_results, out_dir):
    ...
    lines = [
        ...
        r"\caption{H14d: Speech Uncertainty and BGT-Window 25-Day Closing-Quote Bid-Ask Spread DELTA (contemp + $t+1$ lead) [F1D extension]}",
        r"\label{tab:h14d_bgt_delta_spread}", r"\scriptsize",
    ...
    lines += [
        ...
        r"BGTDelta_Spread$_t$ = BGT (2018) 25-day window applied to Lee (2016) closing-quote spread formula. ",
        r"BGTDelta_Spread$_{t+1}$ = next-quarter call's BGTDelta_Spread (calendar quarter, strict consecutive). ",
        r"Lagged\_DV control = true t-1 prior-quarter lag. ",
        r"Unit of observation: individual earnings call.",
        r"\end{minipage}", r"\end{table}",
    ]
    with open(out_dir / "h14d_spread_bgt_delta_table.tex", "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
...
def save_outputs(all_results, out_dir):
    ...
    for r in all_results:
        ...
        with open(out_dir / f"regression_results_col{col_num}.txt", "w", encoding="utf-8") as f:
            ...
            f.write(f"Cluster_Fallback: {meta.get('cluster_fallback', False)}\n")
            f.write("=" * 60 + "\n\n" + str(model.summary))
...
def _write_suite_spec_json(
    ...
    """Emit canonical suite_spec_H14d.json from runner state."""
    ...
    header_rows = [
        [
            {"label": r"BGTDelta\_Spread", "span": 6},
            {"label": r"BGTDelta\_Spread\_lead1", "span": 6},
        ]
    ]
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h14d_spread_bgt_delta" / timestamp
    log_dir = setup_run_logging(log_base_dir=root / "logs", suite_name="H14d_BGTDelta_Spread", timestamp=timestamp)

    print("=" * 80)
    print("STAGE 4: Test H14d BGTDelta_Spread (12-col 2-DV)")
    print("=" * 80)
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics -- H14d BGTDelta_Spread (Main Sample)",
        label="tab:summary_stats_h14d",
    )
    ...
    if all_results:
        ...
        generate_attrition_table([
            ...
            ("BGTDelta_Spread non-null", panel["BGTDelta_Spread"].notna().sum()),
            ("BGTDelta_Spread_lead1 non-null", panel["BGTDelta_Spread_lead1"].notna().sum()),
            ("Complete-case + min-calls (col 1)", first_meta.get("n_obs", 0)),
        ], out_dir, "H14d BGTDelta_Spread")

    generate_manifest(
        output_dir=out_dir, stage="stage4", timestamp=timestamp,
        input_paths={"panel": panel_file},
        output_files={"diagnostics": out_dir / "model_diagnostics.csv",
                      "table": out_dir / "h14d_spread_bgt_delta_table.tex"},
        panel_path=panel_file,
    )

    duration = (datetime.now() - start_time).total_seconds()
    with open(out_dir / "report_step4_H14d.md", "w", encoding="utf-8") as f:
        f.write(f"# H14d Report (12-col 2-DV)\n\n**Duration:** {duration:.1f}s\n")
