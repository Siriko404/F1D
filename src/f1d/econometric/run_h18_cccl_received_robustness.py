...
# ------------------------------------------------------------------
SUITE_ID = "H18.r"
SUITE_DIR_NAME = "h18_cccl_received_robustness"
SUITE_TITLE = "Speech Uncertainty and SEC Comment Letters (Orthogonal Speaker Partition)"
SUITE_CAPTION = "H18 Robustness: Orthogonal Speaker Partition"
SUITE_LABEL = "tab:h18_robust"
SAMPLE_LABEL = "Main sample (excludes financial and utility firms)."
HYP_DIR = "positive"  # H18: beta(uncertainty) > 0 — more uncertainty -> more letters
...
def _save_latex_table(all_results: List[Dict[str, Any]], out_dir: Path) -> None:
    ...
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{Speech Uncertainty and SEC Comment Letters (Orthogonal Speaker Partition)}",
        r"\label{tab:h18_robust}",
        r"\scriptsize",
    ...
    tex_path = out_dir / "h18_cccl_received_robustness_table.tex"
...
def _write_suite_spec_json(
    ...
    """Emit canonical suite_spec_H18.json from runner state."""
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h18_cccl_received_robustness" / timestamp

    log_dir = setup_run_logging(
        log_base_dir=root / "logs",
        suite_name="H18_CCCL_Received_robust",
        timestamp=timestamp,
    )
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics --- H18 CCCL Received Robustness (Main Sample)",
        label="tab:summary_stats_h18_robust",
    )
    ...
    if all_results:
        ...
        generate_attrition_table(attrition_stages, out_dir, "H18 CCCL Received Robustness")
