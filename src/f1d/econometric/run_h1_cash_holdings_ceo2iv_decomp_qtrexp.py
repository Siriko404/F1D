...
"""
================================================================================
STAGE 4: H1 Cash Holdings — CEO 3-IV Decomp (QtrExp / no-look-ahead)
================================================================================
ID: econometric/run_h1_cash_holdings_ceo2iv_decomp_qtrexp
Description: DWZ Eq.5 decomposition variant of H1, no-look-ahead expanding-
             window estimator. Companion to run_h1_cash_holdings_ceo2iv_decomp.py
             (Full method). Each method runs separately on its own natural sample
             and reports its own table.

             Quarterly-expanding (QtrExp) design: at each quarter t, DWZ Eq.4
             is fit on data up to quarter t-1 only, and ClarityCEO_QtrExp +
             UncResCEO_QtrExp are residualized for quarter t obs. Eliminates
             forward-looking contamination at the cost of dropping early-panel
             observations (2002 + Q1s) where insufficient prior data exists.

             Dual-stack 5-IV precursor preserved in commits 484eeda + ae13357.
...
Key Independent Variables (3, all enter simultaneously):
    ClarityCEO_QtrExp  (DWZ negated CEO FE, expanding; tail β<0)
    UncResCEO_QtrExp   (DWZ residual, expanding; tail β>0)
...
Sample: Main only (FF12 codes 1-7, 9-10, 12). Natural QtrExp sample (drops
        early-panel obs where expanding-window residual undefined).

Hypothesis Test (one-tailed, per-IV):
    ClarityCEO_QtrExp  β < 0
    UncResCEO_QtrExp   β > 0
    UncPreCEO          β > 0
...
Outputs:
    - outputs/econometric/h1_cash_holdings_ceo2iv_decomp_qtrexp/{timestamp}/...
...
# QtrExp 3-IV decomp design (one regression per cell on natural QtrExp sample).
#   Cash ~ ClarityCEO_QtrExp + UncResCEO_QtrExp + UncPreCEO + controls + FE
# Companion to Full-method runner (run_h1_cash_holdings_ceo2iv_decomp.py).
KEY_IVS = ["ClarityCEO_QtrExp", "UncResCEO_QtrExp", "UncPreCEO"]
DISPLAY_IVS = KEY_IVS  # back-compat alias
...
# ------------------------------------------------------------------
SUITE_ID = "H1.ceo2.decomp.qtrexp"
SUITE_DIR_NAME = "h1_cash_holdings_ceo2iv_decomp_qtrexp"
SUITE_TITLE = "Speech Uncertainty and Cash Holdings (CEO 3-IV Decomp: DWZ Quarterly-Expanding + Pres)"
SUITE_CAPTION = "H1 CEO 3-IV Decomp: ClarityCEO + UncResCEO (DWZ Qtr-Exp, no-look-ahead) + UncPreCEO"
SUITE_LABEL = "tab:h1_ceo2_decomp_qtrexp"
...
# Per-IV one-tail direction (same theory as Full method, applied to QtrExp variants).
...
# Summary statistics variable list — 3-IV QtrExp decomp.
# Note: under expanding-window estimation, ClarityCEO_QtrExp varies by quarter
# (not constant within CEO like Full method), since the FE is re-estimated each q.
SUMMARY_STATS_VARS = [
...
def load_panel(root_path: Path, panel_path: Optional[str] = None) -> pd.DataFrame:
    """Load call-level H1 panel + merge DWZ QtrExp decomp parquet.

    QtrExp 3-IV design:
      - UncPreCEO: H1 panel (raw).
      - ClarityCEO_QtrExp + UncResCEO_QtrExp (DWZ expanding window): from
        ceo_clarity_expanding/{latest}/ceo_clarity_qtrexp_residuals.parquet,
        both joined on file_name (call-level; FE re-estimated each quarter).
    """
    print("\n" + "=" * 60)
    print("Loading H1 panel + merging DWZ QtrExp decomp parquet")
    print("=" * 60)
    ...
    # Read H1 panel: DV + controls + UncPreCEO + identifiers.
    # ceo_id not strictly required (QtrExp joins on file_name) but kept for diagnostics.
    columns = [
        "file_name",
        "start_date",
    ...
    # ----- Merge: DWZ QtrExp variant (Clarity_QtrExp + UncRes_QtrExp) on file_name -----
    ...
    # Per-IV non-NaN summary on full panel
    print("\n  Per-IV non-NaN counts on QtrExp sample:")
    for iv in KEY_IVS:
        n = panel[iv].notna().sum()
        print(f"    {iv:25s}: {n:,} ({100*n/len(panel):.1f}%)")

    # Build calendar year-quarter index for YQ FE specs (AFTER all merges)
...
def prepare_regression_data(
    ...
    # Natural QtrExp sample: require 3 QtrExp IVs non-NaN.
...
def _fit_one(df_panel: pd.DataFrame, dv: str, exog: List[str], base_fe: str) -> Any:
    """Single PanelOLS fit. Both branches use TimeEffects + firm-clustered SEs.

    Industry FE: constructor API with other_effects (absorbs FF12 dummies).
    Firm FE: from_formula with EntityEffects + TimeEffects.
    """
    if base_fe == "industry":
...
def _stash_iv_to_meta(meta: Dict[str, Any], model: Any, iv: str) -> None:
    ...
    If the IV is missing from model.params (drop_absorbed kicked in), all four
    keys are set to NaN and a warning is printed.
    """
    if iv not in model.params.index:
        print(f"    {iv}: DROPPED by drop_absorbed (collinear with FE) — cell will be empty")
    ...
    print(f"    {iv:25s}: beta={beta:+.4f}  SE={se:.4f}  p1={p_one:.4f} {stars}")
...
def _save_latex_table(all_results: List[Dict[str, Any]], out_dir: Path) -> None:
    ...
    # IV rows (3): ClarityCEO_QtrExp + UncResCEO_QtrExp + UncPreCEO. Each IV gets one coef row + one SE row.
    ...
    lines += [
        ...
        r"component (\textit{ClarityCEO\_QtrExp}, the negative of the CEO fixed effect from DWZ Eq.4) ",
        r"and a within-quarter state component (\textit{UncResCEO\_QtrExp}, the residual from DWZ Eq.4); ",
        r"both estimated under \emph{quarterly-expanding window} — at each quarter $t$, Eq.4 ",
        r"is fit on data through $t-1$ only and applied to quarter-$t$ obs. Eliminates ",
        r"forward-looking contamination at the cost of dropping early-panel observations. ",
        r"\textit{UncPreCEO} (raw presentation-segment uncertainty) enters as a third IV. ",
        r"$^{*}p<0.10$, $^{**}p<0.05$, $^{***}p<0.01$ (one-tailed). ",
        r"Per-IV directions: \textit{ClarityCEO\_QtrExp} $\beta < 0$; ",
        r"\textit{UncResCEO\_QtrExp} and \textit{UncPreCEO} $\beta > 0$. ",
    ...
    tex_path = out_dir / "h1_cash_holdings_ceo2iv_decomp_qtrexp_table.tex"
    with open(tex_path, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    print(f"  Saved: h1_cash_holdings_ceo2iv_decomp_qtrexp_table.tex")
...
def generate_report(
    ...
    lines = [
        "# Stage 4: H1 Cash Holdings — CEO 3-IV Decomp (DWZ Quarterly-Expanding + Pres)",
        "",
        f"**Generated:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        f"**Duration:** {duration:.1f} seconds",
        f"**Unit of observation:** individual earnings call (call-level)",
        f"**Sample:** Main only (excludes Finance FF12=11, Utility FF12=8); natural QtrExp sample",
        "",
        "## Model Specifications",
        "",
        "Three IVs estimated jointly per cell:",
        "- ClarityCEO_QtrExp (DWZ negated CEO FE under expanding window; tail β<0)",
        "- UncResCEO_QtrExp (DWZ residual under expanding window; tail β>0)",
    ...
    lines += [
        ...
        "One-tailed tests, per-IV directions: ClarityCEO_QtrExp β<0; UncResCEO_QtrExp + UncPreCEO β>0.",
        "",
        "## Results Summary",
        "",
    ...
    report_path = out_dir / "report_step4_H1_ceo2iv_decomp_qtrexp.md"
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    print("  Saved: report_step4_H1_ceo2iv_decomp_qtrexp.md")
...
def _write_suite_spec_json(
    ...
    """Emit canonical suite_spec_H1.ceo2.decomp.qtrexp.json from runner state.

    Per-IV directional p_one stitching: ClarityCEO_QtrExp uses negative direction,
    UncResCEO_QtrExp + UncPreCEO use positive. Controls extracted separately
    (p_one always None).
    """
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    log_dir = setup_run_logging(
        ...
        suite_name="H1_ceo2iv_decomp_qtrexp",
        timestamp=timestamp,
    )
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics — H1 Cash Holdings CEO 3-IV Decomp QtrExp (Main Sample)",
        label="tab:summary_stats_h1_ceo2_decomp_qtrexp",
    )
    ...
    if all_results:
        ...
        generate_attrition_table(attrition_stages, out_dir, "H1 Cash Holdings CEO 3-IV Decomp QtrExp")
    ...
    generate_manifest(
        ...
        output_files={
            ...
            "table": out_dir / "h1_cash_holdings_ceo2iv_decomp_qtrexp_table.tex",
        },
    ...
    # H1 significance summary — per-IV direction (Clarity NEG; UncRes/UncPre POS).
    print("Per-IV significance (p<0.05 one-tail; beta sign must match hypothesis direction):")
    for iv in DISPLAY_IVS:
        direction = IV_TAIL_DIRECTION.get(iv, "positive")

        def _sig_match(r, iv=iv, direction=direction):
            beta = r["meta"].get(f"{iv}_beta")
            p_one = r["meta"].get(f"{iv}_p_one")
            if beta is None or p_one is None or pd.isna(beta) or pd.isna(p_one):
                return False
            if direction == "positive" and beta > 0 and p_one < 0.05:
                return True
            if direction == "negative" and beta < 0 and p_one < 0.05:
                return True
            return False

        sig_count = sum(1 for r in all_results if _sig_match(r))
        print(f"  {iv:25s} (β {'>' if direction == 'positive' else '<'} 0): "
              f"{sig_count}/{len(all_results)} sig")
