...
# ------------------------------------------------------------------
SUITE_ID = "H12b.r"
SUITE_DIR_NAME = "h12b_dividend_payer_robustness"
SUITE_TITLE = "Speech Uncertainty and Dividend Payer Indicator (Orthogonal Speaker Partition)"
SUITE_CAPTION = (
    "H12b Robustness: Orthogonal Speaker Partition"
)
SUITE_LABEL = "tab:h12b_robust"
SAMPLE_LABEL = "Main sample (excludes financial and utility firms)."
HYP_DIR = "negative"  # H12b: beta(uncertainty) < 0 — more uncertainty -> less likely to pay
...
def _save_latex_table(all_results: List[Dict[str, Any]], out_dir: Path) -> None:
    ...
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{Speech Uncertainty and Dividend Payer Indicator (Orthogonal Speaker Partition)}",
        r"\label{tab:h12b_robust}",
        r"\scriptsize",
    ...
    tex_path = out_dir / "h12b_dividend_payer_robustness_table.tex"
...
def _write_suite_spec_json(
    ...
    """Emit canonical suite_spec_H12b.json from runner state."""
    ...
    header_rows = [
        [
            {"label": "DivPayerQ", "span": 6},
            {"label": r"DivPayerQ\_lead1", "span": 6},
        ]
    ]
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h12b_dividend_payer_robustness" / timestamp

    log_dir = setup_run_logging(
        log_base_dir=root / "logs",
        suite_name="H12b_Dividend_Payer_robust",
        timestamp=timestamp,
    )
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics --- H12b Dividend Payer Robustness (Main Sample)",
        label="tab:summary_stats_h12b_robust",
    )
    ...
    if all_results:
        ...
        generate_attrition_table(attrition_stages, out_dir, "H12b Dividend Payer Robustness")
