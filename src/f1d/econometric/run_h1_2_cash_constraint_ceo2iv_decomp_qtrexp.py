...
"""
================================================================================
STAGE 4: H1.2 HFC Channel — CEO 3-IV Decomp (QtrExp / no-look-ahead)
================================================================================
ID: econometric/run_h1_2_cash_constraint_ceo2iv_decomp_qtrexp
Description: DWZ Eq.5 decomposition variant of H1.2.ceo2 (HFC moderator),
             no-look-ahead expanding-window estimator. Companion to
             run_h1_2_cash_constraint_ceo2iv_decomp.py (Full method).

             Quarterly-expanding (QtrExp) design: at each quarter t, DWZ Eq.4
             is fit on data through t-1 only and applied to quarter-t obs.
             Eliminates forward-looking contamination at the cost of dropping
             early-panel observations. 3 IVs mean-centered on Main sample.

             Each method runs separately on its own natural sample and reports
             its own table. Dual-stack 5-IV precursor preserved in commits
             ae13357 + fb32343.
...
#   Cash ~ Clarity_QtrExp_c + UncRes_QtrExp_c + UncPre_c + Unr [+ 2 ints] + ctrls + FE
# All 3 centered on Main sample for clean interaction interpretation.
# Companion to Full-method runner (run_h1_2_cash_constraint_ceo2iv_decomp.py).

IVS_QTREXP_RAW = ["ClarityCEO_QtrExp", "UncResCEO_QtrExp", "UncPreCEO"]
IVS_QTREXP_CENTERED = ["ClarityCEO_QtrExp_c", "UncResCEO_QtrExp_c", "UncPreCEO_c"]
# Back-compat aliases (downstream uses IVS_REG_A_RAW / IVS_REG_A_CENTERED naming).
IVS_REG_A_RAW = IVS_QTREXP_RAW
IVS_REG_A_CENTERED = IVS_QTREXP_CENTERED
KEY_IVS = IVS_QTREXP_CENTERED  # canonical IV stack name
...
INT_A_UNCRES_UNRATED = "UncResCEO_QtrExp_c_x_Unrated"
INT_UNCPRE_UNRATED   = "UncPreCEO_c_x_Unrated"
ALL_INTERACTIONS = [INT_A_UNCRES_UNRATED, INT_UNCPRE_UNRATED]

# 6 display IV rows.
DISPLAY_IVS = [
    "ClarityCEO_QtrExp_c",   # Row 1 (uncond, NEG)
    "UncResCEO_QtrExp_c",    # Row 2 (uncond, POS)
...
VARIABLE_LABELS = {
    ...
    MOD_UNRATED:            "Unrated",
    INT_A_UNCRES_UNRATED:   r"UncRes (Qtr-Exp) $\times$ Unrated",
...
# ------------------------------------------------------------------
SUITE_ID = "H1.2.ceo2.decomp.qtrexp"
SUITE_DIR_NAME = "h1_2_cash_constraint_ceo2iv_decomp_qtrexp"
SUITE_TITLE = (
    "Financial Constraint-Moderated CEO Speech Uncertainty and Cash Holdings "
    "(CEO 3-IV Decomp: DWZ Quarterly-Expanding + Pres x Unrated)"
)
SUITE_CAPTION = (
    r"H1.2 CEO 3-IV Decomp HFC: ClarityCEO + UncResCEO (DWZ Qtr-Exp, no-look-ahead) + "
    r"UncPreCEO $\times$ Unrated Constraint"
)
SUITE_LABEL = "tab:h1_2_ceo2_decomp_qtrexp"
SAMPLE_LABEL = (
    "Main sample (excludes financial and utility firms). Fiscal years 2002-2016. "
    "DWZ Eq.5 decomposition under quarterly-expanding window (no-look-ahead)."
)
...
def load_panel(root_path: Path, panel_path: Optional[str] = None) -> Tuple[pd.DataFrame, Path]:
    """Load H1 panel + merge DWZ QtrExp decomp parquet.

    QtrExp 3-IV design:
      - UncPreCEO: H1 panel (raw).
      - ClarityCEO_QtrExp + UncResCEO_QtrExp: ceo_clarity_expanding/{latest}/
        ceo_clarity_qtrexp_residuals.parquet (call-level, file_name join).
    """
    print("\n" + "=" * 60)
    ...
    # Per-IV non-NaN summary on natural QtrExp sample (complete-case downstream).
    print("\n  Per-IV non-NaN counts on QtrExp sample:")
    for iv in IVS_QTREXP_RAW:
...
def center_iv(panel: pd.DataFrame) -> Tuple[pd.DataFrame, Dict[str, float]]:
    """Mean-center 3 QtrExp IVs on Main sample (after FF12 filter, before complete-case)."""
    print("\n" + "=" * 60)
    print("Centering 3 QtrExp IVs on Main sample")
    print("=" * 60)
...
def prepare_regression_data(
    ...
    if use_interactions:
        df[INT_A_UNCRES_UNRATED] = df["UncResCEO_QtrExp_c"] * df[MOD_UNRATED]
...
def _save_latex_table(all_results: List[Dict[str, Any]], out_dir: Path) -> None:
    ...
    tex_path = out_dir / "h1_2_cash_constraint_ceo2iv_decomp_qtrexp_table.tex"
...
def save_outputs(all_results: List[Dict[str, Any]], out_dir: Path) -> pd.DataFrame:
    ...
    for r in all_results:
        ...
        with open(out_dir / fname, "w", encoding="utf-8") as f:
            ...
            f.write(f"H1.2 CEO 3-IV Decomp QtrExp Financing-Constraint-Moderated Cash Holdings [{spec_type}]\n")
...
def generate_report(
    ...
    """Generate markdown report for CEO 3-IV QtrExp decomp variant."""
    iv_means_str = ", ".join(f"{k}={v:+.4f}" for k, v in iv_means.items())
    lines = [
        "# H1.2 CEO 3-IV Decomp Financing-Constraint-Moderated Cash Holdings (QtrExp)",
        "",
        f"**Generated:** {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}",
        f"**Duration:** {duration:.1f} seconds",
        f"**Design:** Cash ~ Clarity_c + UncRes_c + UncPre_c + Unr [+ 2 ints] + ctrls + FE",
        f"**Display rows (6 IVs):**",
        f"  1. ClarityCEO_QtrExp_c (NEG)  2. UncResCEO_QtrExp_c (POS)  3. UncPreCEO_c (POS)",
        f"  4. Unrated (two-tail)         5. UncResCEO_QtrExp_c x Unr (POS)  6. UncPreCEO_c x Unr (POS)",
    ...
    for r in all_results:
        ...
        if not m:
            continue
        b_cl = m.get("ClarityCEO_QtrExp_c_beta", np.nan)
        p_cl = m.get("ClarityCEO_QtrExp_c_p_one", np.nan)
        b_un = m.get("UncResCEO_QtrExp_c_beta", np.nan)
        p_un = m.get("UncResCEO_QtrExp_c_p_one", np.nan)
    ...
    with open(out_dir / "report_step4_H1_2_ceo2iv_decomp_qtrexp.md", "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    print("  Saved: report_step4_H1_2_ceo2iv_decomp_qtrexp.md")
...
def _write_suite_spec_json(
    ...
    """Emit canonical suite_spec_H1.2.ceo2.decomp.qtrexp.json from runner state.
    ...
    (ClarityCEO_QtrExp_c, UncResCEO_QtrExp_c, UncPreCEO_c) come from matched
    UNCONDITIONAL spec (interaction_col - 4); moderator level + interactions
    ...
    for interaction_col in display_cols:
        ...
        # Uncond model: ClarityCEO_QtrExp_c (NEG), UncResCEO_QtrExp_c (POS), UncPreCEO_c (POS)
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    log_dir = setup_run_logging(
        ...
        suite_name="H1_2_CashConstraint_ceo2iv_decomp_qtrexp",
        timestamp=timestamp,
    )

    print("=" * 80)
    print("STAGE 4: H1.2 CEO 3-IV DECOMP QtrExp Financing-Constraint-Moderated Cash Holdings")
    print("=" * 80)
    ...
    make_summary_stats_table(
        ...
        label="tab:summary_stats_h1_2_ceo2_decomp_qtrexp",
    )
    ...
    if all_results:
        ...
        generate_attrition_table(
            ...
            "H1.2 CEO 3-IV DECOMP QtrExp Financing-Constraint-Moderated Cash Holdings",
        )
