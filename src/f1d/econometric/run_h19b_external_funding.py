...
"""
================================================================================
STAGE 4: Test H19b External vs Internal Financing Hypothesis (Chang et al. 2006)
================================================================================
ID: econometric/run_h19b_external_funding
Description: H19b: Speech Uncertainty and External vs Internal Financing
             (Chang, Dasgupta & Hilary 2006, JF)

DV: ChangExternalFunding = 1 if firm used external financing in current fiscal year,
    based on cash-flow statement debt issuance net of repayments
    (dltisy - dltry + dlcchy).
    ChangExternalFunding_lead = 1 if firm used external financing in next fiscal year.

12 Model Specifications:
    Cols 1-6:   DV = ChangExternalFunding (contemporaneous)
    Cols 7-12:  DV = ChangExternalFunding_lead (next fiscal year)
...
shows LPM-FE outperforms logit at base rates <5%. At low treatment rates,
heteroscedasticity is negligible (Var = p(1-p)), linearity approximation
...
Inputs:
    - outputs/variables/h19b_h20b_financing/latest/h19b_h20b_financing_panel.parquet

Outputs:
    - outputs/econometric/h19b_external_funding/{timestamp}/...
...
# ------------------------------------------------------------------
SUITE_ID = "H19b"
SUITE_DIR_NAME = "h19b_external_funding"
SUITE_TITLE = "Speech Uncertainty and External vs Internal Financing (Chang et al. 2006)"
SUITE_CAPTION = (
    r"H19b: Speech Uncertainty and External vs Internal Financing (Chang et al. 2006)"
)
SUITE_LABEL = "tab:h19b"
SAMPLE_LABEL = "Main sample (excludes financial and utility firms)."
HYP_DIR = "negative"  # H19b: beta(uncertainty) < 0 — more uncertainty -> less external financing
...
MODEL_SPECS = [
    # ChangExternalFunding (contemporaneous) — Calendar Year FE
    {"col": 1,  "dv": "ChangExternalFunding",      "fe": "industry",    "controls": "base",     "extra_controls": []},
    {"col": 2,  "dv": "ChangExternalFunding",      "fe": "firm",        "controls": "base",     "extra_controls": []},
    {"col": 3,  "dv": "ChangExternalFunding",      "fe": "industry",    "controls": "extended", "extra_controls": []},
    {"col": 4,  "dv": "ChangExternalFunding",      "fe": "firm",        "controls": "extended", "extra_controls": []},
    # ChangExternalFunding (contemporaneous) — Year-Quarter FE (Extended controls only)
    {"col": 5,  "dv": "ChangExternalFunding",      "fe": "industry_yq", "controls": "extended", "extra_controls": []},
    {"col": 6,  "dv": "ChangExternalFunding",      "fe": "firm_yq",     "controls": "extended", "extra_controls": []},
    # ChangExternalFunding_lead (next fiscal year) — Calendar Year FE
    {"col": 7,  "dv": "ChangExternalFunding_lead", "fe": "industry",    "controls": "base",     "extra_controls": []},
    {"col": 8,  "dv": "ChangExternalFunding_lead", "fe": "firm",        "controls": "base",     "extra_controls": []},
    {"col": 9,  "dv": "ChangExternalFunding_lead", "fe": "industry",    "controls": "extended", "extra_controls": []},
    {"col": 10, "dv": "ChangExternalFunding_lead", "fe": "firm",        "controls": "extended", "extra_controls": []},
    # ChangExternalFunding_lead (next fiscal year) — Year-Quarter FE (Extended controls only)
    {"col": 11, "dv": "ChangExternalFunding_lead", "fe": "industry_yq", "controls": "extended", "extra_controls": []},
    {"col": 12, "dv": "ChangExternalFunding_lead", "fe": "firm_yq",     "controls": "extended", "extra_controls": []},
]
...
SUMMARY_STATS_VARS = [
    {"col": "ChangExternalFunding", "label": "External Funding (current, Chang)"},
    {"col": "ChangExternalFunding_lead", "label": "External Funding (lead, Chang)"},
    {"col": "ChangExternalFunding_lag", "label": "External Funding (lag, Chang)"},
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Stage 4: H19b External Funding Chang (call-level)",
    )
...
def load_panel(root_path: Path, panel_path: Optional[str] = None) -> Tuple[pd.DataFrame, Path]:
    """Load call-level H19b panel."""
    print("\n" + "=" * 60)
    print("Loading H19b panel")
    print("=" * 60)
    ...
    if panel_path:
        ...
        panel_dir = get_latest_output_dir(
            root_path / "outputs" / "variables" / "h19b_h20b_financing",
            required_file="h19b_h20b_financing_panel.parquet",
        )
        panel_file = panel_dir / "h19b_h20b_financing_panel.parquet"
...
def prepare_regression_data(
    ...
    # Create Lagged_DV: previous fiscal year's ChangExternalFunding classification
    panel = panel.copy()
    panel["Lagged_DV"] = panel["ChangExternalFunding_lag"]
...
def run_regression(
    ...
    for iv in KEY_IVS:
        ...
        # One-tailed: H19b expects beta < 0
...
def _save_latex_table(all_results: List[Dict[str, Any]], out_dir: Path) -> None:
    ...
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{Speech Uncertainty and External vs Internal Financing (Chang et al. 2006)}",
        r"\label{tab:h19b_external_funding}",
        r"\scriptsize",
    ...
    lines.append(
        r" & \multicolumn{6}{c}{ChangExternalFunding}"
        r" & \multicolumn{6}{c}{ChangExternalFunding (next year)} \\"
    )
    ...
    lines += [
        ...
        r"\textit{Notes:} ",
        r"$^{*}p<0.10$, $^{**}p<0.05$, $^{***}p<0.01$ (one-tailed for IVs ($\beta < 0$); two-tailed for controls). ",
        r"Standard errors (in parentheses) clustered at firm level. ",
        r"ChangExternalFunding = 1 if firm used external financing (Chang et al. 2006 cash-flow classification); 0 = internal financing only. ",
    ...
    tex_path = out_dir / "h19b_external_funding_table.tex"
...
def save_outputs(all_results: List[Dict[str, Any]], out_dir: Path) -> pd.DataFrame:
    ...
    for r in all_results:
        ...
        with open(out_dir / fname, "w", encoding="utf-8") as f:
            f.write(f"H19b External Funding Regression (Chang et al. 2006)\n")
...
def _write_suite_spec_json(
    ...
    """Emit canonical suite_spec_H19b.json from runner state."""
    ...
    header_rows = [
        [
            {"label": "ChangExternalFunding", "span": 6},
            {"label": r"ChangExternalFunding\_lead", "span": 6},
        ]
    ]
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h19b_external_funding" / timestamp

    log_dir = setup_run_logging(
        log_base_dir=root / "logs",
        suite_name="H19b_ExternalFunding",
        timestamp=timestamp,
    )

    print("=" * 80)
    print("STAGE 4: H19b External vs Internal Financing")
    print("=" * 80)
    ...
    # DV coverage report
    n_dv_valid = panel["ChangExternalFunding"].notna().sum()
    n_external = (panel["ChangExternalFunding"] == 1).sum()
    n_internal = (panel["ChangExternalFunding"] == 0).sum()
    print(f"\n  Main sample: {main_n:,} calls, {panel['gvkey'].nunique():,} firms")
    print(f"  ChangExternalFunding non-null: {n_dv_valid:,}")
    print(f"  External=1: {n_external:,} ({100*n_external/n_dv_valid:.2f}%)")
    print(f"  Internal=0: {n_internal:,} ({100*n_internal/n_dv_valid:.2f}%)")
    print(f"  ChangExternalFunding_lead non-null: {panel['ChangExternalFunding_lead'].notna().sum():,}")
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics --- H19b External Funding (Main Sample, Chang et al. 2006)",
        label="tab:summary_stats_h19b",
    )
    ...
    if all_results:
        ...
        attrition_stages = [
            ...
            ("ChangExternalFunding non-null", n_dv_valid),
            ("After complete-case + min-calls (col 1)", first["n_obs"]),
        ]
        generate_attrition_table(attrition_stages, out_dir, "H19b External Funding (Chang)")
