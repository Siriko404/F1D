...
"""
================================================================================
STAGE 4: Test H1 Cash Holdings Hypothesis — ROBUSTNESS (Orthogonal Speaker Partition)
================================================================================
ID: econometric/test_h1_cash_holdings_robustness
Description: H1 robustness test that swaps the joint-IV stack from the all-manager
             ...
             ladder, controls, FE, sample, and clustering as parent H1.
...
Key Independent Variables (4, speaker-orthogonal, all enter simultaneously):
...
Outputs:
    - outputs/econometric/h1_cash_holdings_robustness/{timestamp}/h1_cash_holdings_robustness_table.tex
    - outputs/econometric/h1_cash_holdings_robustness/{timestamp}/model_diagnostics.csv
    - outputs/econometric/h1_cash_holdings_robustness/{timestamp}/summary_stats.csv
    - outputs/econometric/h1_cash_holdings_robustness/{timestamp}/summary_stats.tex
    - outputs/econometric/h1_cash_holdings_robustness/{timestamp}/report_step4_H1_robustness.md
    - outputs/econometric/h1_cash_holdings_robustness/{timestamp}/sample_attrition.csv
    - outputs/econometric/h1_cash_holdings_robustness/{timestamp}/run_manifest.json
...
# ------------------------------------------------------------------
SUITE_ID = "H1.r"
SUITE_DIR_NAME = "h1_cash_holdings_robustness"
SUITE_TITLE = "Speech Uncertainty and Cash Holdings (Orthogonal Speaker Partition)"
SUITE_CAPTION = "H1 Robustness: Orthogonal Speaker Partition"
SUITE_LABEL = "tab:h1_robust"
...
SUMMARY_STATS_VARS = [
    ...
    # Key IVs (orthogonal speaker partition)
    ...
    {"col": "UncPreNoCEO", "label": "Non-CEO Mgr Pres Uncertainty"},    # Base controls
...
def _save_latex_table(all_results: List[Dict[str, Any]], out_dir: Path) -> None:
    ...
    lines = [
        r"\begin{table}[htbp]",
        r"\centering",
        r"\caption{Speech Uncertainty and Cash Holdings (Orthogonal Speaker Partition)}",
        r"\label{tab:h1_robust}",
        r"\scriptsize",
    ...
    lines += [
        ...
        r"\textit{Notes:} ",
        r"Robustness for H1: replaces the all-manager pool (UncAnsMgr/UncPreMgr) ",
        r"with its CEO/non-CEO speaker partition to address the nesting concern ",
        r"under joint estimation. ",
    ...
    tex_path = out_dir / "h1_cash_holdings_robustness_table.tex"
    with open(tex_path, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    print(f"  Saved: h1_cash_holdings_robustness_table.tex")
...
def generate_report(
    ...
    lines = [
        "# Stage 4: H1 Cash Holdings Robustness — Orthogonal Speaker Partition",
        "",
        ...
        "",
        "All 4 key IVs (orthogonal speaker partition) enter each model simultaneously:",
        "- UncAnsCEO, UncAnsNoCEO",
        "- UncPreCEO, UncPreNoCEO",
        "",
        "| Col | DV | FE | Controls |",
    ...
    report_path = out_dir / "report_step4_H1_robustness.md"
    with open(report_path, "w", encoding="utf-8") as f:
        f.write("\n".join(lines))
    print("  Saved: report_step4_H1_robustness.md")
...
def main(panel_path: Optional[str] = None) -> int:
    ...
    out_dir = root / "outputs" / "econometric" / "h1_cash_holdings_robustness" / timestamp

    # Setup logging
    log_dir = setup_run_logging(
        log_base_dir=root / "logs",
        suite_name="H1_robust_CashRatio",
        timestamp=timestamp,
    )
    ...
    make_summary_stats_table(
        ...
        caption="Summary Statistics — H1 Cash Holdings Robustness (Main Sample)",
        label="tab:summary_stats_h1_robust",
    )
    ...
    if all_results:
        ...
        generate_attrition_table(attrition_stages, out_dir, "H1 Cash Holdings Robustness")
