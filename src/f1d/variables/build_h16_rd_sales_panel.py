...
"""
================================================================================
STAGE 3: Build H16 R&D Sales Panel
================================================================================
ID: variables/build_h16_rd_sales_panel
Description: Build CALL-LEVEL panel for H16 R&D Investment Intensity hypothesis test.

    Following Jiang, John, and Larsen (2021): RDSales = xrdy / saley.
    Missing R&D expense set to zero; nonpositive sales excluded.

    This panel follows the H13 pattern:
...
Outputs:
    - outputs/variables/h16_rd_sales/{timestamp}/h16_rd_sales_panel.parquet
    - outputs/variables/h16_rd_sales/{timestamp}/summary_stats.csv
    - outputs/variables/h16_rd_sales/{timestamp}/run_manifest.json
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Stage 3: Build H16 R&D Sales Panel (call-level)",
...
def create_rdsales_lead(
    panel: pd.DataFrame, root_path: Optional[Path] = None
) -> pd.DataFrame:
    """Create RDSales_lead at call level.
    ...
    3. Sort by gvkey, fyearq_int; shift -1 within gvkey -> next-fiscal-year RDSales.
    ...
    """
    print("\n" + "=" * 60)
    print("Creating RDSales_lead (call-level, fiscal-year proxy)")
    print("=" * 60)

    if "RDSales" not in panel.columns:
        raise ValueError(
            "'RDSales' column missing -- cannot create lead variable."
        )
    ...
    firm_year_eoy = panel_valid.loc[
        latest_idx, ["gvkey", "fyearq_int", "RDSales"]
    ].copy()
    firm_year_eoy = firm_year_eoy.rename(
        columns={"fyearq_int": "fyearq_grp", "RDSales": "RDSales_eoy"}
    )

    print(f"  Unique firm-fiscal-years: {len(firm_year_eoy):,}")

    # Step 3: sort and shift to get next-fiscal-year RDSales
    ...
    firm_year_eoy["RDSales_lead_raw"] = firm_year_eoy.groupby("gvkey")[
        "RDSales_eoy"
    ].shift(-1)
    ...
    firm_year_eoy["RDSales_lead"] = np.where(
        consecutive, firm_year_eoy["RDSales_lead_raw"], np.nan
    )

    n_last_year = firm_year_eoy["RDSales_lead_raw"].isna().sum()
    n_gap_year = ((~consecutive) & firm_year_eoy["RDSales_lead_raw"].notna()).sum()
    n_valid_lead = firm_year_eoy["RDSales_lead"].notna().sum()
    print(
    ...
    lead_lookup = firm_year_eoy[["gvkey", "fyearq_grp", "RDSales_lead"]].copy()
    ...
    n_calls_no_lead = panel["RDSales_lead"].isna().sum()
    print(f"  Call-level rows: {len(panel):,}")
    print(
        f"  Calls without lead (last-fiscal-year + gaps + missing fyearq): {n_calls_no_lead:,}"
    )
    print(f"  Calls with valid lead: {panel['RDSales_lead'].notna().sum():,}")

    return panel


def create_rdsales_lag(
    panel: pd.DataFrame,
) -> pd.DataFrame:
    """Create RDSales_lag at call level (previous fiscal year's RDSales).

    Uses the same firm_year_eoy table built during lead creation (fyearq_int
    already attached). Shift +1 within gvkey → previous fiscal year value.
    Validates consecutive fiscal years (fyearq - 1).
    """
    print("\n" + "=" * 60)
    print("Creating RDSales_lag (call-level, previous fiscal year)")
    print("=" * 60)

    if "RDSales" not in panel.columns or "fyearq_int" not in panel.columns:
        raise ValueError("'RDSales' or 'fyearq_int' missing — run create_rdsales_lead first.")
    ...
    firm_year_eoy = firm_year_eoy.sort_values(["gvkey", "fyearq_grp"]).reset_index(drop=True)

    # Shift +1 → previous fiscal year's value
    firm_year_eoy["fyearq_prev"] = firm_year_eoy.groupby("gvkey")["fyearq_grp"].shift(1)
    firm_year_eoy["RDSales_lag_raw"] = firm_year_eoy.groupby("gvkey")["RDSales_eoy"].shift(1)

    # Validate: previous row must be exactly fyearq - 1
    consecutive = firm_year_eoy["fyearq_prev"] == (firm_year_eoy["fyearq_grp"] - 1)
    firm_year_eoy["RDSales_lag"] = np.where(
        consecutive, firm_year_eoy["RDSales_lag_raw"], np.nan
    )

    n_first_year = firm_year_eoy["RDSales_lag_raw"].isna().sum()
    n_gap_year = ((~consecutive) & firm_year_eoy["RDSales_lag_raw"].notna()).sum()
    n_valid_lag = firm_year_eoy["RDSales_lag"].notna().sum()
    print(f"  Firm-fiscal-years with no prior year (first per firm): {n_first_year:,}")
    ...
    # Merge back to call level
    lag_lookup = firm_year_eoy[["gvkey", "fyearq_grp", "RDSales_lag"]].copy()
    ...
    print(f"  Call-level rows: {len(panel):,}")
    print(f"  Calls with valid lag: {panel['RDSales_lag'].notna().sum():,}")
...
def main(year_start: Optional[int] = None, year_end: Optional[int] = None) -> int:
    ...
    stats: Dict[str, Any] = {
        "step_id": "build_h16_rd_sales_panel",
        "timestamp": timestamp,
        "variable_stats": [],
        "timing": {},
        "panel": {},
    ...
    out_dir = root / "outputs" / "variables" / "h16_rd_sales" / timestamp
    ...
    print("=" * 80)
    print("STAGE 3: Build H16 R&D Sales Panel (call-level)")
    print("=" * 80)
    ...
    # Step 3: Create RDSales_lead and RDSales_lag at call level (use fyearq)
    panel = create_rdsales_lead(panel, root_path=root)
    panel = create_rdsales_lag(panel)
    ...
    for sample in ["Main", "Finance", "Utility"]:
        ...
        n_lead = panel.loc[panel["sample"] == sample, "RDSales_lead"].notna().sum()
    ...
    panel_path = out_dir / "h16_rd_sales_panel.parquet"
    panel.to_parquet(panel_path, index=False)
    print(
        f"  Saved: h16_rd_sales_panel.parquet "
    ...
    report_lines = [
        "# Stage 3: H16 R&D Sales Panel Build Report",
        "",
        ...
        "",
        "## Dependent Variables (R&D Investment Intensity)",
        f"- **RDSales (t):** {panel['RDSales'].notna().sum():,} calls",
    ...
    report_path = out_dir / "report_step3_h16_rd_sales.md"
    with open(report_path, "w") as f:
        f.write("\n".join(report_lines))
    print(f"  Saved: report_step3_h16_rd_sales.md")

    print("\n" + "=" * 80)
    print("H16 R&D Sales Panel build complete.")
