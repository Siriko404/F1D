...
"""
================================================================================
STAGE 3: Build H5b Johnson (2004) Analyst Dispersion Panel
================================================================================
ID: variables/build_h5b_johnson_disp_panel
Description: Build CALL-LEVEL panel for H5b Johnson Dispersion hypothesis test.

    Unit of observation: individual earnings call (file_name).
    DV: JohnsonDISP2 = SD(current-FY analyst forecasts at month-end) / atq
    Lead DV: JohnsonDISP2_lead = next fiscal quarter's JohnsonDISP2
    Lagged DV control: JohnsonDISP2_lag = prior fiscal quarter's JohnsonDISP2
...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Stage 3: Build H5b Johnson Dispersion Panel",
    )
...
def build_panel(
    ...
    print("\n" + "=" * 60)
    print("Building H5b Johnson Dispersion Panel")
    print("=" * 60)
...
def create_lead_lag_variables(panel: pd.DataFrame, root_path: Optional[Path] = None) -> pd.DataFrame:
    ...
    if "JohnsonDISP2" not in panel.columns:
        raise ValueError("'JohnsonDISP2' column missing.")
    ...
    firm_qtr["JohnsonDISP2_lead"] = np.where(
        consecutive_qtr, firm_qtr["JohnsonDISP2_lead_raw"], np.nan
    )

    n_lead = firm_qtr["JohnsonDISP2_lead"].notna().sum()
    print(f"  Firm-quarters with valid next-quarter lead: {n_lead:,}")

    # --- Lag: Prior fiscal quarter's JohnsonDISP2 ---
    firm_qtr["fiscal_qtr_id_prev"] = firm_qtr.groupby("gvkey")["fiscal_qtr_id"].shift(1)
    firm_qtr["JohnsonDISP2_lag_raw"] = firm_qtr.groupby("gvkey")["JohnsonDISP2"].shift(1)
    ...
    firm_qtr["JohnsonDISP2_lag"] = np.where(
        consecutive_prev, firm_qtr["JohnsonDISP2_lag_raw"], np.nan
    )

    n_lag = firm_qtr["JohnsonDISP2_lag"].notna().sum()
    print(f"  Firm-quarters with valid prior-quarter lag: {n_lag:,}")

    # --- Merge lead + lag back to call level ---
    lead_lag_lookup = firm_qtr[["gvkey", "fiscal_qtr_id", "JohnsonDISP2_lead", "JohnsonDISP2_lag"]].copy()
    ...
    print(f"  Calls with next-quarter lead: {panel['JohnsonDISP2_lead'].notna().sum():,}")
    print(f"  Calls with prior-quarter lag: {panel['JohnsonDISP2_lag'].notna().sum():,}")
...
def save_outputs(panel: pd.DataFrame, stats: Dict[str, Any], out_dir: Path, root: Path, timestamp: str) -> None:
    ...
    panel_path = out_dir / "h5b_johnson_disp_panel.parquet"
    panel.to_parquet(panel_path, index=False)
    print(f"\n  Saved: h5b_johnson_disp_panel.parquet ({len(panel):,} rows, {len(panel.columns)} cols)")
...
def main(year_start: Optional[int] = None, year_end: Optional[int] = None) -> int:
    ...
    stats: Dict[str, Any] = {
        "step_id": "build_h5b_johnson_disp_panel",
        "timestamp": timestamp,
        "variable_stats": [],
    }

    root = Path(__file__).resolve().parents[3]
    out_dir = root / "outputs" / "variables" / "h5b_johnson_disp" / timestamp

    log_dir = setup_run_logging(
        log_base_dir=root / "logs",
        suite_name="H5b_JohnsonDisp",
        timestamp=timestamp,
    )
    ...
    print("=" * 80)
    print("STAGE 3: Build H5b Johnson Dispersion Panel")
    print("=" * 80)
