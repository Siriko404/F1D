...
def synthetic_manager_clarity_panel(n_rows: int = 100, seed: int = 42) -> pd.DataFrame:
    ...
    return pd.DataFrame({
        ...
        "UncAnsMgr": np.random.uniform(0.5, 2.5, n_rows),
        "UncPreMgr": np.random.uniform(0.5, 2.0, n_rows),
        "UncQue": np.random.uniform(1.0, 3.0, n_rows),
        "NegCall": np.random.uniform(0.5, 1.5, n_rows),
        "StockRet": np.random.uniform(-0.1, 0.2, n_rows) * 100,
        "MarketRet": np.random.uniform(-0.05, 0.1, n_rows) * 100,
        "EPSgrowth": np.random.uniform(-0.3, 0.5, n_rows),
...
def synthetic_ceo_clarity_panel(n_rows: int = 100, seed: int = 42) -> pd.DataFrame:
    ...
    return pd.DataFrame({
        ...
        "sample": sample_col,
        "year": [2002 + (i // 10) % 17 for i in range(n_rows)],
        "UncAnsCEO": np.random.uniform(0.5, 2.5, n_rows),
        "UncPreCEO": np.random.uniform(0.5, 2.0, n_rows),
        "UncQue": np.random.uniform(1.0, 3.0, n_rows),
        "NegCall": np.random.uniform(0.5, 1.5, n_rows),
        "StockRet": np.random.uniform(-0.1, 0.2, n_rows) * 100,
        "MarketRet": np.random.uniform(-0.05, 0.1, n_rows) * 100,
