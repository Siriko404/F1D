...
def test_mypy_panel_ols():
    ...
    assert shared.panel_ols.__name__ == "shared.panel_ols"
...
def test_mypy_iv_regression():
    ...
    assert shared.iv_regression.__name__ == "shared.iv_regression"
...
def test_mypy_financial_utils():
    ...
    assert shared.financial_utils.__name__ == "shared.financial_utils"
...
def test_panel_ols_function_signature():
    ...
    hints = get_type_hints(shared.panel_ols.run_panel_ols)
...
def test_iv_regression_function_signature():
    ...
    hints = get_type_hints(shared.iv_regression.run_iv2sls)
    # Check that key parameters have type hints
    assert "df" in hints
    assert "dependent" in hints
...
def test_financial_utils_function_signature():
    ...
    hints = get_type_hints(shared.financial_utils.calculate_firm_controls)
    # Check that key parameters have type hints
    assert "row" in hints
