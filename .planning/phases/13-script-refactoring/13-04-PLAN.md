...
<objective>
Parameterize string matching thresholds in config and integrate RapidFuzz library across all scripts using fuzzy matching.
...
Output: Updated config, string_matching.py module, and refactored scripts using config-driven thresholds.
</objective>
...
4. Scripts import from shared.string_matching
5. Scripts use config-driven thresholds
...
4. 1.2_LinkEntities.py refactored to use config-driven matching
5. 1.4_AssembleManifest.py refactored (if it uses fuzzy matching)
6. All fuzzy matching thresholds now configurable
</success_criteria>
