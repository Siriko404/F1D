...
Four fits (advisor-specified):
  primary             ex-EPS  (10 ctrl)         N~2,598   <- HEADLINE
  sens_with_eps       with-EPS (11 ctrl)        N~1,985   paper-text spec
  sens_no_eps_no_ads  ex-EPS ex-ADS (9 ctrl)    N~2,598   tests LEI deviation
  diag_sample_effect  ex-EPS ctrl ON the 1,985  N~1,985   isolates
                      with-EPS sample                      selection vs control
...
# Macro (fx/vix/umcsent/livingston/ads) is TIME-ONLY: 5 vars over only 4
# calendar quarters, perfectly absorbed by the FIC100 x cal_yr_qtr FE
# (Stata reghdfe absorbs silently; linearmodels rank-fails). Campello-
# faithful => macro lives IN the FE, NOT as explicit regressors (Table 8:
# "Controls: Yes" + "Industry x time FE: Yes" + zero macro coefficients).
# Root cause: _diag_s7_rank.py. So explicit controls = FIRM-level only.
...
def main() -> None:
    ...
    primary = _fit(df, FIRM5, FIRM5)                       # N ~ 2,598
    # SENS — + EPS (paper-text spec; treatment-selected subsample)
    sens_with_eps = _fit(df, FIRM5_EPS, FIRM5_EPS)         # N ~ 1,985
    # DIAG — FIRM5 controls but ON the with-EPS 1,985 sample:
    #        isolates sample-selection effect from the EPS-control effect.
    diag_sample_effect = _fit(df, FIRM5, FIRM5_EPS)        # N ~ 1,985
    ...
    out = {
        ...
        "macro_absorbed_by_FE": (
            ...
            "over 4 quarters -> perfectly absorbed by the FIC100 x "
            "cal_yr_qtr FE (root cause _diag_s7_rank.py). They are IN the "
            "model via the FE, NOT explicit regressors — Campello-faithful "
            "(Table 8: 'Controls: Yes' + 'Industry x time FE: Yes', no "
            "macro coefs). The PhillyFed-LEI->ADS deviation now lives in "
            "the FE; a drop-ADS sensitivity is NOT applicable at this FE "
            "structure (would require removing time FE = a different spec)."),
        "selection_bias_note": (
            "EPS missingness 36.1% treated vs 14.2% control (21.9pp gap, "
            "step6 eps_missing_integrity.json). Listwise EPS => "
            "treatment-correlated selection. PRIMARY = ex-EPS firm-controls "
            "(N~2,598); with-EPS demoted to sensitivity."),
        "interpretation_rules": [
            ...
            "compare delta_hat sign/scale to paper_anchor +0.231 as "
            "direction check ONLY; attenuation/sign-flip is structurally "
            "expected (vendor gap + admittedly-imperfect beta^UK first "
            "stage + N 2,598 vs 17,170 + EPS-link selection), NOT a defect "
            "— do NOT change recipe or add sensitivities to 'fix' it",
        ],
        "cash_dv_distribution_no_winsor": {
            "n": int(cash.size),
            "min": float(cash.min()), "p1": float(cash.quantile(.01)),
            "p50": float(cash.quantile(.50)), "p99": float(cash.quantile(.99)),
            "max": float(cash.max()),
            "note": "NOT winsorized (paper Table 8 does not); extreme values "
                    "are a reported observation, not a silent fix",
        },
    }
    ...
    (out_dir / "metadata.json").write_text(json.dumps({
        ...
        "primary_spec": "FIRM-controls-only (tobinq/cf/logassets/"
                        "salesgrowth/stockret, all t-1), N~2,598. Macro "
                        "absorbed by FIC100xcal_yr_qtr FE (not regressors). "
                        "EPS excluded from primary (21.9pp treatment-"
                        "correlated IBES-link gap); with-EPS = sensitivity.",
        "deviations_logged": [
            "macro (5) absorbed by industry x time FE, NOT explicit "
            "regressors — Campello-faithful per Table 8; root cause "
            "_diag_s7_rank.py (5 time-only vars x 4 quarters rank-fail)",
            ...
            "vendor gap (yfinance/CRSP/BoE vs Bloomberg) + admittedly-"
            "imperfect beta^UK (fn13) + N 2,598 vs 17,170 => attenuation/"
            "sign-flip structurally expected, NOT a defect",
        ],
    ...
    print("\nRESULT — eq-14 cash DiD  (paper anchor: delta=+0.231***, "
          "SE=0.059, N=17,170, R2=0.21)")
    row("PRIMARY firm5", primary)
    row("sens_with_eps", sens_with_eps)
    row("diag_sample_eff", diag_sample_effect)
    print(f"\n  CASH_DV (no winsor) p1={cash.quantile(.01):+.3f} "
