"""TEXTUAL §1+7-arm eq-(14) DiD — Campello §IV.A.2, scope=Item1+7.
...
treatment assignment changes: step3 βᵁᴷ-tercile → step3b3 §1+7-scoped textual (>5/==0 verbatim; scope=10-K Item1+Item7
per Campello Appendix-E house convention, supp_FULL L1341-1343).
...
STEP7B = ROOT / "outputs" / "campello_rebuild" / "step7b3_textual_did_sec17"


def main() -> None:
    print("=== TEXTUAL §1+7-arm eq-(14) DiD (treatment = step3b3 >5/0) ===\n")
    ...
    (od / "summary.json").write_text(json.dumps({
        ...
        "model": prev["model"] + "  | TREATMENT = §1+7-scoped textual >5/0 "
            "(step3b3, Sina 2026-05-18; 9-kw verbatim, scope=Item1+7 "
            "Campello house convention)",
        ...
        "treatment_definition": "TEXTUAL §1+7: 9 Campello keywords counted in 10-K "
            "Item1(Business)+Item7(MD&A) only (Appendix-E house "
            "convention); >5/==0/1-5-excl verbatim; step3b3 " + s3b.name + ". "
            "vs Campello 807/433: §1+7 ⇒ control 465≈433 (near-match), "
            "treated 1,458 (1.8x; verdict gated).",
        "results": [{
            "tag": "TEXTUAL_SEC17_FULL_PANEL",
