...
**Cross-reference — do not bury (advisor "don't-lose"):** this integration is orthogonal to and must NOT delay the parked thesis-critical defect: `scripts/campello_rebuild/step7_did_cash.py` `PAPER_ANCHOR` and `outputs/campello_rebuild/REBUILD_CONCLUSION_2026-05-15.md` still hold the wrong `+0.0539`; the true Campello Table 8 col-1 cash coefficient is `+0.231***` SE (0.059) (verified: `tmp/campello_v2/campello_paper_FULL.md` L3785/L3844 + user visual PDF confirmation). Finding is durable in `memory/project_campello_anchor_error_2026_05_15.md`; the FIX is tracked by audit tasks #17 (A1, in_progress) and #25 (A9) and remains outstanding in the repo.
