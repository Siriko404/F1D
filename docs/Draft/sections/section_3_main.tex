...
Section~\ref{sec:framework:estimation} describes the construction of the three CEO speech-uncertainty independent variables (ClarityCEO, UncResCEO, UncPreCEO). The cash-to-assets ratio (Cash$_{t}$, Cash$_{t+1}$) is cash and short-term investments scaled by total assets. Variable definitions are provided in the appendix; summary statistics are reported in Table~\ref{tab:summary_stats}.

\subsection*{III.B Cash Holdings (HC)}
\label{sec:main:hc}
% Governing decisions: Beat 1 + 2 + headline finding.

Table~\ref{tab:h1_ceo2_decomp} reports the main estimation results for cash holdings on the three CEO speech-uncertainty independent variables. Across the twelve specifications, UncResCEO loads positively on cash holdings in every cell, with one-tailed p-values below the ten-percent level.

ClarityCEO loads negatively in all twelve specifications, consistent with the precautionary direction (lower clarity, higher cash); the loading is significant at the ten-percent level under one-tailed tests in nine of twelve cells. UncPreCEO is positive but statistically indistinguishable from zero across all twelve specifications.

As a robustness check, Table~\ref{tab:h1_ceo2_decomp_qtrexp} replicates the main results using a quarterly-expanding-window construction of ClarityCEO and UncResCEO that eliminates look-ahead bias by re-estimating the speech-uncertainty decomposition each quarter from data available through that quarter only. UncResCEO remains positive and significant in twelve of twelve specifications; the ClarityCEO and UncPreCEO patterns are preserved.
