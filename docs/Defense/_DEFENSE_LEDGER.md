...
|---|---|---|---|---|---|---|---|
| Q1 | Where is your identification? This is all correlation. | econometrics | H | RATIFIED SPOKEN ANSWER (~30s): "You're right, and the thesis claims nothing causal. Firm and calendar fixed effects remove stable firm traits and market-wide shocks, but deal timing is chosen, so I characterize a within-firm regularity and leave the mechanism open. What makes it worth documenting is its timing: it appears only in the last pre-deal quarter and is gone the moment the deal is public. That shape is what any future causal design will have to explain." Backup depth: FE handle stable traits + common shocks; unhandled = firm-specific time-varying stress arriving with the deal quarter; "first facts precede causal designs"; a rival busy-quarter story must also end EXACTLY at announcement. | thesis 2.4, 3.1; floor | S14 | RATIFIED 07-09 |
