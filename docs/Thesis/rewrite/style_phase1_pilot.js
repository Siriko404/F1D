export const meta = {
  name: 'style-phase1-pilot',
  description: 'Phase-1 style ANALYSIS for one section type: 3 paraphrased panel agents discover where our prose is needlessly more complex than the corp-fin exemplars, a deterministic in-script gate drops any finding whose quotes are not verbatim, then a redteam verifies+merges by reference (never re-authors). Analysis only — no rewriting.',
  phases: [
...
// ================= workflow =================
const A = args // __BUNDLE_ANCHOR__  (Python combiner injects the bundle literal here)

phase('Panel')
const ctx = buildContext(A)
const raw = await parallel([1, 2, 3].map(v => () =>
  agent(panelPrompt(v, A.type, ctx), { schema: FINDINGS_SCHEMA, phase: 'Panel', label: `panel-${v}` })))

// gate: every quote must be a verbatim substring of its source; cardinality (>=2 quotes, >=2 papers, >=1 ours)
const exByPaper = {}
for (const s of A.exemplars) { (exByPaper[s.paper] = exByPaper[s.paper] || []).push(...s.paragraphs) }
const exAll = Object.values(exByPaper).flat()          // paper-agnostic fallback (labels are fuzzy)
const ourById = {}
...
raw.forEach((r, ai) => (r && r.findings || []).forEach((f, fi) => {
  ...
  // quote must be verbatim in the cited paper OR (paper label may be fuzzy) in ANY exemplar paragraph
...
log(`Panel: ${clean.length} findings passed the verbatim gate, ${rejected.length} rejected`)

if (clean.length === 0) return { type: A.type, clean_findings: [], rejected, decisions: null, note: 'no findings survived the gate' }

phase('Redteam')
const decisions = await agent(redteamPrompt(A.type, clean), { schema: REDTEAM_SCHEMA, phase: 'Redteam', label: 'redteam' })

// ---- assemble profile IN-SCRIPT: copy kept/canonical findings VERBATIM by id (no re-authoring) ----
...
const profile = [...keepIds].map(id => byId[id]).filter(Boolean)   // verbatim panel findings, selected only

// no-silent-caps: surface any clean finding the redteam neither kept, merged, nor rejected
...
if (unhandled.length) log(`WARN: ${unhandled.length} clean findings unhandled by redteam: ${unhandled.join(',')}`)
log(`Profile: ${profile.length} findings kept (of ${clean.length} clean; ${rejected.length} gate-rejected)`)

const guardrail_collisions = profile.filter(f => f.guardrail_collision).map(f => f.id)
if (guardrail_collisions.length) log(`GUARDRAIL: ${guardrail_collisions.length} kept findings sit near load-bearing meaning — mandatory human review: ${guardrail_collisions.join(',')}`)

return {
  type: A.type,
  profile,                                   // <-- the deliverable, persist verbatim to <type>_profile.json
  guardrail_collisions,                      // <-- findings near load-bearing meaning; human MUST review
  side_notes: decisions.side_notes || [],
  gate_rejected: rejected,
  redteam_rejected: decisions.reject || [],
  merges: decisions.merge || [],
  unhandled,
}
