...
6. **Data-source fixes (3.1, verified):** ExecuComp added as the 5th source (CEO identity → the eq-4 manager FE; VERIFIED `src/f1d/sample/build_tenure_map.py` Step 1.3, input `comp_execucomp.parquet`). S&P 1500 sample bound disclosed (ExecuComp coverage; user-confirmed 2026-06-14). **OPEN:** the S&P-1500 wording is scoped to the *residual* sample; user may want it widened to the *whole* sample (verify whether the cash-ratio regressions are also S&P-1500-bounded).
