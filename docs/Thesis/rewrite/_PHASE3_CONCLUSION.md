...
1. **Base bind (theoretically symmetric).** A CEO withholding a pending deal cannot confirm or deny it (basic1988 / rule10b5; verrecchia1983 / dye1985; hollander2010) → uncertainty can surface in the unscripted Q&A. In principle this binds cash and stock alike → predicts a run-up for *any* withheld deal. **Caveat (not shown for stock):** the run-up appears in the *pooled* all-types sample (cash/mixed-driven; alltypes PRE1 0.0533***), but the **stock arm alone is null** (−0.0429 n.s.) — so the bind operating *within stock* is an **assumption**, not independently established.
...
1. **The `placebo` contradiction (advisor-caught; self-audit missed it).** Stock cannot be both an *inert placebo* and an *active tone-manager*. This **re-scopes the tweaks**: it touches **identification framing**, not just motivation (see the tweak-map item above). **Context (Sina 2026-06-25): "placebo" was an *arbitrary, never-supported* theory claim that the supervisors ALREADY flagged.** So replacing it is mandatory regardless of our framing — and the masking "managed comparison" both *fixes that pre-existing flag* and *resolves the contradiction.* (Net: this is a win, not a new burden.) The new framing must be applied to the propositions in Phase 4.
...
5. **The stock-management leg rests on ONE un-refereed SSRN preprint (thewissen — "not peer reviewed", verbatim).** "Literature already supports stock optimism" was *assumed, never searched.* **Phase-4 must cite the published canon** (Shleifer–Vishny market-driven acquisitions; Erickson–Wang / Louis on earnings management before stock-financed mergers) **or state the single-preprint reliance honestly** — else "your cash justification rests on one preprint" lands clean.
6. **"alltypes confirms the bind for all payment types" was the same overclaim** (now softened in FRAMEWORK step 1). `alltypes` PRE1 0.0533*** is *pooled* (cash/mixed-driven); the stock arm alone is −0.0429 n.s. **Recurring pattern: every stock-side claim is a null in our data** — bind-on-stock, stock-suppression, "lower bound", all-types-symmetry. The framework is **MOTIVATION** (cash result + thewissen + a symmetry *assumption*); **C1 carries the paper** regardless.
