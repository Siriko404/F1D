...
def main():
    ...
    for pid, (paper, match, question) in P1.items():
        ...
        props[pid]["verification"] = {
            "method": "NLM",
            "source": {"id": sid, "title": title},
            "query": query,
            "answer": answer,
            "quotes": quotes,            # verbatim cited_text + char-span + chunk_id
            "located": located,          # quote + page + section, from the NLM answer
            "verdict": "PENDING",        # human adjudicates on the verbatim quotes
        }
        ...
        print(f"  wrote {len(quotes)} verbatim quotes, {len(located)} located (page+section); committed")
    print("\nP1 NLM verification written to the ledger. Adjudicate verdicts on the quotes.")
