...
function constraints(brief) {
  return [
    ...
    '- PLAIN LANGUAGE (THE most important style rule; apply the abstract rulebook\'s "Flat Declarative, Plain Gloss" device in EVERY section): write so a smart reader with NO finance training -- a parent, not an economist -- understands it on first read. Short subject-verb-object sentences, plain everyday words instead of ornament, one idea per sentence. Keep each technical term a prop requires (e.g. residual uncertainty, fixed effects, within-$R^2$) but add a brief plain-English gloss that RESTATES only what the prop already says -- never a new fact, number, or analogy. PRECEDENCE: plainness NEVER overrides accuracy -- keep every number, sign, significance star, hedge, and register_lock exactly; if a plain phrasing would blur a null, a hedge, or a "correlational-not-causal" limit, use the precise wording even if it is longer. Simpler, never looser.',
...
const LANES = [
  ...
  ['flow', 'FLOW/VOICE + PLAIN-LANGUAGE lane. Topic-sentence placement, paragraph architecture, first-person author voice, transitions, the seams to adjacent sections; flag breaks in cohesion or register. ALSO enforce PLAIN LANGUAGE (the thesis must read so a smart reader with NO finance training follows on first read): flag any sentence such a reader could not follow, needless ornament/latinate wording where a plain word exists, or a technical term left un-glossed that no proposition required. Flag only CLEAR violations -- not long sentences that are still clear -- and never propose weakening a number, hedge, or register-lock to achieve plainness.'],
...
async function writeTeam(team) {
  ...
  return await parallel(briefs.map(brief => async () => {
    ...
    const merged = await agent(
      `You are the section editor. Below are ${clean.length} independent drafts of the SAME section, each already number- and honesty-gate-clean. Produce ONE best version: choose the strongest AND PLAINEST faithful rendering of each paragraph (when two renderings are equally faithful, pick the one a reader with no finance training follows more easily) and smooth the flow. Invent nothing; every number/citation must already appear in one of the drafts; keep all hedges and register-locks.\n\n${brmain(brief)}\n\nDRAFTS:\n${JSON.stringify(clean.map(d => d.paragraphs), null, 1)}`,
