...
- EXTERNAL LITERATURE (intro Move 1 + positioning): every cited prior-work claim is typed external-NLM and carries its bibkey + the NLM-receipt pointer it rests on (unit 8). If a needed claim has NO live receipt yet, mark it in open_decisions as "needs NLM verification (to be run BETWEEN Phase A and Phase B, before allocation)" -- the verify step happens after this plan names its exact cites and before Phase B builds on them, so an NLM refutation costs a chain edit, not a re-allocation. The four dropped bibkeys (everhart2025, gokkaya2025, bushee2018, lerman2026): cite ONLY if load-bearing AND a live receipt exists AND mark "re-add bibitem at Phase D"; else drop the claim. The gap statement ("no prior work measures...") is typed framing (a negative cannot be NLM-proven) -- mark and keep.
- ABSTRACT CITATIONS: the abstract carries NO \\citet/\\citep. It names the measure DESCRIPTIVELY (e.g. "a residual measure of CEO question-and-answer uncertainty"); the DWZ decomposition and all author names are cited in Section 2 and the Introduction, never in the abstract. (This pre-empts the abstract's "name the measure" requirement from contradicting its "no citation" convention.)
