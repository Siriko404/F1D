...
2. **PER-SUBSECTION workflow (Sina, 2026-06-24 — supersedes the earlier "review once at end"). NO SCRIPTS (Sina: "scripts are worst").** For EACH subsection, in order:
   - (a) **PULL** all analysis — the style-profile findings + the ledger spine (propositions / number_audit / guardrails).
   - (b) **REWRITE** the language (constrained edit: remove named anti-pattern, keep every proposition + protected string; splits ok; no reorder/merge/add/drop).
   - (c) **GATE — BY HAND, no code.** List every `number_audit` value + every protected guardrail phrase; confirm each survives VERBATIM in the new prose; confirm no new digit introduced; confirm sentence count only holds or rises. Shown in the diff file so it is auditable.
   - (d) **ADVISOR — seeded with a CLOSED checklist** (not "is the meaning ok?"): every proposition claim still present + every guardrail NEVER-trap absent. (LLM meaning-check failed 3× this session → it gets a checklist, never an open vibes-check, and it is NOT the gate — step (c) is.)
   - (e) **RATIFY** — present to Sina; he approves each subsection individually (NOT one approval for all 16). On approval: commit the diff + progress-table row (checkpoint, so a mid-run compaction resumes exactly).
