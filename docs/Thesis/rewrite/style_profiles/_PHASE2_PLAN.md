...
> ⚠️ **SCOPE (2026-06-24): decisions 1–7 below were the MANUAL per-subsection workflow used to rewrite the ABSTRACT (done, ratified `5a073b4b`). For subsections 2–16 Sina chose to AUTOMATE via an opus-agent harness → `_PHASE2_HARNESS_DESIGN.md` governs.**
> - **SUPERSEDED for 2–16:** decision 1 ("inline, NOT a harness") and per-subsection ratify (2g) → in the harness, the red-team is the per-unit authority and **Sina reviews the FINAL output**, not each subsection. The manual v2 workflow (decision 2) is the RECORD of how the abstract was done, not the plan for the rest.
> - **STILL HOLD everywhere:** spine frozen, clones = write target, keep jargon, propositions beat old prose, and the invariant = each proposition's CLAIM MEANING (not its words).
