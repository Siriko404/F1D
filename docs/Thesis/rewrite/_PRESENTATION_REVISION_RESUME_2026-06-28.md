...
3. [x] Issue 1 -- DONE (commit a8bf95a8). Thinned redundant floor closers (deeper sec-4 pass). Verified by
       `floor_inventory.py` (no section drops any element to zero; CORR 17->12, NOCAUSE 20->15 thesis-wide),
       math-untouched guard + destars PASS (no number moved), and a manual sole-survivor trace + skeptical
       MA1->4.5 flow-read (floor reads honest, NOT under-hedged). No agents (Sina). See _ISSUE1_DEHEDGE_PLAN.md.
4. [ ] FINAL (remaining): (a) optional 3-agent coherence panel on the fresh FLAT -- Sina said "no agents"
       for Issue 1, so confirm whether he wants the panel; (b) set canonical JOB name = "thesis_draft_uottawa"
       in build_uottawa_rewrite.py and rebuild, once Sina closes all PDF viewers holding rev2.pdf open.
