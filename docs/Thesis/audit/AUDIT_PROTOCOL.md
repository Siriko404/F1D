...
2. **The paper title is the anchor — never the upload filename.** Sources are identified and referred to by the paper's **exact title** (from the bibitem), never by the NLM upload filename (`ssrn-4900453.pdf`, `1-s2.0-…`, `w22740.pdf`). Filenames are opaque and untrustworthy as identifiers. Source-id discovery is itself a **scripted, title-anchored** NLM step: ask the notebook to locate each known bibitem title and read the `source_id` back from NLM's own citation. Identification is the *one* unscoped query (you cannot `-s` a source you have not yet identified); **every** subsequent attribution query is single-source scoped (E3 #2, E4). The notebook is large (94 sources, mostly other projects) — scoping by the confirmed `source_id` is what prevents cross-project contamination.
