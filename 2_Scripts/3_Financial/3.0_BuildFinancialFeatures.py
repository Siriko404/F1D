...
def analyze_missing_values(df):
    ...
    # Handle duplicate column names by getting unique columns
    unique_cols = []
    seen = set()
...
def print_stats_summary(stats):
    ...
    if stats["processing"]:
        print(f"\n{'Processing Step':<30} {'Value':>10}")
        print("-" * 42)
        for step, value in stats["processing"].items():
            if isinstance(value, (int, float)):
                print(f"{step:<30} {value:>10,}")
            elif isinstance(value, dict):
                # For nested dicts, print the first key-value pair
                first_key = list(value.keys())[0]
                first_value = value[first_key]
                print(f"{step:<30} {first_key}: {first_value:,}")
            else:
                print(f"{step:<30} {value}")

    # Merge diagnostics if available
    if "merges" in stats:
        print(f"\n{'Merge Diagnostics':<30} {'Records':>10}")
        print("-" * 42)
        for merge_name, merge_data in stats["merges"].items():
            print(f"  {merge_name}:")
            print(f"    Matched: {merge_data['matched']:,}")
            print(f"    Left unmatched: {merge_data['unmatched_left']:,}")
            print(f"    Right unmatched: {merge_data['unmatched_right']:,}")
