...
def load_all_data(root):
    ...
    manifest = pd.read_parquet(manifest_path, columns=[
        'file_name', 'gvkey', 'start_date', 'ceo_id', 'ff12_code', 'ff48_code'
    ])
    ...
    for year in range(CONFIG['year_start'], CONFIG['year_end'] + 1):
        lv_path = root / '4_Outputs' / '2_Textual_Analysis' / '2.2_Variables' / 'latest' / f'linguistic_variables_{year}.parquet'
        if lv_path.exists():
    ...
    all_fc = []
    for year in range(CONFIG['year_start'], CONFIG['year_end'] + 1):
        fc_path = root / '4_Outputs' / '3_Financial_Features' / 'latest' / f'firm_controls_{year}.parquet'
        if fc_path.exists():
    ...
    fc_cols.append('shift_intensity_sale_ff48') if 'shift_intensity_sale_ff48' in firm.columns else None
    ...
    all_mv = []
    for year in range(CONFIG['year_start'], CONFIG['year_end'] + 1):
        mv_path = root / '4_Outputs' / '3_Financial_Features' / 'latest' / f'market_variables_{year}.parquet'
        if mv_path.exists():
            mv = pd.read_parquet(mv_path)
            all_mv.append(mv)
    market = pd.concat(all_mv, ignore_index=True)
    mv_cols = ['file_name', 'Delta_Amihud', 'Delta_Corwin_Schultz', 'Volatility', 'StockRet', 'MarketRet', 'Amihud', 'Corwin_Schultz']
    ...
    regime_path = root / '4_Outputs' / '4.1_CeoClarity' / 'latest' / 'ceo_clarity_scores.parquet'
    if regime_path.exists():
        regime_clarity = pd.read_parquet(regime_path, columns=['ceo_id', 'ClarityCEO', 'sample'])
    ...
    ceo_path = root / '4_Outputs' / '4.1.1_CeoClarity_CEO_Only' / 'latest' / 'ceo_clarity_scores.parquet'
    if ceo_path.exists():
        ceo_clarity = pd.read_parquet(ceo_path, columns=['ceo_id', 'ClarityCEO', 'sample'])
    ...
    if len(regime_clarity) > 0:
        ...
        df = df.merge(regime_clarity[['ceo_id', 'ClarityRegime', 'sample']], 
                     left_on=['ceo_id', 'sample_clarity'], 
                     right_on=['ceo_id', 'sample'], 
                     how='left', suffixes=('', '_regime'))
    if len(ceo_clarity) > 0:
        df = df.merge(ceo_clarity[['ceo_id', 'ClarityCEO', 'sample']], 
                     left_on=['ceo_id', 'sample_clarity'], 
                     right_on=['ceo_id', 'sample'], 
                     how='left', suffixes=('', '_ceo'))
...
def run_first_stage(df, out_dir):
    ...
    for endog_var, label in [
        ...
        formula = f"{endog_var} ~ {CONFIG['instrument']} + " + " + ".join(controls) + " + C(year)"
...
def run_ols_regression(df, dep_var, clarity_var, uncertainty_var, pres_unc_var, sample_name, out_file):
    """Run OLS regression (no instrument)."""
    # Define all controls (excluding the endogenous uncertainty_var to avoid duplication)
    other_controls = [clarity_var, pres_unc_var] + CONFIG['linguistic_controls'] + CONFIG['firm_controls']
    other_controls = [c for c in other_controls if c in df.columns and c != uncertainty_var]
...
def run_iv_regression(df, dep_var, clarity_var, uncertainty_var, pres_unc_var, sample_name, out_file):
    ...
    # Prepare data
    controls = [clarity_var, pres_unc_var] + CONFIG['linguistic_controls'] + CONFIG['firm_controls']
    ...
    # Create year dummies
    year_dummies = pd.get_dummies(reg_df['year'].astype(str), prefix='year', drop_first=True).astype(np.float64)
    ...
    try:
        ...
        first_stage = sm.OLS(endog, sm.add_constant(pd.concat([instruments, exog.drop('const', axis=1, errors='ignore')], axis=1))).fit()
...
def main():
    ...
    for sample_name, sample_filter in CONFIG['samples'].items():
        ...
        for dep_var in CONFIG['dep_vars']:
            ...
            res = run_ols_regression(
                df_sample, dep_var, 
                'ClarityRegime', 'Manager_QA_Uncertainty_pct', 'Manager_Pres_Uncertainty_pct',
                sample_name, ols_regime_file
            )
            if res:
                res['Model'] = 'OLS'
                res['Type'] = 'Regime'
            ...
            res = run_ols_regression(
                df_sample, dep_var,
                'ClarityCEO', 'CEO_QA_Uncertainty_pct', 'CEO_Pres_Uncertainty_pct',
                sample_name, ols_ceo_file
            )
            if res:
                res['Model'] = 'OLS'
                res['Type'] = 'CEO'
    ...
    for fs in first_stage_results:
        report_lines.append(f"- {fs['Endogenous']}: F={fs['F_stat']:.2f} ({'Strong' if fs['Strong'] else 'Weak'})")
    ...
    if all_results:
        ...
        report_lines.append(results_df[['Model', 'Type', 'Sample', 'DepVar', 'N', 'R2']].to_markdown(index=False))
