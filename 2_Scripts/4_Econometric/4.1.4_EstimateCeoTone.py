...
def compute_ceo_stats(df_sample_filtered, ceo_fe, model_spec, sample_name):
    ...
    ceo_stats.columns = ['ceo_id', 'n_calls', 'avg_tone', 'std_tone',
    ...
    tone_col = [c for c in ceo_fe.columns if c.startswith('Tone') and c != 'Tone_raw'][0]
    ceo_scores = ceo_scores.sort_values(tone_col, ascending=False).reset_index(drop=True)
...
def save_outputs(all_ceo_scores, all_diagnostics, all_models, out_dir, stats=None):
    ...
    for (model_name, sample_name), model in all_models.items():
        if model is not None:
            results_path = out_dir / f'regression_results_{model_name.lower()}_{sample_name.lower()}.txt'
            with open(results_path, 'w') as f:
                f.write(model.summary().as_text())
            print(f"  Saved: regression_results_{model_name.lower()}_{sample_name.lower()}.txt")
...
def generate_report(all_diagnostics, out_dir, duration):
    ...
    report_lines.append("- **ToneRegime:** CEO fixed effect on non-CEO manager tone (spillover)")
    report_lines.append("")
...
def main(year_start=None, year_end=None):
    ...
    for model_name, model_spec in MODEL_SPECS.items():
        ...
        for sample_name in ['Main', 'Finance', 'Utility']:
            ...
            # Run regression
            model, df_reg, valid_ceos = run_regression(df_sample, model_name, model_spec, sample_name)
            ...
            # Compute CEO stats
            ceo_scores = compute_ceo_stats(df_sample_filtered, ceo_fe, model_spec, sample_name)
            all_ceo_scores.append(ceo_scores)
            
            # Compute diagnostics
            diag = compute_diagnostics(model, model_name, sample_name,
                                        len(valid_ceos),
