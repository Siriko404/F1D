...
def load_all_data(root, year_start, year_end):
    ...
    for year in range(year_start, year_end + 1):
        ...
        if len(mv) > 0:
            ...
            mv_cols = ['file_name'] + [c for c in mv.columns if c in all_mkt_cols and c not in existing]
...
def generate_report(all_diagnostics, out_dir, duration):
    ...
    report_lines.append("")
    report_lines.append("If R-squared increases only marginally when adding extended controls,")
    report_lines.append("this suggests the CEO fixed effects are robust and not confounded by")
...
def main(year_start=None, year_end=None):
    ...
    for model_name, model_config in MODELS.items():
        ...
        for sample_name in ['Main', 'Finance', 'Utility']:
            ...
            # Compute diagnostics
            n_controls = len([c for c in model_config['linguistic_controls'] + model_config['firm_controls'] 
                             if c in df_reg.columns])
            diag = compute_diagnostics(
                model, model_name, sample_name,
                len(valid_ceos), 
