...
def load_all_data(root, year_start, year_end):
    ...
    for year in range(year_start, year_end + 1):
        ...
        # Merge firm controls
        if len(fc) > 0:
            fc_cols = ['file_name'] + [c for c in fc.columns if c in CONFIG['firm_controls']]
            merged = merged.merge(fc[fc_cols], on='file_name', how='left')
        
        # Merge market variables
        if len(mv) > 0:
            mv_cols = ['file_name'] + [c for c in mv.columns if c in CONFIG['firm_controls']]
...
def extract_ceo_fixed_effects(model, df_reg, sample_name):
    ...
    print(f"  Range: [{ceo_fe['ClarityCEO'].min():.4f}, {ceo_fe['ClarityCEO'].max():.4f}]")
...
def compute_ceo_stats(df_sample_filtered, ceo_fe, sample_name):
    ...
    ceo_stats = df_sample_filtered.groupby('ceo_id').agg({
    ...
    ceo_stats.columns = ['ceo_id', 'n_calls', 'avg_uncertainty', 'std_uncertainty',
...
def save_outputs(all_ceo_scores, all_diagnostics, all_models, out_dir):
    ...
    var_ref = pd.DataFrame([
        ...
        {'Variable': 'sample', 'Source': 'Calculated', 'Description': 'Industry sample (Main/Finance/Utility)'},
        {'Variable': 'gamma_i', 'Source': 'Calculated', 'Description': 'CEO fixed effect coefficient'},
        {'Variable': 'ClarityCEO_raw', 'Source': 'Calculated', 'Description': '-gamma_i'},
        {'Variable': 'ClarityCEO', 'Source': 'Calculated', 'Description': 'Standardized clarity score'},
        {'Variable': 'n_calls', 'Source': 'Calculated', 'Description': 'Number of calls per CEO'},
        {'Variable': 'avg_uncertainty', 'Source': 'Calculated', 'Description': 'Mean Manager_QA_Uncertainty_pct_mean'},
        {'Variable': 'std_uncertainty', 'Source': 'Calculated', 'Description': 'Std of Manager_QA_Uncertainty_pct_mean'},
        {'Variable': 'ceo_name', 'Source': 'Step 1', 'Description': 'CEO full name'},
        {'Variable': 'first_call_date', 'Source': 'Calculated', 'Description': 'Min(start_date) per CEO'},
        {'Variable': 'last_call_date', 'Source': 'Calculated', 'Description': 'Max(start_date) per CEO'},
        {'Variable': 'n_firms', 'Source': 'Calculated', 'Description': 'nunique(gvkey) per CEO'}
    ])
...
def main(year_start=None, year_end=None):
    ...
    for sample_name in ['Main', 'Finance', 'Utility']:
        ...
        # Compute diagnostics
        diag = compute_diagnostics(model, sample_name, 
                                    len(valid_ceos), 
        ...
        for i, row in ceo_scores.head(3).iterrows():
            ceo_name = row['ceo_name'] if pd.notna(row['ceo_name']) else 'Unknown'
            print(f"    {i+1}. {ceo_name}: Clarity={row['ClarityCEO']:.3f}, n={int(row['n_calls'])}")
        
        print(f"\n  Top 3 Most Uncertain CEOs ({sample_name}):")
        for i, row in ceo_scores.tail(3).iterrows():
            ceo_name = row['ceo_name'] if pd.notna(row['ceo_name']) else 'Unknown'
            print(f"    {ceo_scores.index.get_loc(i)+1}. {ceo_name}: Clarity={row['ClarityCEO']:.3f}, n={int(row['n_calls'])}")
