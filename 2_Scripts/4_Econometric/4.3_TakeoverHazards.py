...
def load_data():
    ...
    if ling_files:
        ...
        df = df.merge(ling.drop_duplicates('file_name'), on='file_name', how='left', suffixes=('', '_ling'))
        print(f"  Linguistic: {ling['Manager_QA_Uncertainty_pct'].notna().sum():,} with Q&A Unc")
    ...
    if regime_path.exists():
        ...
        df = df.merge(regime[['ceo_id', 'sample', 'ClarityCEO']].rename(columns={'ClarityCEO': 'ClarityRegime'}),
                      left_on=['ceo_id', 'sample_for_clarity'], right_on=['ceo_id', 'sample'], how='left')
    ...
    if sdc_path.exists():
        ...
        sdc = sdc[(sdc['announce_date'] >= '2002-01-01') & (sdc['announce_date'] <= '2018-12-31')]
        ...
        sdc['target_ticker'] = sdc['Target Primary Ticker Symbol'].astype(str).str.upper().str.strip()
        sdc['target_name_clean'] = sdc['Target Full Name'].astype(str).str.upper().str.strip()
        sdc['target_name_clean'] = sdc['target_name_clean'].str.replace(r'[^\w\s]', '', regex=True)
            ...
            if t and t != 'NAN':
                for check_year in [y-1, y, y+1]:
                    if t in ticker_by_year.get(check_year, set()):
                        ...
                        matches = df[(df['ticker_clean'] == t) & (df['call_year'].between(y-1, y+1))]
                        if len(matches) > 0:
        ...
        for cusip in matched_cusips:
            ...
            for idx in firm_calls_idx:
                ...
                window_end = call_date + pd.Timedelta(days=CONFIG['forward_window_days'])
...
def main():
    ...
    covariates_regime = [CONFIG['clarity_var_regime'], CONFIG['uncertainty_var']] + CONFIG['firm_controls']
    covariates_regime = [c for c in covariates_regime if c in df_main.columns]
    
    # Define covariates for CEO model
    covariates_ceo = [CONFIG['clarity_var_ceo'], CONFIG['uncertainty_var']] + CONFIG['firm_controls']
    ...
    # Regime Model
    cph_all_regime = run_cox_ph(df_main, 'Takeover', covariates_regime, 
                                "All Takeovers (Regime Clarity)", out_file)
    if cph_all_regime:
        for var in [CONFIG['clarity_var_regime'], CONFIG['uncertainty_var']]:
            if var in cph_all_regime.summary.index:
                results.append({
                    'Model': 'CPH_All', 'Type': 'Regime', 'Variable': var,
    ...
    # CEO Model
    cph_all_ceo = run_cox_ph(df_main, 'Takeover', covariates_ceo,
                             "All Takeovers (CEO Clarity)", out_file)
    if cph_all_ceo:
        for var in [CONFIG['clarity_var_ceo'], CONFIG['uncertainty_var']]:
            if var in cph_all_ceo.summary.index:
                results.append({
                    'Model': 'CPH_All', 'Type': 'CEO', 'Variable': var,
    ...
    fg_uninv_regime = run_fine_gray(df_main, 'Uninvited', covariates_regime,
                                     "Uninvited (Regime Clarity)", out_file_uninv)
    if fg_uninv_regime:
        for var in [CONFIG['clarity_var_regime'], CONFIG['uncertainty_var']]:
            if var in fg_uninv_regime.summary.index:
                results.append({
                    'Model': 'FG_Uninvited', 'Type': 'Regime', 'Variable': var,
    ...
    fg_uninv_ceo = run_fine_gray(df_main, 'Uninvited', covariates_ceo,
                                  "Uninvited (CEO Clarity)", out_file_uninv)
    if fg_uninv_ceo:
        for var in [CONFIG['clarity_var_ceo'], CONFIG['uncertainty_var']]:
            if var in fg_uninv_ceo.summary.index:
                results.append({
                    'Model': 'FG_Uninvited', 'Type': 'CEO', 'Variable': var,
    ...
    fg_friend_regime = run_fine_gray(df_main, 'Friendly', covariates_regime,
                                      "Friendly (Regime Clarity)", out_file_friend)
    if fg_friend_regime:
        for var in [CONFIG['clarity_var_regime'], CONFIG['uncertainty_var']]:
            if var in fg_friend_regime.summary.index:
                results.append({
                    'Model': 'FG_Friendly', 'Type': 'Regime', 'Variable': var,
    ...
    fg_friend_ceo = run_fine_gray(df_main, 'Friendly', covariates_ceo,
                                   "Friendly (CEO Clarity)", out_file_friend)
    if fg_friend_ceo:
        for var in [CONFIG['clarity_var_ceo'], CONFIG['uncertainty_var']]:
            if var in fg_friend_ceo.summary.index:
                results.append({
                    'Model': 'FG_Friendly', 'Type': 'CEO', 'Variable': var,
