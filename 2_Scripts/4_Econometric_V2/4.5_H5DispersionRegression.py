...
from shared.diagnostics import check_multicollinearity, compute_vif, MulticollinearityError
...
def prepare_regression_data(df, uncertainty_cols, dw=None):
    ...
    control_cols = [
        'prior_dispersion', 'earnings_surprise', 'analyst_coverage',
        'loss_dummy', 'firm_size', 'leverage', 'earnings_volatility', 'tobins_q'
    ]
...
def run_single_h5_regression(df, uncertainty_var, model_type, spec_name, spec_config,
    ...
    if len(exog) < 2:
        if dw:
            dw.write(f"  ERROR: Insufficient variables for regression\n")
        return None
    ...
    if len(continuous_controls) >= 2:
        try:
            vif_result = check_multicollinearity(
                df_reg, continuous_controls, vif_threshold=vif_threshold,
    ...
    # Run panel OLS
    cluster_cols = ['gvkey', 'fiscal_year'] if spec_config.get('double_cluster') else ['gvkey']
...
def run_all_h5_regressions(reg_df, uncertainty_measures, specs, control_vars,
    ...
    if dw:
        dw.write(f"\n=== Primary Model (H5-A) ===\n")
        dw.write(f"Testing whether hedging predicts dispersion beyond uncertainty\n")
    ...
    for uncertainty_var in uncertainty_measures:
        ...
        for spec_name, spec_config in specs.items():
            ...
            result = run_single_h5_regression(
                reg_df, uncertainty_var, model_type, spec_name, spec_config,
            ...
            if result:
                ...
                if dw:
                    ...
                    if result['control_var']:
                        dw.write(f", beta2({result['control_var']})={result['beta2']:.4f} (sig={result['beta2_signif']})\n")
                    else:
                        dw.write(f"\n")
    ...
    if dw:
        dw.write(f"\n=== Gap Model (H5-B) ===\n")
        dw.write(f"Testing whether spontaneous-scripted gap predicts dispersion\n")
    ...
    for spec_name, spec_config in specs.items():
        ...
        result = run_single_h5_regression(
            reg_df, 'uncertainty_gap', 'gap', spec_name, spec_config,
            control_vars, vif_threshold, dw
        )
...
def generate_results_markdown(results, output_dir, dw=None):
    ...
    lines.append("")
    lines.append("- **H5-A:** Hedging language (weak modal verbs) predicts higher analyst forecast dispersion,")
    lines.append("          even after controlling for general uncertainty words (beta1 > 0)")
    lines.append("- **H5-B:** Spontaneous-scripted uncertainty gap predicts dispersion (beta1 > 0)")
    lines.append("")
    ...
    for r in results:
        if r['spec'] == 'primary' and r['model_type'] in ['primary', 'solo']:
            ...
            control_note = f" (ctrl: {r['control_var']})" if r.get('control_var') else ""

            lines.append(f"| {uncertainty}{control_note} | {n:,} | {r2:.4f} | "
                        f"{beta1:.4f} ({beta1_se:.4f}) | {p1:.4f} | {h5a} |")

    lines.append("")
    lines.append("*Significance: p < 0.05 (one-tailed). `ctrl` indicates control variable included.*")
    lines.append("")
    ...
    primary_weak_modal = [r for r in results if r['spec'] == 'primary'
                         and 'Weak_Modal' in r['uncertainty_var']
                         and r['beta1_signif']]
    primary_uncertainty = [r for r in results if r['spec'] == 'primary'
                          and 'Uncertainty_pct' in r['uncertainty_var']
                          and 'Weak_Modal' not in r['uncertainty_var']
                          and r['beta1_signif']]

    lines.append(f"**Primary Specification (Firm + Year FE):**")
    lines.append(f"- Weak Modal measures: {len(primary_weak_modal)}/3 significant")
    lines.append(f"- Uncertainty measures: {len(primary_uncertainty)}/2 significant (QA only)")
    lines.append("")

    # List significant measures
    if primary_weak_modal:
        lines.append(f"**Supporting H5-A (Weak Modal sig):**")
        for r in primary_weak_modal:
            control_str = f" (controlling for {r['control_var']})" if r.get('control_var') else ""
            lines.append(f"- {r['uncertainty_var']}: beta1={r['beta1']:.4f}, p={r['beta1_p_one']:.4f}{control_str}")
    else:
    ...
    weak_modal_ctrl = [r for r in results if r['spec'] == 'primary'
    ...
    if weak_modal_ctrl:
        ...
        lines.append(f"|---|---|")
        lines.append(f"| Weak Modal (beta1) | {r['beta1']:.4f} (p={r['beta1_p_one']:.4f}) |")
        lines.append(f"| Uncertainty control (beta2) | {r['beta2']:.4f} (sig={beta1_sig}) |")
        lines.append(f"| Weak Modal sig after controlling | **{sig}** |")
        lines.append("")

        if r['beta1_signif']:
            lines.append("**Conclusion:** Hedging language ADDS incremental predictive power beyond uncertainty.")
        elif r.get('beta2_signif'):
            lines.append("**Conclusion:** Uncertainty predicts dispersion, but hedging does NOT add incremental power.")
        else:
            lines.append("**Conclusion:** Neither hedging nor uncertainty significantly predict dispersion in this specification.")
    else:
        lines.append("**No incremental contribution test available (Weak_Modal with Uncertainty control not found)**")

    lines.append("")

    # Gap model results
    gap_results = [r for r in results if r['model_type'] == 'gap' and r['spec'] == 'primary']
    if gap_results:
        ...
        lines.append(f"| Metric | Result |")
        lines.append(f"|---|---|")
        lines.append(f"| Uncertainty Gap (beta1) | {r['beta1']:.4f} (p={r['beta1_p_one']:.4f}) |")
        lines.append(f"| Gap significant | **{sig}** |")
        lines.append("")

        if r['beta1_signif']:
            if r['beta1'] > 0:
                lines.append("**Interpretation:** Spontaneous (Q&A) uncertainty HIGHER than scripted (Pres)")
                lines.append("reveals hidden information that increases analyst disagreement.")
            else:
                lines.append("**Interpretation:** Gap effect is significant but negative.")
        else:
            lines.append("**Interpretation:** Gap does not significantly predict dispersion.")
    else:
        lines.append("## Gap Model Results (H5-B)")
        lines.append("")
    ...
    lines.append("")
    lines.append("Robustness variants: Without lagged DV, without NUMEST, CEO-only measures")
    lines.append("")
...
def main():
    ...
    try:
        ...
        dw.write(f"  {len(UNCERTAINTY_MEASURES)} uncertainty measures x {len(SPECS)} specifications + gap model\n")
        dw.write(f"  Estimated {len(UNCERTAINTY_MEASURES) * len(SPECS) + len(SPECS)} regressions\n")

        results = run_all_h5_regressions(
            reg_df, UNCERTAINTY_MEASURES, SPECS, available_controls,
        ...
        weak_modal_sig = any(r['beta1_signif'] for r in results
                            if 'Weak_Modal' in r['uncertainty_var'] and r['spec'] == 'primary')
        gap_sig = any(r['beta1_signif'] for r in results
        ...
        if weak_modal_ctrl:
            r = weak_modal_ctrl[0]
            if r['beta1_signif']:
                stats['hypothesis_summary']['interpretation'] = "Hedging adds incremental power beyond uncertainty"
            elif r.get('beta2_signif'):
                stats['hypothesis_summary']['interpretation'] = "Uncertainty predicts dispersion, hedging does not add power"
            else:
                stats['hypothesis_summary']['interpretation'] = "Neither hedging nor uncertainty significant"
        else:
            stats['hypothesis_summary']['interpretation'] = "Unable to determine (test not run)"
