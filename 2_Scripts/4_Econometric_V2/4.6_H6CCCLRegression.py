...
SPECS = {
    ...
    'double_cluster': {'entity_effects': True, 'time_effects': True, 'cluster_cols': ['gvkey', 'fiscal_year']},
}
...
def load_h6_variables(h6_dir, dw=None):
    ...
    if dw:
        ...
        dw.write(f"    Year range: {df['fiscal_year'].min()}-{df['fiscal_year'].max()}\n")
...
def run_single_h6_regression(df, uncertainty_var, cccl_var, spec_name, spec_config,
    ...
    if len(exog) >= 2:
        try:
            vif_result = check_multicollinearity(
                df_reg, exog, vif_threshold=vif_threshold,
    ...
    beta_cccl = coeffs_df.loc[cccl_var, 'Coefficient'] if cccl_var in coeffs_df.index else np.nan
    se_cccl = coeffs_df.loc[cccl_var, 'Std. Error'] if cccl_var in coeffs_df.index else np.nan
    t_cccl = coeffs_df.loc[cccl_var, 't-stat'] if cccl_var in coeffs_df.index else np.nan
...
def run_primary_regressions(reg_df, uncertainty_measures, cccl_var, specs,
                           vif_threshold=5.0, dw=None):
    """
    ...
    if dw:
        ...
        dw.write(f"Testing whether CCCL reduces speech uncertainty\n")
    ...
    for uncertainty_var in uncertainty_measures:
        ...
        for spec_name, spec_config in specs.items():
            ...
            result = run_single_h6_regression(
                reg_df, uncertainty_var, cccl_var, spec_name, spec_config,
                vif_threshold, dw
            )
            ...
            if result:
                ...
                if dw:
                    ...
                    dw.write(f"beta_CCCL={result['beta_cccl']:.4f} (p1={result['p_value_one_tail']:.4f})\n")
...
def run_mechanism_test(reg_df, cccl_var, vif_threshold=5.0, dw=None):
    ...
    if dw:
        dw.write(f"\n=== Mechanism Test (H6-B) ===\n")
        dw.write(f"Comparing QA vs Presentation effects\n")
    ...
    h6b_supported = len(comparison) > 0 and sum(c['qa_larger'] for c in comparison) > len(comparison) / 2
...
def run_gap_analysis(reg_df, cccl_var, vif_threshold=5.0, dw=None):
    ...
    if dw:
        dw.write(f"\n=== Gap Analysis (H6-C) ===\n")
        dw.write(f"Testing whether CCCL reduces Q&A-Presentation uncertainty gap\n")

    if 'uncertainty_gap' not in reg_df.columns:
        if dw:
            dw.write(f"  WARNING: uncertainty_gap not found\n")
        return None

    result = run_single_h6_regression(
        reg_df, 'uncertainty_gap', cccl_var, 'primary',
        {'entity_effects': True, 'time_effects': True, 'cluster_cols': None},
        vif_threshold, dw=dw
    )
...
def run_instrument_robustness(reg_df, uncertainty_var, cccl_variants, specs,
    ...
    if dw:
        ...
        dw.write(f"Testing alternative CCCL instruments\n")
...
def apply_fdr_correction(results, alpha=0.05, dw=None):
    ...
    # Extract p-values
    p_values = [r['p_value_one_tail'] for r in primary_results if not np.isnan(r['p_value_one_tail'])]
...
def generate_results_markdown(results, fdr_map, pre_trends, mechanism_comp,
                              gap_result, output_dir, dw=None):
    ...
    lines.append("")
    lines.append("- **H6-A (Primary):** SEC scrutiny (CCCL exposure) reduces speech uncertainty (beta_CCCL < 0)")
    lines.append("- **H6-B (Mechanism):** CCCL effect is stronger in spontaneous Q&A than prepared Presentation")
    ...
    for r in results:
        if r['spec'] == 'primary' and 'robustness' not in r['spec']:
            ...
            lines.append(f"| {uncertainty} | {n:,} | {n_firms} | {n_years} | {r2:.4f} | "
                        f"{beta:.4f} ({se:.4f}) | {p1:.4f} | {fdr_q:.4f}{fdr_sig} | {h6a} |")

    lines.append("")
    lines.append("*Significance: p < 0.05 (one-tailed). FDR q is Benjamini-Hochberg corrected p-value. * indicates FDR-significant at q=0.05.*")
    lines.append("")
    ...
    if fdr_map:
        ...
        lines.append(f"Benjamini-Hochberg FDR correction across {len(fdr_map)} primary tests:")
        lines.append("")
        ...
        for measure, vals in fdr_map.items():
            ...
            lines.append(f"| {measure} | {vals['p_original']:.4f} | {vals['p_fdr']:.4f} | {sig_str} |")
        lines.append("")
    ...
    if fdr_sig:
        lines.append(f"**FDR-significant measures:**")
        for r in fdr_sig:
            lines.append(f"- {r['uncertainty_var']}: beta_CCCL={r['beta_cccl']:.4f}, p_FDR={r['fdr_p_value']:.4f}")
    elif primary_sig:
        lines.append(f"**Significant before FDR (not after):**")
        for r in primary_sig:
            lines.append(f"- {r['uncertainty_var']}: beta_CCCL={r['beta_cccl']:.4f}, p={r['p_value_one_tail']:.4f}")
    else:
    ...
    if mechanism_comp:
        ...
        lines.append("")
        lines.append("Tests whether CCCL effect is stronger in spontaneous Q&A than prepared Presentation.")
        lines.append("")

        lines.append("| QA Measure | QA beta | Pres Measure | Pres beta | |QA| > |Pres| |")
        lines.append("|---|---|---|---|---|")
        ...
        for comp in mechanism_comp:
            ...
            if comp['qa_larger']:
                qa_larger_count += 1
            lines.append(f"| {comp['qa_measure']} | {comp['qa_beta']:.4f} | "
                        f"{comp['pres_measure']} | {comp['pres_beta']:.4f} | {qa_larger_str} |")

        lines.append("")
        h6b_supported = qa_larger_count > len(mechanism_comp) / 2
        if h6b_supported:
            lines.append(f"**H6-B: SUPPORTED** - {qa_larger_count}/{len(mechanism_comp)} QA effects larger than Pres.")
        else:
            lines.append(f"**H6-B: NOT SUPPORTED** - Only {qa_larger_count}/{len(mechanism_comp)} QA effects larger.")
        lines.append("")
    ...
    if gap_result:
        ...
        lines.append(f"|---|---|")
        lines.append(f"| beta_CCCL | {gap_result['beta_cccl']:.4f} (SE: {gap_result.get('se_cccl', 0):.4f}) |")
        lines.append(f"| p-value (one-tail) | {gap_result['p_value_one_tail']:.4f} |")
        lines.append(f"| H6-C supported | {'Yes' if gap_result.get('h6c_supported') else 'No'} |")
        lines.append("")

        if gap_result.get('h6c_supported'):
            lines.append("**Interpretation:** CCCL significantly reduces the uncertainty gap, suggesting")
...
def main():
    ...
    try:
        ...
        dw.write(f"  {len(available_measures)} uncertainty measures x {len(SPECS)} specifications\n")
        dw.write(f"  Estimated {len(available_measures) * len(SPECS)} regressions\n")

        all_results = []
        primary_results = run_primary_regressions(
            reg_df, available_measures, primary_cccl, SPECS,
        ...
        stats['hypothesis_summary'] = {
            ...
            'h6b_supported': h6b_supported,
            'h6c_supported': gap_result.get('h6c_supported', False) if gap_result else False,
            'pre_trends_pass': pre_trends.get('pre_trends_pass', False) if pre_trends else None,
        }
        ...
        generate_results_markdown(
            all_results, fdr_map, pre_trends, mechanism_comp,
            gap_result, paths["output_dir"], dw
        )
        ...
        dw.write(f"\n  Hypothesis Summary:\n")
        dw.write(f"    H6-A (FDR sig): {stats['hypothesis_summary']['h6a_fdr_significant']}/{stats['hypothesis_summary']['h6a_primary_measures']}\n")
        dw.write(f"    H6-B (QA > Pres): {stats['hypothesis_summary']['h6b_supported']}\n")
        dw.write(f"    H6-C (Gap): {stats['hypothesis_summary']['h6c_supported']}\n")
        if pre_trends:
            dw.write(f"    Pre-trends: {'PASS' if stats['hypothesis_summary']['pre_trends_pass'] else 'FAIL'}\n")
        dw.write("=" * 80 + "\n")
        dw.write("COMPLETE\n")
