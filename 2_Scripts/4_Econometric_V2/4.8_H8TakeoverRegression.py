...
def run_h8_logit(df, uncertainty_var, dv_col='takeover_fwd',
                 controls=None, include_fe=True, spec='primary'):
    """
...
def create_takeover_timing_variants(df, dv_base='takeover_completed', lags=[0, -1, -2]):
    ...
    dv_col_map = {
        'takeover_completed': 'takeover_fwd',
        'takeover_fwd': 'takeover_fwd'
    }
    ...
    for lag in lags:
        ...
        if lag == 0:
            ...
            if actual_dv_base == 'takeover_fwd':
                df_work['takeover_timing'] = df_work.groupby('gvkey')[actual_dv_base].shift(1)
            else:
            ...
            df_work['takeover_timing'] = df_work.groupby('gvkey')[actual_dv_base].shift(-abs(lag) + 1)
...
def run_h8_cox_ph(df, uncertainty_var, dv_col='time_to_takeover', controls=None):
    ...
    try:
        ...
        return {
            ...
            'p_one_sided': summary['p'] / 2 if summary['coef'] > 0 else 1 - summary['p'] / 2,
        ...
        return {
            ...
            'n_firms': survival_df['gvkey'].nunique() if 'survival_df' in locals() else 0,
            'n_takeovers': 0,
            'converged': False,
            'error': str(e)
        }
    ...
    for dv_col, dv_label in H8_ROBUSTNESS_CONFIG['alternative_dvs'].items():
        ...
        for uv in uncertainty_measures:
            ...
            if result:
                ...
                if result['converged']:
                    print(f"    {uv}: p={result['p_one_sided']:.4f}, OR={result['odds_ratio']:.2f}")
                else:
        ...
        for uv in uncertainty_measures:
            ...
            if cox_result:
                ...
                if cox_result['converged']:
                    print(f"  {uv}: HR={cox_result['hazard_ratio']:.2f}, p={cox_result['p_one_sided']:.4f}")
                elif cox_result.get('error'):
                    print(f"  {uv}: FAILED - {cox_result['error']}")
                else:
                    print(f"  {uv}: Skipped (lifelines unavailable or insufficient data)")
...
def save_regression_results(results_list, output_dir):
    ...
    col_order = [
        ...
        'hazard_ratio', 'ci_lower', 'ci_upper',  # Cox PH columns
        'dv_type', 'iv_type', 'timing', 'model_type', 'robustness_dim',  # Robustness columns
...
def generate_h8_results_report(results_list, sample_stats, timestamp):
    ...
    # Executive summary
    primary_results = [r for r in results_list if r.get('spec') == 'primary' and r.get('converged', False) and r.get('robustness_dim') is None]
    ...
    robustness_results = [r for r in results_list if r.get('robustness_dim') is not None]
    n_rob = len(robustness_results)
    n_rob_sig = sum(1 for r in robustness_results if r.get('p_one_sided', 1) < 0.05 and r.get('converged', False))
    ...
    pooled_results = [r for r in results_list if r.get('spec') == 'pooled' and r.get('converged', False)]
    if pooled_results:
    ...
    if robustness_results:
        ...
        # Alternative DVs
        alt_dv_results = [r for r in robustness_results if r.get('robustness_dim') == 'alt_dv']
        if alt_dv_results:
            ...
            n_sig = sum(1 for r in alt_dv_results if r.get('p_one_sided', 1) < 0.05 and r.get('converged', False))
            report += f"\n**Summary:** {n_sig}/{len(alt_dv_results)} significant\n"

        # Alternative IVs
        alt_iv_results = [r for r in robustness_results if r.get('robustness_dim') == 'alt_iv']
        if alt_iv_results:
            ...
            for iv_type in ['ceo_only', 'presentation_only', 'qa_only']:
                ...
                if iv_results:
                    n_sig = sum(1 for r in iv_results if r.get('p_one_sided', 1) < 0.05 and r.get('converged', False))
        ...
        # Timing tests
        timing_results = [r for r in robustness_results if r.get('robustness_dim') == 'timing']
        if timing_results:
            report += "\n### Timing Tests\n\n"
            report += "| Timing | UV | Beta | Odds Ratio | p (one-tailed) | Significant? |\n"
            report += "|--------|----|------|------------|----------------|--------------|\n"
            ...
            for timing in ['concurrent', 'forward', 'lead']:
                ...
                if timing_tests:
                    n_sig = sum(1 for r in timing_tests if r.get('p_one_sided', 1) < 0.05 and r.get('converged', False))
                    report += f"- **{timing}**: {n_sig}/{len(timing_tests)} significant\n"
        ...
        if cox_results:
            ...
            report += "| UV | Hazard Ratio | 95% CI HR | p (one-tailed) | Significant? |\n"
            report += "|----|--------------|-----------|----------------|--------------|\n"
...
def main():
    ...
    stats["processing"]["n_robustness_converged"] = sum(1 for r in robustness_results if r.get('converged', False))
    stats["processing"]["n_robustness_sig"] = sum(
        1 for r in robustness_results
    ...
    print(f"  Significant (p < 0.05, one-tailed): {stats['processing']['n_robustness_sig']}")
    ...
    print(f"  Primary regressions: {stats['processing']['n_converged']}/{stats['processing']['n_regressions']} converged")
    print(f"  Primary significant (p < 0.05): {stats['processing']['n_significant']}")
    print(f"  Primary after FDR: {n_fdr_sig}")
    print(f"  Robustness tests: {stats['processing']['n_robustness_converged']}/{stats['processing']['n_robustness']} converged")
    print(f"  Robustness significant (p < 0.05): {stats['processing']['n_robustness_sig']}")
