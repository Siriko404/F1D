...
TABLE_IA1_SPECS = {
    ...
    7: {
        ...
        "formula_vars": ["CEO_Pres_Uncertainty_pct", "Analyst_QA_Uncertainty_pct", "delta_UncPreCEO"],
...
def load_data(root, args):
    ...
    for year in years:
        ...
        if fc_path.exists():
            fc = pd.read_parquet(fc_path, columns=[
                "file_name", "gvkey", "SurpDec", "EPS_Growth"
            ])
        else:
        ...
        if mv_path.exists():
            mv = pd.read_parquet(mv_path, columns=[
                "file_name", "gvkey", "StockRet", "MarketRet"
            ])
        else:
...
def prepare_regression_data(df, period_name, min_calls):
    ...
    print(f"    {period_name.capitalize()} period ({year_start}-{year_end}): {len(df_period):,} calls")

    # Check required columns exist
    required_cols = [
        "ceo_id", "ceo_name", "year",
        "CEO_QA_Uncertainty_pct",  # Dependent variable
        "CEO_Pres_Uncertainty_pct",  # UncPreCEO
        "Analyst_QA_Uncertainty_pct",  # UncQue
        "CEO_All_Negative_pct",  # NegCall
        "SurpDec", "EPS_Growth", "StockRet", "MarketRet",  # Firm controls
    ]
    ...
    print(f"    After >=5 calls filter: {len(df_complete):,} calls, {df_complete['ceo_id'].nunique():,} CEOs")
    ...
    df_complete["delta_UncPreCEO"] = df_complete.groupby("ceo_id")["CEO_Pres_Uncertainty_pct"].diff()
...
def extract_ceo_fixed_effects(model, df_reg, sample_period):
    ...
    """
    print(f"    Extracting CEO fixed effects...")
    ...
    ceo_stats = df_reg_numeric.groupby("ceo_id").agg({
        "ceo_name": "first",
        "year": "count"
        ...
        for j in spec_ids:
            if i == j:
                ...
                merged = fe_i[["ceo_id", "ClarityCEO"]].merge(
                    fe_j[["ceo_id", "ClarityCEO"]],
                    on="ceo_id",
                    suffixes=("_i", "_j")
                )
                if len(merged) > 0:
                    corr_matrix.loc[i, j] = merged["ClarityCEO_i"].corr(merged["ClarityCEO_j"])
                else:
...
def create_figure3_histogram(ceo_fe, sample_period, output_path):
    ...
    if not MATPLOTLIB_AVAILABLE:
        print(f"    WARNING: matplotlib not available, skipping histogram")
        return
    ...
    ax.hist(ceo_fe["ClarityCEO_raw"], bins=50, edgecolor='black', alpha=0.7, color='steelblue')
    ...
    ax.set_title(f'Figure 3 Replication: Distribution of CEO Clarity ({sample_period})\n'
                 f'Histogram shows "fatter left tail" - more extremely unclear CEOs', fontsize=14)
...
def generate_report(outputs, sample_stats, output_path):
    ...
    for spec_id, spec_config in TABLE_IA1_SPECS.items():
        lines.append(f"| {spec_id} | {spec_config['name']} | See `table_ia1_correlation_matrix.csv` |")
...
def main():
    ...
    output_dir = ensure_output_dir(root / "4_Outputs" / "4.9_CEOFixedEffects" / timestamp)
    ...
    try:
        ...
        for period_name in periods_to_run:
            ...
            sample_stats[period_name] = {
                ...
                "n_firms": df_period["gvkey"].nunique() if "gvkey" in df_period.columns else "N/A",
            }

            # Run Table 3 Column (1) regression (baseline)
            writer.write("\n  Table 3 Column (1): Baseline")
            model_col1, df_reg_col1 = run_ceo_fe_regression(df_period, specification="col1", cov_type="cluster")
            ceo_fe_col1 = extract_ceo_fixed_effects(model_col1, df_reg_col1, f"{SAMPLE_PERIODS[period_name][0]}-{SAMPLE_PERIODS[period_name][1]}")
            ...
            model_col2, df_reg_col2 = run_ceo_fe_regression(df_period, specification="col2", cov_type="cluster")
            ...
            if MATPLOTLIB_AVAILABLE:
                hist_path = output_dir / f"clarity_distribution_histogram_{SAMPLE_PERIODS[period_name][0]}-{SAMPLE_PERIODS[period_name][1]}.png"
                create_figure3_histogram(ceo_fe_col1, f"{SAMPLE_PERIODS[period_name][0]}-{SAMPLE_PERIODS[period_name][1]}", hist_path)
            ...
            ceo_fe_by_spec, models_by_spec = run_robustness_specs(df_period, baseline_spec=3)
