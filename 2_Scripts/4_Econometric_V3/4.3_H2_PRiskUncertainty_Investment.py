...
"""
==============================================================================
STEP 4.3: H2 PRisk x Uncertainty -> Investment Efficiency Regression
==============================================================================
...
Inputs:
    - 4_Outputs/3_Financial_V3/4.2_H2_PRiskUncertaintyMerge/latest/H2_PRiskUncertainty_Analysis.parquet
      (complete regression dataset from Plan 53-02)

Outputs:
    - 4_Outputs/4_Econometric_V3/4.3_H2_PRiskUncertainty_Investment/{timestamp}/H2_Regression_Results.parquet
      (all regression coefficients, SEs, p-values, diagnostics)
    - 4_Outputs/4_Econometric_V3/4.3_H2_PRiskUncertainty_Investment/{timestamp}/stats.json
      (regression summaries, hypothesis tests, execution metadata)
    - 4_Outputs/4_Econometric_V3/4.3_H2_PRiskUncertainty_Investment/{timestamp}/H2_RESULTS.md
      (human-readable summary of key findings)
    - 3_Logs/4_Econometric_V3/4.3_H2_PRiskUncertainty_Investment/{timestamp}_H2.log
...
ROBUSTNESS_SPECS = {
    ...
    'lagged_iv': {
        'dependent': 'InvestmentResidual',
        'exog': ['PRisk_x_Uncertainty_lag', 'PRisk_std_lag', 'Manager_QA_Uncertainty_pct_std_lag',
                 'CashFlow', 'Size', 'Leverage', 'TobinQ', 'SalesGrowth'],
        'entity_effects': True,
        'time_effects': True,
        'cluster_cols': ['gvkey', 'year'],
...
def setup_paths(config, timestamp):
    ...
    analysis_dir = get_latest_output_dir(
        root / "4_Outputs" / "3_Financial_V3" / "4.2_H2_PRiskUncertaintyMerge",
    ...
    # Output directory
    output_base = root / "4_Outputs" / "4_Econometric_V3" / "4.3_H2_PRiskUncertainty_Investment"
    paths["output_dir"] = output_base / timestamp
    ensure_output_dir(paths["output_dir"])

    # Log directory
    log_base = root / "3_Logs" / "4_Econometric_V3" / "4.3_H2_PRiskUncertainty_Investment"
...
def load_analysis_dataset(analysis_dir, dw=None):
    ...
    if not analysis_file.exists():
        raise FileNotFoundError(f"H2_PRiskUncertainty_Analysis.parquet not found in {analysis_dir}")
...
def prepare_regression_datasets(df, dw=None):
    ...
    # Primary dataset
    primary_cols = ['gvkey', 'year', 'ff48_code', 'InvestmentResidual',
    ...
    regression_vars = ['InvestmentResidual', 'PRisk_x_Uncertainty', 'PRisk_std',
                       'Manager_QA_Uncertainty_pct_std', 'CashFlow', 'Size', 'Leverage',
                       'TobinQ', 'SalesGrowth']

    missing_counts = primary_df[regression_vars].isna().sum()
    if missing_counts.sum() > 0:
        if dw:
            dw.write(f"  Warning: Missing values in regression variables:\n")
    ...
    # Lagged IVs dataset
    lagged_df = df[['gvkey', 'year', 'ff48_code', 'InvestmentResidual'] +
                    ['PRisk_x_Uncertainty', 'PRisk_std', 'Manager_QA_Uncertainty_pct_std',
                     'CashFlow', 'Size', 'Leverage', 'TobinQ', 'SalesGrowth']].copy()
...
def run_single_regression(df, spec_name, spec_config, industry_col='ff48_code',
    ...
    if dw:
        ...
        dw.write(f"      Entity FE: {entity_effects}, Time FE: {time_effects}, Industry FE: {industry_effects}\n")
...
def run_robustness_checks(datasets, vif_threshold=5.0, dw=None):
    ...
    if 'primary' in datasets:
        ...
        result = run_single_regression(
            datasets['primary'], 'industry_fe', ROBUSTNESS_SPECS['industry_fe'],
            vif_threshold=vif_threshold, dw=dw
        )
    ...
    if 'abs_residual' in datasets:
        ...
        result = run_single_regression(
            datasets['abs_residual'], 'abs_residual', ROBUSTNESS_SPECS['abs_residual'],
            vif_threshold=vif_threshold, dw=dw
        )
    ...
    if 'lagged_iv' in datasets:
        ...
        result = run_single_regression(
            datasets['lagged_iv'], 'lagged_iv', ROBUSTNESS_SPECS['lagged_iv'],
            vif_threshold=vif_threshold, dw=dw
        )
    ...
    if 'subsample_2006' in datasets:
        ...
        result = run_single_regression(
            datasets['subsample_2006'], 'subsample_2006', ROBUSTNESS_SPECS['subsample_2006'],
            vif_threshold=vif_threshold, dw=dw
        )
...
def save_regression_results(primary_result, robustness_results, output_dir, dw=None):
    ...
    for var in primary_result['coefficients'].index:
        row = {
            ...
            'variable': var,
            'coefficient': float(primary_result['coefficients'].loc[var, 'Coefficient']),
...
def save_stats_json(primary_result, robustness_results, hypothesis_test,
                    timestamp, git_sha, execution_time, output_dir, dw=None):
...
def generate_results_markdown(primary_result, robustness_results, hypothesis_test,
                               timestamp, output_dir, dw=None):
    """
    ...
    lines.append("")
    lines.append(f"**Result:** {'SUPPORTED' if hypothesis_test['supported'] else 'NOT SUPPORTED'}")
    lines.append("")
    ...
    lines.append("")
    lines.append("Specification: InvestmentResidual ~ PRisk_x_Uncertainty + PRisk_std + Uncertainty_std + Controls + Firm FE + Year FE")
    lines.append("Standard errors: Double-clustered at (firm, year) per Petersen (2009)")
    lines.append("")
    ...
    if not np.isnan(primary_result['r_squared_within']):
        lines.append(f"| R-squared (within) | {primary_result['r_squared_within']:.4f} |")
    ...
    for spec_name, result in robustness_results.items():
        ...
        lines.append(f"| {result['description']} | {n:,} | {r2:.4f} | {b1_str} | {se_str} | {p1_str} | {supp} |")
    ...
    n_supporting = sum(1 for r in robustness_results.values() if r['hypothesis_supported'])
    n_total = len(robustness_results) + 1  # +1 for primary

    if hypothesis_test['supported']:
        lines.append(f"H2 is SUPPORTED in the primary specification ({n_supporting}/{n_total-1} robustness checks also support).")
        lines.append("")
        lines.append("Compound uncertainty from political risk and managerial speech uncertainty")
        lines.append("significantly decreases investment efficiency. Firms facing both high political")
        lines.append("risk AND high managerial uncertainty invest less efficiently than predicted by")
        lines.append("fundamental factors alone.")
    else:
        lines.append(f"H2 is NOT SUPPORTED in the primary specification (0/{n_total-1} robustness checks support).")
        lines.append("")
        lines.append("Compound uncertainty does not have a statistically detectable effect on")
        lines.append("investment efficiency. Political risk and managerial uncertainty may affect")
        lines.append("investment through independent channels rather than multiplicatively.")
...
def main():
    ...
    # Script header
    dw.write("=" * 80 + "\n")
    dw.write("STEP 4.3: H2 PRisk x Uncertainty -> Investment Efficiency Regression\n")
    dw.write("=" * 80 + "\n")
    dw.write(f"Timestamp: {timestamp}\n")
    dw.write(f"Git SHA: {get_git_sha()}\n")
    dw.write(f"Config: {config.get('step_id', '4.3_H2_PRiskUncertainty_Investment')}\n")
    dw.write("")
    ...
    try:
        ...
        primary_result = run_single_regression(
            datasets['primary'], 'primary', PRIMARY_SPEC,
            vif_threshold=5.0, dw=dw
        )

        # Test hypothesis
        hypothesis_test = test_hypothesis(
            primary_result['beta1'],
            primary_result['p_one_tailed'],
            alpha=0.05
        )
        ...
        if args.specifications in ["robustness", "all"]:
            robustness_results = run_robustness_checks(datasets, vif_threshold=5.0, dw=dw)
        ...
        # Save outputs
        dw.write("\n[5] Saving outputs...\n")
        save_regression_results(primary_result, robustness_results, paths["output_dir"], dw)

        stats = save_stats_json(
            primary_result, robustness_results, hypothesis_test,
            timestamp, get_git_sha(), execution_time,
            paths["output_dir"], dw
        )

        generate_results_markdown(
            primary_result, robustness_results, hypothesis_test,
            timestamp, paths["output_dir"], dw
        )
        ...
        dw.write(f"  Memory (RSS): {start_mem['rss_mb']:.1f} MB -> {end_mem['rss_mb']:.1f} MB\n")
        dw.write(f"  Output directory: {paths['output_dir']}\n")
