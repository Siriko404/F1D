"""β^UK proper implementation per paper verbatim:
- vol(r_it) = annualized monthly realized vol from daily returns (%)
- vol(FTSE100) = VFTSE level (annualized %)
- vol(SP500) = VIX level (annualized %)
- vol(FX£$) = annualized monthly realized vol from daily GBP/USD returns (%)
- Window: 2010-01 to 2014-12 (paper page 17 last date Dec 31, 2014)
- Regression per firm: OLS over up to 60 monthly obs."""
...
# 1. VFTSE (annualized % - paper page 17)
...
vftse_eom = vftse.groupby("ym")["VFTSE"].last().reset_index().rename(columns={"VFTSE": "vol_FTSE"})
...
vix_eom = vix.groupby("ym")["Close"].last().reset_index().rename(columns={"Close": "vol_SP500"})

# 3. FX: annualized monthly realized vol from daily GBP/USD returns (%)
...
fx_vol["vol_FX"] = fx_vol["vol_FX"] * ANN * 100  # annualized %

market = vftse_eom.merge(vix_eom, on="ym").merge(fx_vol, on="ym")
print(f"Market panel: {len(market)} months")
print(f"  vol_FTSE (VFTSE, ann%): mean={market['vol_FTSE'].mean():.2f}  sd={market['vol_FTSE'].std():.2f}")
print(f"  vol_SP500 (VIX, ann%): mean={market['vol_SP500'].mean():.2f}  sd={market['vol_SP500'].std():.2f}")
print(f"  vol_FX (ann%): mean={market['vol_FX'].mean():.2f}  sd={market['vol_FX'].std():.2f}")

# 4. Firm vol — annualized monthly realized vol (%)
crsp_frames = []
for year in range(2010, 2015):
    for q in (1, 2, 3, 4):
        f = ROOT / "inputs" / "CRSP_DSF" / f"CRSP_DSF_{year}_Q{q}.parquet"
        if f.exists():
            crsp_frames.append(pd.read_parquet(f, columns=["PERMNO", "date", "RET"]))
...
firm_vol["vol_r"] = firm_vol["vol_r"] * ANN * 100  # annualized %
firm_vol = firm_vol.merge(market, on="ym")
print(f"\nvol_r (ann%): mean={firm_vol['vol_r'].mean():.2f}  sd={firm_vol['vol_r'].std():.2f}")
...
# Regression per firm: vol_r ~ vol_FTSE + vol_SP500 + vol_FX (all in ann%)
results = {}
for gv, gr in merged.groupby("gvkey"):
    g = gr.dropna(subset=["vol_r","vol_FTSE","vol_SP500","vol_FX"])
    if len(g) < 24:
        continue
    X = np.column_stack([np.ones(len(g)), g["vol_FTSE"], g["vol_SP500"], g["vol_FX"]])
    y = g["vol_r"].values
    try:
