"""Table 8 col 1 DiD: CASH ~ POST*HIGH + controls(t-1) + Firm FE + Ind*Qtr FE.
Two-way cluster by firm + calendar quarter. Paper: delta=0.231*** (SE 0.059)"""
...
crsp = pd.concat(frames); crsp["date"]=pd.to_datetime(crsp["date"]); crsp["RET"]=pd.to_numeric(crsp["RET"],errors="coerce")
...
cr2=pd.concat(f2); cr2["date"]=pd.to_datetime(cr2["date"]); cr2["sprtrn"]=pd.to_numeric(cr2["sprtrn"],errors="coerce")
cr2["ym"]=cr2["date"].dt.to_period("M"); sp=cr2[["date","sprtrn","ym"]].drop_duplicates()
spg=sp.groupby("ym"); sp500=spg["sprtrn"].std(); sp500=sp500[spg["sprtrn"].count()>=MIN_DAYS].reset_index()
...
for pn,grp in mg.groupby("PERMNO"):
    ...
    try:
        b=np.linalg.lstsq(X,y,rcond=None)[0]; yh=X@b; ssr=np.sum((y-yh)**2); sst=np.sum((y-y.mean())**2)
...
high_gvkeys=set(betas[betas["HIGH"]==1]["gvkey"]); low_gvkeys=set(betas[betas["LOW"]==1]["gvkey"])
allbeta_gvkeys=high_gvkeys|low_gvkeys
print(f"beta: {len(betas):,} firms  HIGH={len(high_gvkeys):,} LOW={len(low_gvkeys):,}")

print("\n"+"="*60)
print("2. COMPUSTAT F1-F7 + FIC (full 2010-2016 panel)")
print("="*60)
...
comp=comp[(comp["fyearq"]>=2009)&(comp["fyearq"]<=2017)]
...
fic["gvkey"]=fic["gvkey"].astype(str).str.zfill(6); fic=fic[(fic["year"]>=2010)&(fic["year"]<=2017)]
comp_f7["year"]=comp_f7["cal_yr_qtr"]//10
comp_f7=comp_f7.merge(fic,on=["gvkey","year"],how="inner")
f7_keys=set(comp_f7["gvkey"].unique())
print(f"F7+FIC: {len(f7_keys):,} firms, {len(comp_f7):,} obs")

# Keep only 2010-2016 for regression
comp_f7=comp_f7[(comp_f7["fyearq"]>=2010)&(comp_f7["fyearq"]<=2016)]

print("\n"+"="*60)
print("3. BUILD ALL VARIABLES (including lags)")
print("="*60)

# Panel index
comp_f7["entity"]=comp_f7["gvkey"]
comp_f7["time"]=comp_f7["cal_yr_qtr"]
comp_f7=comp_f7.set_index(["entity","time"]).sort_index()

# CASH = cheq / atq_l1 (already computed)
comp_f7["CASH"]=comp_f7["cheq"]/comp_f7["atq_l1"]

# SIZE
comp_f7["SIZE"]=np.log(comp_f7["atq"])

# CASH_FLOW
comp_f7["CASH_FLOW"]=comp_f7["oibdpq"]/comp_f7["atq_l1"]
...
# Winsorize all at 1%
for v in ["CASH","CASH_FLOW","TOBIN_Q","SALES_GROWTH"]:
    lo=comp_f7[v].quantile(0.01); hi=comp_f7[v].quantile(0.99)
    comp_f7[v]=comp_f7[v].clip(lo,hi)

lo_sz=comp_f7["SIZE"].quantile(0.01); hi_sz=comp_f7["SIZE"].quantile(0.99)
comp_f7["SIZE"]=comp_f7["SIZE"].clip(lo_sz,hi_sz)

# LAG all controls by 1 quarter (t-1)
for v in ["SIZE","CASH_FLOW","TOBIN_Q","SALES_GROWTH"]:
    comp_f7[f"{v}_lag"]=comp_f7.groupby(level=0)[v].shift(1)

print(f"Panel obs: {len(comp_f7):,}")

print("\n"+"="*60)
print("4. STOCK_RETURNS + CONSENSUS_EPS (merge)")
print("="*60)

# STOCK_RETURNS (quarterly BHR, then lag)
frames3=[]
...
cep["entity"]=cep["gvkey"]; cep["time"]=cep["cyq"]; cep=cep.set_index(["entity","time"])[["CEPS"]]
cep.columns=["CONSENSUS_EPS"]

# Merge SR + CEPS
comp_f7=comp_f7.merge(sr,left_index=True,right_index=True,how="left")
comp_f7=comp_f7.merge(cep,left_index=True,right_index=True,how="left")
...
# Complete-case
vars_need=["CASH","SIZE_lag","CASH_FLOW_lag","TOBIN_Q_lag","SALES_GROWTH_lag","STOCK_RETURNS_lag","CONSENSUS_EPS","beta_uk","icode100","POST","HIGH","interaction"]
did_cc=did.dropna(subset=vars_need)
print(f"CC: {len(did_cc):,} obs, {did_cc['gvkey'].nunique():,} firms")

# Set panel index for regression
...
print(f"\nPOST=1: {did_cc['POST'].sum():,}  POST=0: {(did_cc['POST']==0).sum():,}")
print(f"Interaction=1: {did_cc['interaction'].sum():,}")
print(f"Paper N=17,170  Paper delta=0.231*** (0.059)")

print("\n"+"="*60)
print("5. DID REGRESSION")
print("="*60)

# Formula: CASH ~ interaction + SIZE_lag + CASH_FLOW_lag + TOBIN_Q_lag + SALES_GROWTH_lag + STOCK_RETURNS_lag + CONSENSUS_EPS + EntityEffects + ind_time FE
did_cc["const"]=1.0

from linearmodels.panel import PanelOLS

# DV: Table 8 formula — CASH net of cash holdings
did_cc["CASH_DV"] = did_cc["CASH"] / (1 - did_cc["CASH"])  # cheq/(atq-cheq) = (cheq/atq)/(1-cheq/atq)

# Two approaches:
# A: Entity + Time FE (same as industry*quarter when industry FEs are absorbed)
# B: Entity + Industry*Quarter FE via absorption

Y = did_cc["CASH_DV"]
X = did_cc[["interaction","SIZE_lag","CASH_FLOW_lag","TOBIN_Q_lag",
            "SALES_GROWTH_lag","STOCK_RETURNS_lag","CONSENSUS_EPS"]]

# A: Entity + Time FE (paper: industry*quarter, but time FE is close when industry is time-invariant under entity FE)
print("\n--- Spec A: Entity + Time FE ---")
modA = PanelOLS(Y, X, entity_effects=True, time_effects=True, drop_absorbed=True)
resA = modA.fit(cov_type="clustered", cluster_entity=True, cluster_time=True)
print(resA)

# B: Entity + Industry*Quarter FE via absorption (this IS the paper's spec)
print("\n--- Spec B: Entity + Industry*Quarter FE (paper spec) ---")
did_cc["ind_time_cat"] = pd.Categorical(did_cc["ind_time"])
modB = PanelOLS(Y, X, entity_effects=True, other_effects=did_cc["ind_time_cat"], drop_absorbed=True)
resB = modB.fit(cov_type="clustered", cluster_entity=True, cluster_time=True)
print(resB)

print(f"\nPaper: delta=0.231*** (0.059), N=17,170, R2=0.21")
