"""β^UK with 24-month rolling window (paper page 32 fn 27 verbatim).
Use last 24m before end of estimation period (Dec 31, 2014)."""
...
START = pd.Timestamp("2013-01-01")  # 24m before Dec 2014
END = pd.Timestamp("2014-12-31")

vix = yf.download("^VIX", start="2012-12-01", end="2015-02-01", progress=False, auto_adjust=False)
...
crsp_frames = [pd.read_parquet(ROOT/f"inputs/CRSP_DSF/CRSP_DSF_{y}_Q{q}.parquet", columns=["PERMNO","date","RET","sprtrn"])
...
sp = crsp.drop_duplicates(["date"]).groupby("ym")["sprtrn"].std().reset_index().rename(columns={"sprtrn":"vol_SP500"})
market = ftse_vol.merge(vix_eom, on="ym").merge(fx_vol, on="ym").merge(sp, on="ym")
...
firm_vol["ym_date"] = pd.to_datetime((firm_vol["ym"]//100).astype(str)+"-"+(firm_vol["ym"]%100).astype(str).str.zfill(2)+"-15")
...
print(f"\nβ^UK 24m rolling (2013-2014, 4 controls):")
print(f"  N={len(df):,} mean={b.mean():.4f} sd={b.std():.4f} %neg={(b<0).mean()*100:.1f}%")
print(f"  terciles: t1={pos.quantile(1/3):.4f} t2={pos.quantile(2/3):.4f}")
