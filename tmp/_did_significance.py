"""DiD with expanded CONSENSUS_EPS (STDEV≥$0.01 + cross-sec demean)."""
...
print(f"CONSENSUS_EPS N={len(ceps):,} mean={ceps['CONSENSUS_EPS'].mean():.4f} sd={ceps['CONSENSUS_EPS'].std():.4f}")
...
for dv,label in [("CASH","CASH_T1"),("CASH_T8","CASH_T8")]:
    sub = did.dropna(subset=[dv,"beta_uk"]+[f"{c}_lag1" for c in ctrl_cols]).set_index(["gvkey","cal_yr_qtr"])
    exog = sub[["POST_BETA"]+[f"{c}_lag1" for c in ctrl_cols]]
    try:
        ...
        print(f"\n--- {label} Linear β DiD ---")
        print(f"  δ = {b:.4f}{sig}  SE={se:.4f}  t={t:.2f}  p={p:.4f}")
        print(f"  N = {res.nobs:,}")
