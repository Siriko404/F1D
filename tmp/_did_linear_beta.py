"""Paper Table 8 column 1: linear β^UK model (continuous treatment).
Y = α + δ·(POST × β^UK) + θ·CONTROLS + α_i + α_t + ε
Estimates δ using ALL firms (including middle tercile). Full N."""
...
for dv,label in [("CASH","CASH_T1"),("CASH_T8","CASH_T8")]:
    sub = did_panel.dropna(subset=[dv,"beta_uk"]+[f"{c}_lag1" for c in ctrl_cols]).copy()
    sub_di = sub.set_index(["gvkey","cal_yr_qtr"])
    exog = sub_di[["POST_BETA"]+[f"{c}_lag1" for c in ctrl_cols]]
    try:
        mod = PanelOLS(sub_di[dv], exog, entity_effects=True, time_effects=True)
        res = mod.fit(cov_type="clustered", cluster_entity=True, cluster_time=True)
        b = res.params["POST_BETA"]
        se = res.std_errors["POST_BETA"]
        t = res.tstats["POST_BETA"]
        p = res.pvalues["POST_BETA"]
        print(f"\n--- {label} Linear β model ---")
        print(f"  δ(POST×β^UK) = {b:.4f}  SE={se:.4f}  t={t:.2f}  p={p:.4f}  N={res.nobs:,}")
        print(f"  Paper Table 8 col 1: δ=-0.047*** (for INVESTMENT)")
        print(f"  Paper Table 8 col 1 CASH: β-based N=17,170")
    except Exception as e:
        print(f"\n{label}: PanelOLS failed: {e}")
