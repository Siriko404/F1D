...
OUT = Path(__file__).with_name("nlm_intro_conclusion_structure_detailed.json")

QUERY = (
    "Across the corporate finance empirical research papers in this notebook, "
    "synthesize a thorough, decisive description of how the Introduction "
    "section and the Conclusion section are written, so I can follow it as a "
    "convention. For EACH of the two sections, report: "
    "(a) the ordered sequence of rhetorical 'moves' (components in the order "
    "they appear); "
    "(b) for EACH move, its typical DEPTH: roughly how many sentences or "
    "paragraphs and an approximate word count, and what that move contains; "
    "(c) the typical TOTAL length of the whole section, in paragraphs and in "
    "words; "
    "(d) style conventions: verb tense, first-person 'we' versus impersonal "
    "voice, whether the main numerical results are previewed or restated with "
    "specific coefficients/figures or only described qualitatively, citation "
    "density, and how hedging or limitations are voiced; "
    "(e) how the Conclusion differs from the Introduction, what it adds and "
    "what it must not merely repeat. "
