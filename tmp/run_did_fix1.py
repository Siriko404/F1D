"""Table 8 col 1 DiD — Fix 1 applied: beta estimated only on Compustat F1-F8 survivors.
Paper: delta=0.231*** (SE 0.059), N=17,170, R2=0.21"""
...
# ============================================================
# 1. BUILD COMPUSTAT SURVIVOR LIST (F1-F8 + FIC)
# ============================================================
print("1. COMPUSTAT F1-F8 + FIC survivor list")
...
# ============================================================
# 2. BETA_UK — only for survivor PERMNOs
# ============================================================
print("2. BETA_UK (survivors only)")
...
print(f"  Survivor PERMNOs: {len(survivor_permnos):,}")
...
# RANK tercile: split nonnegative into equal thirds (paper says "upper/bottom tercile")
...
print(f"  beta firms: {len(betas):,}  HIGH: {betas['HIGH'].sum():,}  LOW: {betas['LOW'].sum():,}")
print(f"  Tercile T1/T2: {t1:.4f}/{t2:.4f}")

# ============================================================
# 3. COMPUSTAT PANEL + VARIABLES (same as run_did.py but survivor-based)
# ============================================================
print("3. COMPUSTAT panel + variables")
...
comp=comp.sort_values(["gvkey","datadate"]); comp["atq_l1"]=comp.groupby("gvkey")["atq"].shift(1)
...
for v in ["CASH","CASH_FLOW","TOBIN_Q","SALES_GROWTH"]:
    lo,hi=comp_f7[v].quantile(0.01),comp_f7[v].quantile(0.99); comp_f7[v]=comp_f7[v].clip(lo,hi)
...
# ============================================================
# 4. STOCK_RETURNS + CONSENSUS_EPS
# ============================================================
print("4. STOCK_RETURNS + CONSENSUS_EPS")
frames3=[]
...
f7_keys=set(comp_f7.reset_index()["entity"].unique()); cep=cep[cep["gvkey"].isin(f7_keys)]
cep["Mw"]=np.nan
for qt,ix in cep.groupby("cyq").groups.items():
    v=cep.loc[ix,"M"]
    if v.notna().sum()<10: continue
    lo,hi=v.quantile(0.01),v.quantile(0.99); cep.loc[ix,"Mw"]=v.clip(lo,hi)
cep["CEPS"]=np.nan
for qt,ix in cep.groupby("cyq").groups.items():
    v=cep.loc[ix,"Mw"]
    if v.notna().sum()<10: continue
...
print(f"    HIGH: {did[did['HIGH']==1]['gvkey'].nunique():,}  LOW: {did[did['LOW']==1]['gvkey'].nunique():,}")
...
print(f"  Complete-case: {len(did_cc):,} obs, {did_cc['gvkey'].nunique():,} firms")
print(f"    Paper: N=17,170, 449T/360C")
...
# DV: Table 8 formula
did_cc["CASH_DV"] = did_cc["CASH"] / (1 - did_cc["CASH"])
...
print(f"Previous run (no fixes): delta=0.067 (0.060), N=18,660, R2=0.03")
