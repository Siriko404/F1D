"""β^UK: quarterly vol_r, all in SAME units (raw monthly/daily decimal, no annualization).
vol_r = monthly std of daily returns (decimal).
vol_FTSE = VFTSE level / (100*√252) → daily decimal.
vol_SP500 = VIX level / (100*√252) → daily decimal.
vol_FX = monthly std of daily GBP/USD returns (decimal)."""
...
vix_eom = vix.groupby("ym")["Close"].last().reset_index()
vix_eom["VIX_daily"] = vix_eom["Close"] / (100 * ANN)

# 3. FX: monthly raw std (decimal)
...
market = vftse_eom[["ym","VFTSE_daily"]].merge(vix_eom[["ym","VIX_daily"]], on="ym").merge(fx_vol, on="ym")

# 4. CRSP monthly firm vol (raw decimal)
crsp_frames = []
...
ctrls = ["VFTSE_daily","VIX_daily","vol_FX"]
results = {}
...
print(f"\nβ^UK [daily-decimal, VFTSE+VIX+FX]:")
print(f"  N={len(df):,}  mean={b.mean():.4f}  sd={b.std():.4f}  %neg={(b<0).mean()*100:.1f}%")
print(f"  terciles: t1={pos.quantile(1/3):.4f}  t2={pos.quantile(2/3):.4f}  (paper 0.28/0.68)")
